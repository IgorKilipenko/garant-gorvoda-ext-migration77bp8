...
Процедура ГП_УстановитьФильтрПоДоговоруНаСервере(Знач ДоговорСсылка)
    ...
    МассивКонтрагентовОтбора = ГП_ПолучитьСписокКонтрагентовДоговораНаСервере(ДоговорСсылка);
    ОтборПоДоговору.ПравоеЗначение = МассивКонтрагентовОтбора;
    ОтборПоДоговору.Использование = Истина;
КонецПроцедуры

&НаСервере
Функция ГП_ПолучитьСписокКонтрагентовДоговораНаСервере(Знач ДоговорСсылка)
    Запрос = Новый Запрос;
    Запрос.Текст =
        ...
        |   И ДоговорыКонтрагентов.Владелец = Контрагенты.Ссылка
        |";

    Запрос.УстановитьПараметр("Договор", ДоговорСсылка);

    РезультатЗапроса = Запрос.Выполнить();
    РезультатФункции = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Контрагент");
    Возврат РезультатФункции;
КонецФункции
