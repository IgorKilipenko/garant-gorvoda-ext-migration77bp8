...
Процедура ПолучитьОбщуюТаблицуРасшифровкиОплат()
    ...
    ЭтотОбъект.Объект.ОбщаяТаблицаРасшифровкиОплат = РезультатЗапроса.Скопировать();
    ЭтотОбъект.Объект.ОбщаяТаблицаРасшифровкиОплат.Индексы.Добавить("Ссылка, Объект");
    // Гарант+ Килипенко --
КонецПроцедуры
...
Функция ЗаполнитьРасшифровкиПлатежа(СтруктураПараметров)
    ...
    РезультатПоиска = ЭтотОбъект.Объект.ОбщаяТаблицаРасшифровкиОплат.НайтиСтроки(СтруктураПараметров);
...
Процедура ВыборкаОстатков(Документ, ЛицевойСчет, Услуга)
    ...
    ЭтотОбъект.Объект.ТаблицаОстатков = Запрос.Выполнить().Выгрузить();
    // Гарант+ Килипенко --
...
Процедура ПолучитьОбщуюТаблицуОстатковПоДокументу(Документ)
    ...
    Если Документ <> ЗапомнитьДокумент Тогда
        ...
        ЭтотОбъект.Объект.ТаблицаОстатков = Запрос.Выполнить().Выгрузить();
        ЭтотОбъект.Объект.ТаблицаОстатков.Индексы.Добавить("ЛицевойСчет, Услуга");
        // Гарант+ Килипенко --
