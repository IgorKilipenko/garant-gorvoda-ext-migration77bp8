...
Процедура СтрокиРегистрацииОплатПриАктивизацииСтроки(Элемент)
    ...
    Если Элементы.СтрокиРегистрацииОплат.ТекущиеДанные <> Неопределено Тогда 
        ...
        РасшифровкаСтрокиРегистрацииОплат.Очистить();
        ...
        Для Каждого СтрокаТЧ Из ТЧЧека Цикл
            ...
            НоваяСтрокаДляОтображения = РасшифровкаСтрокиРегистрацииОплат.Добавить();
...
Процедура Заполнить(Команда)
    ...
    Если БазаДанных = "ALL" Тогда
        ...
        ПолучитьДанные1С7(Логин_, Пароль_, "D:\CHSEK2018-19", "CHSEK");
        ПолучитьДанныеBUH(Логин_, Пароль_, "D:\buh2022-23", "BUH");
...
&НаКлиенте
Процедура ПолучитьДанные1С7(Логин, Пароль, Путь, ИмяБазы, Подключение = Неопределено) 
	
	Если Подключение = Неопределено Тогда
	    Попытка
	        
	        ПолучениеБазы1С7(Логин, Пароль, Путь, ИмяБазы);
			Возврат;
    ...
    ВсегоВТЗ = ТЗ.Количество();
    ИндВТЗ = 0;
    Для Каждого СтрокаОплаты Из ТЗ Цикл
...
&НаКлиенте
Процедура ПолучениеБазы1С7(Логин, Пароль, Путь, ИмяБазы)
...
Процедура ЗакрытаФормаПодключенияКБазе(Ответ,ДопПараметры)Экспорт
	Если Ответ = Неопределено Тогда
		Сообщить("Не удалось произвести подключение к информационной базе");
	Иначе
		ПолучитьДанныеBUH(ДопПараметры.Логин,ДопПараметры.Пароль,ДопПараметры.Путь,ДопПараметры.ИмяБазы,Ответ);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьДанныеBUH(Логин, Пароль, Путь, ИмяБазы, Подключение = Неопределено)        
...
Функция ЗаполнитьНаСервере()
    ...
    ИндексВыполнения = 0;
    ЗапомнитьОбъект = Null;
	ЗапомнитьДокумент = Null;
    ЭтоПовтор = Ложь;
    ...
    Для Каждого ВыборкаДетальныеЗаписи Из РезультатЗапроса Цикл
		...
		НоваяСтрока.external_id          = УникальныйНомерДокумента(ВыборкаДетальныеЗаписи.Ссылка.Дата, ВыборкаДетальныеЗаписи.Ссылка.Номер, ВыборкаДетальныеЗаписи.Объект.Код);	
        ...
        ЗапомнитьОбъект = ВыборкаДетальныеЗаписи.Объект;
...
Функция ЗаполнитьРасшифровкиПлатежа(СтруктураПараметров)
    ...
    РезультатПоиска = ОбщаяТаблицаРасшифровкиОплат.НайтиСтроки(СтруктураПараметров);
	...
	// добавил УслугаДляПараметра "Экспресс-анализ"
            ...
            Если ВыборкаДетальныеЗаписи.Сумма > 0 И ВыборкаДетальныеЗаписи.ДолгСумма = 0 Тогда // На аванс
				...
				Если ТаблицаОстатков <> Неопределено Тогда 
                	ДеталиА = ТаблицаОстатков.НайтиСтроки(СП);
				КонецЕсли;
                
                // проверяем чтобы была только одна строка в результате,
                // иначе выводим ошибку
                Если ТаблицаОстатков <> Неопределено И ДеталиА.Количество() = 1 Тогда
                    ...
                    Иначе 
                        ...
                        Сообщить("Ошибка (код 7). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")" + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                        Объект.РасшифровкаСтрокРегистрациОплат.Очистить();
                        Объект.СтрокиРегистрацииОплат.Очистить();
                        Объект.ТЧЧека.Очистить();
                        Возврат Ложь;
                ...
                Иначе
                    ...
                    Сообщить("Ошибка (код 6). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")" + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                    Объект.РасшифровкаСтрокРегистрациОплат.Очистить();
                    Объект.СтрокиРегистрацииОплат.Очистить();
                    Объект.ТЧЧека.Очистить();
                    Возврат Ложь;    
            ...
            ИначеЕсли ВыборкаДетальныеЗаписи.Сумма = ВыборкаДетальныеЗаписи.ДолгСумма Тогда // Полная оплата
				...
				Если ТаблицаОстатков <> Неопределено Тогда 
					ДеталиПО = ТаблицаОстатков.НайтиСтроки(СП);
				КонецЕсли;                
				
				Если ТаблицаОстатков = Неопределено ИЛИ ДеталиПО.Количество() = 0 Тогда
                ...
                ИначеЕсли ДеталиПО.Количество() = 1 Тогда
                    ...
                    Если ДеталиПО[0].СуммаНачисленияОстаток > 0 Тогда 
                        ...
                        ДеталиПО2 = ТаблицаОстатков.НайтиСтроки(СП);
                        СуммаДляПроверки = 0;
                ...
                Иначе
                    ...
                    Сообщить("Ошибка (код 5). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")"  + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                    Объект.РасшифровкаСтрокРегистрациОплат.Очистить();
                    Объект.СтрокиРегистрацииОплат.Очистить();
                    Объект.ТЧЧека.Очистить();
                    Возврат Ложь;    
            ...
            ИначеЕсли ВыборкаДетальныеЗаписи.Сумма > ВыборкаДетальныеЗаписи.ДолгСумма Тогда
				...
				Если ТаблицаОстатков <> Неопределено Тогда 
					ОстаткиПроверки = ТаблицаОстатков.НайтиСтроки(СП);
				иначе
                ...
                Для Каждого СтрОст Из ОстаткиПроверки Цикл 
                    ...
                    Если СтрОст.СуммаНачисленияОстаток > 0 Тогда 
                        ...
                        Сообщить("Ошибка (код 4). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")"  + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                        Объект.РасшифровкаСтрокРегистрациОплат.Очистить();
                        Объект.СтрокиРегистрацииОплат.Очистить();
                        Объект.ТЧЧека.Очистить();
                        Возврат Ложь;
            ...
            ИначеЕсли ВыборкаДетальныеЗаписи.Сумма < ВыборкаДетальныеЗаписи.ДолгСумма Тогда 
                ...
                Детали = ТаблицаОстатков.НайтиСтроки(СП);
                        ...
                        Если (СаммаПослеПроведенияДокумента * -1) > ВыборкаДетальныеЗаписи.Сумма 
                            ...
                            Если ОстаткиПроверкиАванса.Количество() <> 0
                                ...
                                Иначе 
                                    ...
                                    Сообщить("Ошибка (код 35). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")"  + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);    
                        ...
                        Иначе // здесь сумма после оплаты , меньше суммы самой оплаты(пока что не понятно, что делать в этом случе, вывожу ошибку)
                            ...
                            Для Каждого СтрПроверкиУслуги Из ОбщийОстатокПоВсемУслугам Цикл
                                ...
                                Если СтрПроверкиУслуги.Услуга.Наименование = "Горячее водоснабжение" Тогда 
                                    ...
                                    Сообщить("ВАЖНО!!!Исправьте(остатки на ГВ). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")"  + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                                    Продолжить;
                            ...
                            Если ОбщийОстатокПоВсемУслугам.Количество() <> 0
                                ...
                                Сообщить("ВАЖНО!!! Исправьте(скорее всего сумма оплаты разбита некорректно). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")"  + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                                
                            Иначе 
                                
                                Сообщить("ВАЖНО!!! Исправьте(должна быть полная оплата). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")"  + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                ...
                Иначе
                    ...
                    Сообщить("Ошибка (код 2). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")"  + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                    Объект.РасшифровкаСтрокРегистрациОплат.Очистить();
                    Объект.СтрокиРегистрацииОплат.Очистить();
                    Объект.ТЧЧека.Очистить();
                    Возврат Ложь;
            ...
            Иначе
                ...
                Сообщить("Ошибка (код 1). Лицевой счет: " + ВыборкаДетальныеЗаписи.Объект.Наименование + "(" + ВыборкаДетальныеЗаписи.Объект.Код + ")"  + ";" + ВыборкаДетальныеЗаписи.Объект.Адрес.Владелец.Наименование);
                Объект.РасшифровкаСтрокРегистрациОплат.Очистить();
                Объект.СтрокиРегистрацииОплат.Очистить();
                Объект.ТЧЧека.Очистить();
                Возврат Ложь;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Объект.КаталогДляВременныхФайлов = "D:\TMA\TokenSchetmash";
    Объект.token = "";
	Объект.id_Магазина = 59;
	Объект.login = "wkx_53";
	Объект.password = "P9cyRb3?";
    Объект.Организация = Справочники.Организации.НайтиПоКоду("00-000001");
	//Объект.id_Магазина = 42;
	//Объект.login = "test_api";
	//Объект.password = "123456";
    Объект.ПрефикНомера = "P_";
    Объект.ПоУполчаниюНаименованиеУслуги = Справочники.КВП_Услуги.НайтиПоКоду("000001");
    Объект.РасшифровыватьУслуги = Истина;
    Объект.ПоУмолчанию_type = 1;
    Объект.ПоУмолчанию_ПризнакПредметаРасчета = 4;
    Объект.ПоУмолчанию_ПризнакСпособаРасчета = 1;
    
    ХВ = Справочники.КВП_Услуги.НайтиПоНаименованию("Холодное водоснабжение");
    ГВ = Справочники.КВП_Услуги.НайтиПоНаименованию("Горячее водоснабжение");
    Кон = Справочники.КВП_Услуги.НайтиПоНаименованию("Водоотведение");
	Подогрев = Справочники.КВП_Услуги.НайтиПоНаименованию("Холодная вода на подогрев в целях предоставления горячего водоснабжения");
...
Процедура КоррекцияПрихода(Команда)
    ...
    ФормаДоступа = ПолучитьФорму("ВнешняяОбработка.APIСервисСчетмашОнлайн_v6_1.Форма.ФормаДоступа");
    Проверка = ФормаДоступа.ОткрытьМодально();
