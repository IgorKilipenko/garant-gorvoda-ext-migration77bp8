...
Процедура ГП_ПроверкаНаОшибки(ИдентификаторСтроки, ПриЗаполненииТаблицы)
    ...
    Если НЕ ТекВетка.ПолучитьРодителя() = Неопределено Тогда
        ...
        Если ТекВетка.Сумма = 0 И НЕ (ТекВетка.ЕстьРО И Не ТекВетка.ПолучитьРодителя() = Неопределено И ТекВетка.ПолучитьРодителя().СуммыСовпадают) Тогда
            ...
            // Готовность = ГотовностьДляРаспределения;
            // Если ТекВетка.ДоговорСовпадает Тогда
            //     ТекВетка.ПолучитьРодителя().Готовность = ?(ГотовностьДляРаспределения = Истина, 1, 0);
            // КонецЕсли;
        ...
        Если НЕ ТекВетка.ПолучитьРодителя() = Неопределено И НЕ ТекВетка.ПолучитьРодителя().СуммыСовпадают Тогда
            ...
            ГотовностьДляРаспределения = Ложь;
        ...
        Если ПриЗаполненииТаблицы Тогда
            ...
            //  Если ГотовностьДляРаспределения = Ложь Тогда
            //      ТекВетка.Пометка = Ложь;
            //  Иначе
            //      ТекВетка.Готовность = 1;
            //  КонецЕсли;
            Если ГотовностьДляРаспределения = Истина И ТекВетка.Сумма = 0 Тогда
                РодительТекВетки.Готовность = 1;
                ЭтотОбъект.ГП_ДополнительныеСвойстваФормы.КэшВетокРаспределения.Вставить(РодительТекВетки.ПолучитьИдентификатор(), Истина);
...
Функция ГП_ЗаполнитьТаблицыДаннымиДляФормированияРОПоискПоКонтрагенту(ТаблицаРО, ТаблицаБезРО, ТаблицаПлатежныхДокументов)
    ...
    ДополнитьКолонкиТаблицыБезРОНаСервере(ТаблицаБезРО);
...
&НаКлиенте
&После("ЗаполнитьДанныеНаКлиентеПродожение")
Процедура ГП_ЗаполнитьДанныеНаКлиентеПродожение()
    Если Объект.ЭтоПоискПоКонтрагенту = Истина И ДеревоПлатежныхДокументовСОшибками.ПолучитьЭлементы().Количество() > 0 Тогда
        ГП_ПринятьИзмененияСуммДереваСОшибками();
    КонецЕсли;
КонецПроцедуры
...
Функция ДополнитьКолонкиТаблицыБезРОНаСервере(Знач ТаблицаБезРО)
...
&НаКлиенте
Процедура ГП_ПринятьИзмененияСуммДереваСОшибками()
    Для каждого ТекВеткаКЗ Из ЭтотОбъект.ГП_ДополнительныеСвойстваФормы.КэшВетокРаспределения Цикл
        ТекВетка = ЭтотОбъект.ДеревоПлатежныхДокументовСОшибками.НайтиПоИдентификатору(ТекВеткаКЗ.Ключ);
