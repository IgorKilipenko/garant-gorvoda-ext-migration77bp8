...
Процедура ГП_СформироватьНовыеДокументыРО(ПараметрыФормирования, ТаблицаБезРО, ТаблицаПлатежныхДокументов, МассивСформированныхДокументов)
    ...
    Если НЕ ТаблицаБезРО.Количество() = 0 Тогда
        ...
        //ПоляГруппировкиДанных.ЗаполнитьЗначения(Неопределено, "Дом");
        ПоляГруппировкиДанных = "Дата, КонтрагентРодительВетки, ВидОперации, ВариантРаспределенияОплатКапРемонт, НеРаспределятьОплатуНаДобровольноеСтрахование, ВидОплаты";
        ГруппировкаПлатежей.Свернуть(ПоляГруппировкиДанных);
...
Функция ДополнитьКолонкиТаблицыБезРОНаСервере(Знач ТаблицаБезРО)
    ...
    ИмяКолонкиДополнения = "КонтрагентРодительВетки";
    РезультатФункции = ТаблицаБезРО.Колонки.Найти(ИмяКолонкиДополнения);
    Если РезультатФункции = Неопределено Тогда
        РезультатФункции = ТаблицаБезРО.Колонки.Добавить(ИмяКолонкиДополнения, Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
    КонецЕсли;
