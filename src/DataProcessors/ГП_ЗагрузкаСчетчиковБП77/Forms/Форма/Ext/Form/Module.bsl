...
Асинх Процедура СопоставитьСчетчики(Команда)
    РезультатДиалога = Ждать ВопросАсинх(
            "Значения поля Счетчик в регистре ГП_СчетчикиБП77 будут обновлены для всех записей. Продолжить?",
...
&НаКлиенте
Асинх Процедура ОбновитьНазначенияУслуг(Команда)
    РезультатДиалога = Ждать ВопросАсинх(
            "Будут созданы и обновлены документы ""Назначение услуг"" по данным записей регистра Счетчики БП 7.7.
            |Продолжить?", РежимДиалогаВопрос.ДаНет, , , "Внимание!");
    Если РезультатДиалога <> КодВозвратаДиалога.Да Тогда
        Возврат;
    КонецЕсли;

    РезультатСозданияДокументов = СоздатьДокументыУправленияУслугамиНаСервере();
...
//
&НаСервереБезКонтекста
Функция СоздатьДокументыУправленияУслугамиНаСервере()
    СписокУслугДляСозданияДокументов = Новый Массив;
    СписокУслугДляСозданияДокументов.Добавить("ХолоднаяВода");

    Возврат Обработки.ГП_ЗагрузкаСчетчиковБП77.СоздатьДокументыУправлениеУслугамиПоДаннымРегистраСчетчиковБП77(
        СписокУслугДляСозданияДокументов);
КонецФункции
