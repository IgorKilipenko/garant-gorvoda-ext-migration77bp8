...
// Гарант+ Килипенко 16.08.2024 [F00226620] назначение тарифов ++
&НаКлиенте
Асинх Процедура СоздатьДокументыУстановкиТарифов(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будут созданы документы ""Установка тарифов"" по данным из БП 7.7"
            + " (по данным из регистра ""Тарифы объектов абонентов БП 7.7"").");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    // Массив видов услуг для создания
    МассивВидовУслугБП77 = Новый Массив;
    Для Каждого ЭлементУслуги Из ЭтотОбъект.СписокВидовУслугДляУправленияУслугами Цикл
        МассивВидовУслугБП77.Добавить(ЭлементУслуги.Значение);
    КонецЦикла;
    ...
    // Создание документов
    РезультатСозданияДокументов = СоздатьДокументыУстановкиТарифамиПоДаннымБП77НаСервере(МассивВидовУслугБП77);
    Для Каждого СообщениеОбОшибке Из РезультатСозданияДокументов.СообщенияОбОшибках Цикл
        ОбщегоНазначенияКлиент.СообщитьПользователю(СообщениеОбОшибке);
    КонецЦикла;
    ...
    Иначе
        СообщениеПользователю = СтрШаблон("Создание документов выполнено %1.
                |Создано ""%2"" новых документов",
                ?(РезультатСозданияДокументов.СообщенияОбОшибках.Количество() > 0, "с ошибками", "успешно"),
                РезультатСозданияДокументов.СозданныеДокументы.Количество());
    КонецЕсли;
...
&НаКлиенте
Асинх Процедура СоздатьБлокВиртуальныхСчетчиковОИ(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет создан блок виртуальных счетчиков ОИ.");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    РезультатСоздания = СоздатьБлокВиртуальныхСчетчиковОИНаСервере();
    Если РезультатСоздания.Успех = Истина Тогда
        ОбщегоНазначенияКлиент.СообщитьПользователю("Выполнено создание блока виртуальных счетчиков ОИ");
    Иначе
        ОбщегоНазначенияКлиент.СообщитьПользователю("Создание блока виртуальных счетчиков ОИ не выполнено");
    КонецЕсли;
КонецПроцедуры

&НаКлиенте
Асинх Процедура СоздатьБлокВиртуальныхСчетчиковПотерьВоды(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет создан блок виртуальных счетчиков потерь воды.");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    РезультатСоздания = СоздатьБлокВиртуальныхСчетчиковПотерьВодыНаСервере();
    Если РезультатСоздания.Успех = Истина Тогда
        ОбщегоНазначенияКлиент.СообщитьПользователю("Выполнено создание блока виртуальных счетчиков потерь воды");
    Иначе
        ОбщегоНазначенияКлиент.СообщитьПользователю("Создание блока виртуальных счетчиков потерь воды не выполнено");
    КонецЕсли;
КонецПроцедуры

&НаКлиенте
Асинх Процедура СоздатьБлокВиртуальныхСчетчиковПоСреднемуВ2(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет создан блок виртуальных счетчиков по среднему (в.2).");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    РезультатСоздания = СоздатьБлокВиртуальныхСчетчиковПоСреднемуВ2НаСервере();
    Если РезультатСоздания.Успех = Истина Тогда
        ОбщегоНазначенияКлиент.СообщитьПользователю("Выполнено создание блока виртуальных счетчиков по среднему (в.2)");
    Иначе
        ОбщегоНазначенияКлиент.СообщитьПользователю("Создание блока виртуальных счетчиков по среднему (в.2) не выполнено");
    КонецЕсли;
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция СоздатьДокументыУстановкиТарифамиПоДаннымБП77НаСервере(Знач МассивВидовУслугБП77)
    Возврат ГП_МиграцияТарифовАбонентов.СоздатьДокументыУстановкиТарифамиПоДаннымБП77(МассивВидовУслугБП77);
...
// Выполняет создание, установку, назначение услуг и зависимостей виртуальных счетчиков ОИ
// Возвращаемое значение:
//  - Структура
&НаСервереБезКонтекста
Функция СоздатьБлокВиртуальныхСчетчиковОИНаСервере()
...
//  - Структура
&НаСервереБезКонтекста
Функция СоздатьБлокВиртуальныхСчетчиковПотерьВодыНаСервере()
    РезультатФункции = Новый Структура("Успех", Истина);
...
// Выполняет создание и установку виртуальных счетчиков по среднему (в.2)
// Возвращаемое значение:
//  - Структура
&НаСервереБезКонтекста
Функция СоздатьБлокВиртуальныхСчетчиковПоСреднемуВ2НаСервере()
    РезультатФункции = Новый Структура("Успех", Истина);

    // Создание счетчиков
    РезультатСозданияСчетчиков = ГП_МиграцияПриборовУчета.СоздатьВиртуальныеСчетчикиПоСреднемуДляЛСБезСчетчиков(ТекущаяДатаСеанса());
    Если РезультатСозданияСчетчиков.Успех = Ложь Тогда
        ОбщегоНазначения.СообщитьПользователю(РезультатСозданияСчетчиков.ТекстСообщения);
        РезультатФункции.Успех = Ложь;
        Возврат РезультатФункции;
    Иначе
        ОбщегоНазначения.СообщитьПользователю("Выполнена создание виртуальных счетчиков по среднему (в.2)");
    КонецЕсли;

    // Установка счетчиков
    РезультатУстановкиСчетчиков = ГП_МиграцияПриборовУчета.СоздатьДокументыУстановкиВиртуальныхСчетчиковПоСреднемуДляЛСБезСчетчиков(
            Дата(2024, 01, 01), Дата(2024, 01, 01));
    Если РезультатУстановкиСчетчиков.Успех = Ложь Тогда
        ОбщегоНазначения.СообщитьПользователю(РезультатУстановкиСчетчиков.ТекстСообщения);
        РезультатФункции.Успех = Ложь;
        Возврат РезультатФункции;
    Иначе
        ОбщегоНазначения.СообщитьПользователю("Выполнена установка виртуальных счетчиков по среднему (в.2)");
    КонецЕсли;
