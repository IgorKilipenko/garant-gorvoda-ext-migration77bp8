...
Функция СоздатьДокументыУправлениеУслугамиПоДаннымРегистраСчетчиковБП77(
    ...
    ДанныеДляЗаполнения = ГП_МиграцияПриборовУчета.ПолучитьДанныеЗаполненияНазначенийУслуг();
...
Функция ЗагрузитьДанныеУслугВРегистр(ПараметрыЗагрузки, АдресХранилища = Неопределено) Экспорт
    ...
    // Чтение данных из файла XML
    РезультатЧтенияДанных = ГП_МиграцияПриборовУчета.ПрочитатьДанныеИзФайлаXML(ПараметрыЗагрузки.ДвоичныеДанныеФайла, "Услуга");
    Если РезультатЧтенияДанных.Успех = Ложь Тогда
        РезультатФункции.ТекстСообщения = РезультатЧтенияДанных.ТекстСообщения;
        Возврат ПоместитьВХранилищеИВернуть(РезультатФункции, АдресХранилища);
    КонецЕсли;
...
//      * СообщениеОбОшибке - Строка, Неопределено
Функция ЗаписатьДанныеУслугВРегистр(Знач ИсходныеДанныеУслуг, Знач СохранятьСвязи = Истина)
    ...
    Попытка
        ...
        Для Каждого УслугаБП77 Из ИсходныеДанныеУслуг Цикл
            ...
            ЭтоПлатаЗаХолодноеВодоснабжениеОИ =
                ГП_МиграцияПриборовУчета.ЭтоНаименованиеУслугиХолодногоВодоснабженияОИ(УслугаБП77.ОбъектАбонентаНаименование);
            ...
            ЭтоНегативноеВоздействиеЦСВ =
                ГП_МиграцияПриборовУчета.ЭтоНаименованиеУслугиНегативногоВоздействия(УслугаБП77.ОбъектАбонентаНаименование);
