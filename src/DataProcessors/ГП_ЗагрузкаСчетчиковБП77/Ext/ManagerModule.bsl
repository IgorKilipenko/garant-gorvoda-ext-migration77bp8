...
Процедура ЗагрузитьДанныеВИБ(ПараметрыВыгрузки, АдресХранилища) Экспорт
    ...
    Попытка
        СправочникиБП77 = ПрочитатьJSON(ЧтениеJSON);
    Исключение
    ...
    Если ТипЗнч(СправочникиБП77) <> Тип("Структура") ИЛИ СправочникиБП77.Свойство("Счетчики") = Ложь Тогда
        РезультатВыполнения.ТекстСообщения = НСтр("ru = 'Данные счетчиков не обнаружены.'");
        Возврат;
    КонецЕсли;

    РезультатЗаписи = ЗаписатьДанныеСчетчиковВРегистр(СправочникиБП77.Счетчики.Счетчик);
    ...
    Попытка
        ...
        Для Каждого СчетчикБП77 Из ИсходныеДанныеСчетчиков Цикл
            ...
            Для каждого Ключ Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КлючиУсловийСчетчика, ",", Ложь, Истина) Цикл
                НоваяЗапись[Ключ] = ?(ЗначениеЗаполнено(СчетчикБП77[Ключ]) = Ложь ИЛИ СчетчикБП77[Ключ] = 0, Ложь, Истина);
            КонецЦикла;
