...
Процедура ЗагрузитьДанныеСчетчиковВРегистр(ПараметрыЗагрузки, АдресХранилища) Экспорт
    РезультатВыполнения = Новый Структура("Успех, КоличествоЗаписанныхСчетчиков, ТекстСообщения", Ложь, 0);

    ИмяВременногоФайла = ПолучитьИмяВременногоФайла("xml");
    ДвоичныеДанныеФайла = ПараметрыЗагрузки.ДвоичныеДанныеФайла;
    ДвоичныеДанныеФайла.Записать(ИмяВременногоФайла);

    ЧтениеXML = Новый ЧтениеXML();
    ЧтениеXML.ОткрытьФайл(ИмяВременногоФайла);

    СчетчикиБП77 = Неопределено;
    Попытка
        // Выполняем разбор файла xml
        СчетчикиБП77 = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML);
    Исключение
    ...
    ЭтоДопустимыйОбъект = (ТипЗнч(СчетчикиБП77) = Тип("ОбъектXDTO") И СчетчикиБП77.Свойства().Получить("Счетчик") <> Неопределено);
    ...
    РезультатЗаписи = ЗаписатьДанныеСчетчиковВРегистр(СчетчикиБП77.Счетчик);
...
//      * ТекстСообщения - Строка
//  - Неопределено
Функция ЗагрузитьДанныеУслугВРегистр(ДвоичныеДанныеФайла, АдресХранилища) Экспорт
    РезультатВыполнения = Новый Структура("Успех, КоличествоЗаписанныхУслуг, ТекстСообщения", Ложь, 0);
    ...
    УслугиБП77 = Неопределено;
    Попытка
        // Выполняем разбор файла xml
        УслугиБП77 = ФабрикаXDTO.ПрочитатьXML(ЧтениеXML);
    Исключение
        // Ошибка разбора файла xml
        РезультатВыполнения.ТекстСообщения = НСтр("ru = 'Ошибка анализа файла xml.'");
        РезультатВыполнения.ТекстСообщения = СтрШаблон(
        ...
        Если АдресХранилища <> Неопределено Тогда
            ПоместитьВоВременноеХранилище(РезультатВыполнения, АдресХранилища);
            Возврат Неопределено; // Ошибка разбора файла
        Иначе
    ...
    // Проверка валидности данных счетчиков
    ЭтоДопустимыйОбъект = (ТипЗнч(УслугиБП77) = Тип("ОбъектXDTO") И УслугиБП77.Свойства().Получить("Услуга") <> Неопределено);
    Если ЭтоДопустимыйОбъект = Ложь Тогда
        РезультатВыполнения.ТекстСообщения = НСтр("ru = 'Данные услуг не обнаружены.'");

        Если АдресХранилища <> Неопределено Тогда
            ПоместитьВоВременноеХранилище(РезультатВыполнения, АдресХранилища);
            Возврат Неопределено; // Нет данных счетчиков или данные недопустимой структуры
        Иначе
            Возврат РезультатВыполнения;
        КонецЕсли;
    КонецЕсли;
    ...
    РезультатЗаписи = ЗаписатьДанныеУслугВРегистр(УслугиБП77.Услуга);

    // Формирование результата
    РезультатВыполнения.Успех = РезультатЗаписи.Успех;
    РезультатВыполнения.ТекстСообщения = РезультатЗаписи.СообщениеОбОшибке;
    РезультатВыполнения.КоличествоЗаписанныхУслуг = РезультатЗаписи.КоличествоЗаписанныхУслуг;
