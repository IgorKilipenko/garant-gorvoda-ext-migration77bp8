...
Функция ОбновитьСчетчикиИзРегистра(Знач ПараметрыОбновления, АдресХранилища = Неопределено) Экспорт
    РезультатФункции = Новый Структура("Успех, ОбновленныеСчетчики, НовыеСчетчики, КоличествоСопоставленных, ТекстСообщения", Истина, Новый Массив, Новый Массив, 0, "");
    ...
    Попытка
        ...
        Для Каждого СчетчикБП77 Из ТаблицаСчетчиковБП77 Цикл
            ...
            Если ЗначениеЗаполнено(СчетчикБП77.Счетчик) = Ложь И СоздаватьНовыеСчетчики Тогда
                ...
                СчетчикСсылка = СоздатьНовыйСчетчик(СчетчикБП77).ПолучитьОбъект();
    ...
    Если РезультатФункции.Успех Тогда
        РезультатСопоставления = УстановитьСоответствияСчетчиковРегистраСОбъектамиИБ(ТаблицаСчетчиковБП77);
        РезультатФункции.КоличествоСопоставленных = РезультатСопоставления.ОбновленныеСчетчики;
    КонецЕсли;
...
Функция УстановитьСоответствияСчетчиковРегистраСОбъектамиИБ(Знач ТаблицаСчетчиковРегистра)
    ...
    Попытка
        Для Каждого СчетчикБП77 Из ТаблицаСчетчиковРегистра Цикл
            Если СчетчикБП77.Счетчик = Неопределено ИЛИ ЗначениеЗаполнено(СчетчикБП77.Счетчик) = Ложь Тогда
                Продолжить;
            КонецЕсли;
...
Функция ПолучитьВсеСозданныеСчетчики()
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ
        ...
        |ГДЕ
        |	КВП_Счетчики.ГП_ИдентификаторБП77 <> """"
        |";
        РезультатЗапроса = Запрос.Выполнить();
    Возврат РезультатЗапроса.Выгрузить();
КонецФункции
