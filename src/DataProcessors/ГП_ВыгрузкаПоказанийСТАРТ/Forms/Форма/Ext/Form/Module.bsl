...
Асинх Процедура ЗаполнитьТаблицуПоказанийСчетчиков(Команда)
    ...
    ЗаполнитьТаблицуПоказанийСчетчиковНаСервере(
        ЭтотОбъект.Период.ДатаНачала, ЭтотОбъект.Период.ДатаОкончания, ЭтотОбъект.Услуга, ЭтотОбъект.ТолькоСПоказаниями);
КонецПроцедуры
...
&НаСервере
Процедура ЗаполнитьТаблицуПоказанийСчетчиковНаСервере(Знач НачалоПериода, Знач КонецПериода, Знач Услуга, Знач ТолькоСПоказаниями = Ложь)
    ДанныеЗаполнения = ГП_ОбменСоСТАРТ.ПолучитьДанныеПриборовУчетаДляВыгрузки(НачалоПериода, КонецПериода, Услуга, ТолькоСПоказаниями);
