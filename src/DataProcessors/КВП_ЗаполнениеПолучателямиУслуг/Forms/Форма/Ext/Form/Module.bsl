...
#Область ГП_ПереопределениеОбработчиковФормы
// Настраивает отбор формы выбора договора
&НаКлиенте
Процедура ГП_ОбъектЛСОбработкаВыбораПосле(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
    Если ВыбранноеЗначение = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		СтандартнаяОбработка = Ложь;

        ПараметрыОтбораДоговоров = Новый Структура("Отбор", Новый Структура);

        // Отбор договора по организации
        Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Организация) Тогда
            ПараметрыОтбораДоговоров.Отбор.Вставить("Организация", ЭтотОбъект.Объект.Организация);
        КонецЕсли;

        // Отбор договора по контрагенту
        Если ЭтотОбъект.КонтрагентыЛС.Количество() > 0 Тогда
            ТекущаяСтрокаКонтрагента = ЭтотОбъект.Элементы.КонтрагентыЛС.ТекущиеДанные;
            Если ТекущаяСтрокаКонтрагента = Неопределено
                ИЛИ ЗначениеЗаполнено(ТекущаяСтрокаКонтрагента.Контрагент) = Ложь Тогда
                ТекущаяСтрокаКонтрагента = ЭтотОбъект.КонтрагентыЛС[0].Контрагент;
            КонецЕсли;

            ПараметрыОтбораДоговоров.Отбор.Вставить("Владелец", ТекущаяСтрокаКонтрагента.Контрагент);
        КонецЕсли;

        // Открытие формы выбора договора
        ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьВыборГруппыЛицевыхСчетов", ЭтаФорма);
		ОткрытьФорму("Справочник.ДоговорыКонтрагентов.ФормаВыбора", ПараметрыОтбораДоговоров,
            ЭтотОбъект, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры
#КонецОбласти // ГП_ПереопределениеОбработчиковФормы
...
#Область ГП_ПереопределениеФункцийФормы
&НаСервере
...
КонецПроцедуры
#КонецОбласти // ГП_ПереопределениеФункцийФормы
