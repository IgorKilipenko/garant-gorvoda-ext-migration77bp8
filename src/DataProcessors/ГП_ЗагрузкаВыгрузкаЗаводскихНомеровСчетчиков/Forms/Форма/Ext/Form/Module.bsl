...
Функция ПерезаписатьЗначенияЗаводскихНомеровНаСервере()
    ...
    Запрос.Текст =
        ...
        |   ВТ_Счетчики.Наименование КАК Наименование,
        |   ВТ_Счетчики.ЗаводскойНомер КАК ЗаводскойНомер,
        |   ВТ_Счетчики.ГП_ИдентификаторБП77 КАК ГП_ИдентификаторБП77,
        ...
        |   ПО ВЫБОР
        |       КОГДА (ВТ_ДанныеОбмена.Счетчик ЕСТЬ NULL) ИЛИ (ВТ_Счетчики.Счетчик ЕСТЬ NULL)
        |           ТОГДА ВТ_Счетчики.ГП_ИдентификаторБП77 = ВТ_ДанныеОбмена.ГП_ИдентификаторБП77
        |               И ВТ_Счетчики.Наименование = ВТ_ДанныеОбмена.Наименование
        |       ИНАЧЕ ВТ_Счетчики.Счетчик = ВТ_ДанныеОбмена.Счетчик
        |   КОНЕЦ
        |ГДЕ
        |   ВТ_Счетчики.ЗаводскойНомер <> ВТ_ДанныеОбмена.ЗаводскойНомер
        |";

    Запрос.УстановитьПараметр("Счетчики", ИсходныеСчетчики);
