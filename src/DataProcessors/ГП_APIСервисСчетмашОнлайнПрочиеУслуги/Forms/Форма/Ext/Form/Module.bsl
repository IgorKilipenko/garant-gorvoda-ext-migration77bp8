...
Асинх Процедура ВыполнитьПробитиеЧека(Команда)
    ЭтоВыаринтВыполненияКомандыПоВсемСтрокам = СтрНайти(Команда.Имя, "ДляВсехСтрок", НаправлениеПоиска.СКонца) > 0;

    Если ЭтоВыаринтВыполненияКомандыПоВсемСтрокам = Ложь Тогда
...
Процедура ВыполнитьПроверкуСтатусаЧека(Команда)
    ...
    Если ЭтоВыаринтВыполненияКомандыПоВсемСтрокам = Ложь Тогда
        ...
        Состояние("Обновление статуса чека завршено");
    ...
    Иначе
        ...
        Состояние("Обновление статусов чеков завршено");
    КонецЕсли;
КонецПроцедуры
...
Процедура СтрокиРегистрацииОплатПриАктивизацииСтроки(Элемент)
    ...
    Если ЭтотОбъект.Элементы.СтрокиРегистрацииОплат.ТекущиеДанные <> Неопределено Тогда
        ...
        ЭтотОбъект.Элементы.РасшифровкаСтрокиРегистрацииОплатСумма.ТекстПодвала =
            Формат(ИтогиТаблицыРасшифровкаСтрокиРегистрацииОплат.Сумма, "ЧДЦ=2");
        ЭтотОбъект.Элементы.РасшифровкаСтрокиРегистрацииОплатАванс.ТекстПодвала =
            Формат(ИтогиТаблицыРасшифровкаСтрокиРегистрацииОплат.Аванс, "ЧДЦ=2");
        ЭтотОбъект.Элементы.РасшифровкаСтрокиРегистрацииОплатСуммаНДС.ТекстПодвала =
            Формат(ИтогиТаблицыРасшифровкаСтрокиРегистрацииОплат.СуммаНДС, "ЧДЦ=2");

        // Контроль сумм
        ДопустимаяПогрешностьСравненияСумм = 0.0001;
        РасхождениеСуммСтркиОплаты = ИтогиТаблицыРасшифровкаСтрокиРегистрацииОплат.Сумма
        ...
        Если ГП_ВспомогательныеФункцииКлиентСервер.АбсолютноеЗначение(
                РасхождениеСуммСтркиОплаты) > ДопустимаяПогрешностьСравненияСумм Тогда
...
Функция ПолучитьСтатусЧекаСчетмашНаСервере(
    ...
    Если РезультатПроверкиСтатуса.Успех = Ложь Тогда
        Если ВыводитьСообщения = Истина И РезультатПроверкиСтатуса.ДанныеОтвета.КодОшибки <> 8 Тогда
