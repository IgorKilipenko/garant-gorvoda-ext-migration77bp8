...
Функция СформироватьПечатнуюФормуДоговораПодряда(
    ...
    ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СчетНаОплатуПокупателю_ДоговорПодряда";
    ...
    Для Каждого ДокументСсылка Из МассивОбъектов Цикл
        ...
        ОтветственныеЛицаОрганизации = ОтветственныеЛицаБППовтИсп.ОтветственныеЛица(
                ДанныеЗаполнения.ДанныеДокумента.Организация, ДанныеЗаполнения.ДанныеДокумента.Дата); //!!!!!!!
        ...
        ОбластьШапка.Параметры.НомерДокумента = ДанныеЗаполнения.ДанныеДокумента.Номер;
        ...
        Если РуководительФИО <> Неопределено Тогда
            ОбластьШапка.Параметры.ФИОДиректора = РуководительФИО.ФамилияИО;
        КонецЕсли;
        ...
        Попытка
            ОбластьШапка.Параметры.ДолжностьФИОЗаказчика = СклонениеПредставленийОбъектов.ПросклонятьФИО(ПодготовитьПредставлениеКонтрагентаДляСклонения(
        ...
        Если ДанныеЗаполнения.ДанныеДокумента.ДокументБезНДС = Ложь Тогда
            ...
            //Если ДанныеЗаполнения.ДанныеДокумента.СуммаВключаетНДС = Истина Тогда
            //    ОбластьТелоДоговора.Параметры.ВариантНДС = ", в т. ч. НДС";
            //Иначе
            //    ОбластьТелоДоговора.Параметры.ВариантНДС = ", сумма НДС";
            //КонецЕсли;
...
Функция ПолучитьПредставлениеСпискаТоваров(Знач ТаблицаТоваров)
    ...
    Для Каждого СтрокаДанных Из ТаблицаТоваров Цикл
        РезультатФункции = СтрШаблон(" %1%2", РезультатФункции, СтрокаДанных.Содержание);
    КонецЦикла;

    Если СтрДлина(РезультатФункции) > 0 Тогда
        РезультатФункции = Сред(РезультатФункции, 2);
    КонецЕсли;
