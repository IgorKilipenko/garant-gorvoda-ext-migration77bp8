...
&НаКлиенте
Асинх Процедура ВозвратПрихода(Команда)
    ...
    РезультатПробитияЧека = ВыполнитьВозвратПриходаЧекаСчетмаш(ТекущаяСтрокаОплаты);
КонецПроцедуры
...
Функция ПробитьЧекСчетмаш(Знач ТипОперации, ТекущаяСтрокаОплаты)
    ...
    РезультатПроверкиСтатуса = ВыполнитьПроверкуСтатусаЧека(ТекущаяСтрокаОплаты);
    Если (ТипОперации <> ГП_СчетмашAPIКлиентСервер.НовыйТипыОперацииРегистрацииЧека().ВозвратПрихода)
        И РезультатПроверкиСтатуса.Успех И РезультатПроверкиСтатуса.ДанныеОтвета.Статус = "success" Тогда

        ОбщегоНазначенияКлиент.СообщитьПользователю("Чек уже пробит");

        РезультатФункции.Успех = Ложь;
        РезультатФункции.Результат = РезультатПроверкиСтатуса.ДанныеОтвета;
...
&НаКлиенте
Функция ВыполнитьВозвратПриходаЧекаСчетмаш(ТекущаяСтрокаОплаты)
...
Функция ПолучитьЗначениеСвойстваНаСервере(Знач Контейнер, Знач Ключ, Знач ЗначениеПоУмолчанию = Неопределено)
    ...
    Попытка
        ЗначениеПоУмолчанию = Контейнер[Ключ];
    Исключение
        РезультатФункции = ЗначениеПоУмолчанию;
    КонецПопытки;
