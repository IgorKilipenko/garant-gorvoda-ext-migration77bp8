...
Функция ПолучитьДанныеЗаполненияВводаНачальногоСальдо(
    ...
    Запрос.УстановитьПараметр("ВидДоговораНегативноеВоздействие",
        ГП_РаботаСДоговорами.ПолучитьДополнительныйВидДоговораПлатаЗаНегативноеВоздействие().Ссылка);

    ТаблицаСоответствийДоговоровНВ = НовыйТаблицаСоответствийДоговоровНегативногоВоздействия();
    Если ВидУслугиБП77 = "Негативное воздействие" Тогда
        ТаблицаСоответствийДоговоровНВ = ГП_РаботаСДоговорами.ПолучитьСоответствияДоговоровНегативногоВоздействия();
        Запрос.УстановитьПараметр("ТаблицаСоответствийДоговоровНВ", ТаблицаСоответствийДоговоровНВ);
    КонецЕсли;
    Запрос.УстановитьПараметр("ТаблицаСоответствийДоговоровНВ", ТаблицаСоответствийДоговоровНВ);
...
// Параметры:
//  Удалить_ДляНегативногоВоздействия - Булево - Устарела. Не используется
// Возвращаемое значение:
//  - Строка
Функция ПолучитьТекстЗапросаВводаНачальногоСальдо(Знач Удалить_ДляНегативногоВоздействия = Ложь)
    ТекстЗапроса =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |	КонтрагентыБП77.КПП КАК КонтрагентКПП,
        ...
        |	ВТ_ВзаиморасчетыБП77Подготовка.КонтрагентКПП КАК КонтрагентКПП,
        ...
        |			И (ВТ_ДоговорыБП77.ЭтоДоговорПлатыЗаНВ = (&ВидУслуги = ""Негативное воздействие""))
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |ПОМЕСТИТЬ ВТ_ДоговорыКонтрагентов_Подготовка
        |ИЗ
        |	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
        |ГДЕ
        |	ДоговорыКонтрагентов.ЭтоГруппа = ЛОЖЬ
        |	И ДоговорыКонтрагентов.ПометкаУдаления = ЛОЖЬ
        |	И ДоговорыКонтрагентов.lc_ВидДоговора В (ВЫБОР
        |				КОГДА &ВидУслуги = ""Негативное воздействие""
        |					ТОГДА &ВидДоговораНегативноеВоздействие
        |				ИНАЧЕ &ВидДоговораАбонентскийОтдел
        |			КОНЕЦ)
        ...
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ТаблицаСоответствийДоговоровНВ.Договор КАК Договор,
        |	ТаблицаСоответствийДоговоровНВ.ДоговорНегативноеВоздействие КАК ДоговорНегативноеВоздействие,
        |	ТаблицаСоответствийДоговоровНВ.Номер КАК Номер,
        |	ТаблицаСоответствийДоговоровНВ.Контрагент КАК Контрагент
        |ПОМЕСТИТЬ ВТ_ТаблицаСоответствийДоговоровНВ
        |ИЗ
        |	&ТаблицаСоответствийДоговоровНВ КАК ТаблицаСоответствийДоговоровНВ
        |ГДЕ
        |	&ВидУслуги = ""Негативное воздействие""
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |	ВЫБОР
        |		КОГДА &ВидУслуги = ""Негативное воздействие""
        |			ТОГДА ВЫРАЗИТЬ(ВТ_ТаблицаСоответствийДоговоровНВ.Договор КАК Справочник.ДоговорыКонтрагентов)
        |		ИНАЧЕ ВТ_ДоговорыКонтрагентов_Подготовка.Договор
        |	КОНЕЦ КАК Договор,
        |	ВЫРАЗИТЬ(ВТ_ТаблицаСоответствийДоговоровНВ.ДоговорНегативноеВоздействие КАК Справочник.ДоговорыКонтрагентов) КАК ДоговорНегативноеВоздействие,
        |	ВТ_ДоговорыКонтрагентов_Подготовка.Номер КАК Номер,
        |	ВТ_ДоговорыКонтрагентов_Подготовка.Контрагент КАК Контрагент,
        |	ВТ_ДоговорыКонтрагентов_Подготовка.Контрагент.ГП_ИдентификаторБП77 КАК ИдентификаторКонтрагента
        |ПОМЕСТИТЬ ВТ_ДоговорыКонтрагентов
        |ИЗ
        |	ВТ_ДоговорыКонтрагентов_Подготовка КАК ВТ_ДоговорыКонтрагентов_Подготовка
        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТаблицаСоответствийДоговоровНВ КАК ВТ_ТаблицаСоответствийДоговоровНВ
        |		ПО (ВТ_ТаблицаСоответствийДоговоровНВ.ДоговорНегативноеВоздействие = ВТ_ДоговорыКонтрагентов_Подготовка.Договор)
        |			И (ВТ_ТаблицаСоответствийДоговоровНВ.Контрагент = ВТ_ДоговорыКонтрагентов_Подготовка.Контрагент)
        |			И (&ВидУслуги = ""Негативное воздействие"")
        |ГДЕ
        |	ВЫБОР
        |			КОГДА &ВидУслуги = ""Негативное воздействие""
        |				ТОГДА НЕ ВТ_ДоговорыКонтрагентов_Подготовка.Договор ЕСТЬ NULL
        |			ИНАЧЕ ИСТИНА
        |		КОНЕЦ
        |;
        |
        ...
        |	ВТ_ВзаиморасчетыБП77.КонтрагентКПП КАК КонтрагентКПП,
        ...
        |	ВТ_ДоговорыКонтрагентов.ИдентификаторКонтрагента КАК ИдентификаторКонтрагента
        ...
        |			И (ВЫБОР
        |				КОГДА ВТ_ВзаиморасчетыБП77.КонтрагентКод <> """"
        |						И ВТ_ВзаиморасчетыБП77.КонтрагентКод = ВТ_ДоговорыКонтрагентов.ИдентификаторКонтрагента
        |					ТОГДА ИСТИНА
        |				КОГДА ВТ_ВзаиморасчетыБП77.КонтрагентИНН <> """"
        |						И ВТ_ВзаиморасчетыБП77.КонтрагентИНН = ВТ_ДоговорыКонтрагентов.Контрагент.ИНН
        |					ТОГДА ИСТИНА
        |				ИНАЧЕ ВТ_ВзаиморасчетыБП77.КонтрагентНаименование = ВТ_ДоговорыКонтрагентов.Контрагент.Наименование
        |			КОНЕЦ)
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |	ВЫБОР
        |		КОГДА &ВидУслуги = ""Водоснабжение""
        |			ТОГДА ВТ_ВзаиморасчетыПоКонтрагентамВодоснабжение.Водоснабжение
        |		КОГДА &ВидУслуги = ""Водоотведение""
        |			ТОГДА ВТ_ВзаиморасчетыПоКонтрагентамВодоснабжение.Водоотведение
        |		КОГДА &ВидУслуги = ""Негативное воздействие""
        |			ТОГДА ВТ_ВзаиморасчетыПоКонтрагентамВодоснабжение.Сальдо
...
Функция НовыйТаблицаСоответствийДоговоровНегативногоВоздействия()
    РезультатФункции = Новый ТаблицаЗначений;
    РезультатФункции.Колонки.Добавить("Договор", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
    РезультатФункции.Колонки.Добавить("ДоговорНегативноеВоздействие", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
