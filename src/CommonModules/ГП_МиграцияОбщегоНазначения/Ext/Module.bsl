...
// Устарела. Использовать `ГП_РаботаСДоговорами.ПолучитьНомерДоговораПоНаименованию`
//
...
Функция ПолучитьНомерДоговораПоНаименованию(Знач НаименованиеДоговора) Экспорт
    Возврат ГП_РаботаСДоговорами.ПолучитьНомерДоговораПоНаименованию(НаименованиеДоговора);
КонецФункции
...
Функция ЭтоНаименованиеУслугиХолодногоВодоснабженияОИ(Знач НаименованиеОбъектаАбонента) Экспорт
    ШаблонПоиска = ПолучитьШаблонСуффиксаХолодногоВодоснабженияОИ(Ложь);
...
// Возвращаемое значение:
//  - Строка
Функция УдалитьСуффиксХолодногоВодоснабженияОИ(Знач НаименованиеОбъектаАбонента) Экспорт
    ШаблонПоиска = ПолучитьШаблонСуффиксаХолодногоВодоснабженияОИ(Истина);
    Возврат СтрЗаменитьПоРегулярномуВыражению(НаименованиеОбъектаАбонента, ШаблонПоиска, "$1");
КонецФункции

// Возвращаемое значение:
//  - Строка
Функция ПолучитьШаблонСуффиксаХолодногоВодоснабженияОИ(Знач Строгий = Ложь) Экспорт
    Если Строгий = Ложь Тогда
        Возврат "(?i)^(.*)(?:п?лата)\s+(?:(?:за)|(?:по))\s+(?:х[оа]лодн.*)\s+(?:водосн.+бж[а-яА-Я]+)[\s,]+.*(?:(?:ОИ)|(?:потребленное\s+при\s+содержан.*))\s*.*";
    КонецЕсли;

    Возврат "(?i)^(.+)\s*\(+\s*плата\s+за\s+холодное\s+водоснабжение\s+ОИ[\s\)]*";
КонецФункции
...
Функция ЭтоНаименованиеУслугиНегативногоВоздействия(Знач НаименованиеОбъектаАбонента) Экспорт
    БазоваяСтрокаНВ = "негативное воздействие";

    Если СтрНайти(НаименованиеОбъектаАбонента, БазоваяСтрокаНВ, НаправлениеПоиска.СКонца) > 0 Тогда
        Возврат Истина;
    КонецЕсли;

    ШаблонПоиска = ПолучитьШаблонСуффиксаНегативногоВоздействия(Ложь);
    РезультатПоиска = СтрНайтиПоРегулярномуВыражению(НаименованиеОбъектаАбонента, ШаблонПоиска);
...
// Возвращаемое значение:
//  - Строка
Функция УдалитьСуффиксНегативногоВоздействия(Знач НаименованиеОбъектаАбонента) Экспорт
    ШаблонПоиска = ПолучитьШаблонСуффиксаНегативногоВоздействия(Истина);
    Возврат СтрЗаменитьПоРегулярномуВыражению(НаименованиеОбъектаАбонента, ШаблонПоиска, "$1");
КонецФункции
...
// Возвращаемое значение:
//  - Строка
Функция ПолучитьШаблонСуффиксаНегативногоВоздействия(Знач Строгий = Ложь) Экспорт
    Если Строгий = Ложь Тогда
        Возврат "(?i)^(.+)\s*[(\s]+(?:негативное\s+воздейст?вие(?:\s+на\s*ЦСВ)?)[\s)]*";
    КонецЕсли;
...
Функция ЭтоНаименованиеПотериВоды(Знач НаименованиеОбъектаАбонента) Экспорт
    ...
    ШаблонПоиска = ПолучитьШаблонСуффиксаПотериВоды(Ложь);
    РезультатПоиска = СтрНайтиПоРегулярномуВыражению(НаименованиеОбъектаАбонента, ШаблонПоиска);
...
// Возвращаемое значение:
//  - Строка
Функция УдалитьСуффиксПотериВоды(Знач НаименованиеОбъектаАбонента) Экспорт
    ШаблонПоиска = ПолучитьШаблонСуффиксаПотериВоды(Истина);
    Возврат СтрЗаменитьПоРегулярномуВыражению(НаименованиеОбъектаАбонента, ШаблонПоиска, "$1");
КонецФункции
...
// Возвращаемое значение:
//  - Строка
Функция ПолучитьШаблонСуффиксаПотериВоды(Знач Строгий = Ложь) Экспорт
    Если Строгий = Ложь Тогда
        Возврат "(?i)\(+\s*потери\s+воды";
    КонецЕсли;

    Возврат "(?i)^(.+)\s*\(+\s*потери\s+воды";
КонецФункции
...
Функция ПолучитьВодозаборБезВодозабора() Экспорт
    Возврат ГП_РаботаСВодозаборами.ПолучитьВодозаборБезВодозабора();
КонецФункции

#КонецОбласти // Водозаборы
