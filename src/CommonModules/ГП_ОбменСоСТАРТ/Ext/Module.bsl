...
//  ТолькоСПоказаниями - Булево - По умолчанию Ложь
...
Функция ПолучитьДанныеПриборовУчетаДляВыгрузки(Знач НачалоПериода, Знач КонецПериода, Знач Услуга, Знач ТолькоСПоказаниями = Ложь) Экспорт
    Запрос = Новый Запрос;
    ...
    Запрос.УстановитьПараметр("ТолькоСПоказаниями", ТолькоСПоказаниями);
...
Функция ПолучитьТекстЗапросаДанныхСчетчиковВыгрузки()
    ТекстЗапроса =
        ...
        |ГДЕ
        |   ВЫБОР
        |       КОГДА &ТолькоСПоказаниями = ИСТИНА ТОГДА (НЕ ВТ_РасчетПриборовУчета.Счетчик ЕСТЬ NULL)
        |       ИНАЧЕ ИСТИНА
        |   КОНЕЦ
        |	И ВЫБОР
