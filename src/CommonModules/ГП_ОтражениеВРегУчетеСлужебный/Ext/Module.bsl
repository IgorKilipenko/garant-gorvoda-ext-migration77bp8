...
Функция СформироватьСодержаниеУслугиСУчетомЛС(Знач СтруктураПараметровСтроки, Знач ДанныеПоказанийСчетчиков) Экспорт
    ...
    ДатаАктуальностиАдресаЛС = СтруктураПараметровСтроки.ДокументОбъект.Дата;
    ДатаАктуальностиАдресаЛС = ?(ЗначениеЗаполнено(ДатаАктуальностиАдресаЛС), ДатаАктуальностиАдресаЛС, Неопределено);
    НаименованиеЗданияРасшифровки = ?(ЗначениеЗаполнено(ЛицевойСчетРасшифровки),
            ГП_РаботаСЛицевымиСчетами.ПолучитьПредставлениеАдресаЛицевогоСчетаДляСодержания(
                ЛицевойСчетРасшифровки,
                Новый Структура("СУчетомАдресаДляПечати, ДатаАктуальности", Истина, ДатаАктуальностиАдресаЛС)),
            Неопределено);
...
Функция СформироватьСчетНаОплатуДокументаРеализации(Знач ДокументРеализацииОбъект) Экспорт
    ...
    СчетНаОплатуОбъект.ВариантПечатиQRКода = Перечисления.ВариантПечатиQRКода.ПлатежныеРеквизиты;
    СчетНаОплатуОбъект.ДополнительныеУсловия = Справочники.ДополнительныеУсловия.ПустаяСсылка();
    СчетНаОплатуОбъект.Записать(?(ДокументРеализацииОбъект.Проведен, РежимЗаписиДокумента.Проведение, РежимЗаписиДокумента.Запись));
...
// Параметры:
//  ДокументОтраженияОбъект - ДокументОбъект.КВП_ОтражениеНачисленийВРеглУчете - При выполнении будет модифицирована ТЧ Контрагенты
// Возвращаемое значение:
//  - Структура
//      * Успех - Булево
//      * ПомеченныеНаУдалениеДокументы - Массив из ДокументСсылка.КВП_ОтражениеНачисленийВРеглУчете - Помеченные на удаление документы
Функция ПометитьНаУдалениеВсеДокументОтраженияНачислений(ДокументОтраженияОбъект) Экспорт
    РезультатФункции = Новый Структура("Успех, ПомеченныеНаУдалениеДокументы, ТекстСообщения", Истина, Новый Массив, Новый Массив);

    ТаблицаКонтрагентов = ДокументОтраженияОбъект.Контрагенты;

    НачатьТранзакцию();
    Попытка
        Для Каждого СтрокаДанных Из ТаблицаКонтрагентов Цикл
            Если СтрокаДанных.Документ = Неопределено ИЛИ СтрокаДанных.Документ.Ссылка.Пустая() Тогда
                Продолжить; // Нет сформированного документа
            КонецЕсли;

            ДокументОбъект = СтрокаДанных.Документ.ПолучитьОбъект();
            ДокументОбъект.УстановитьПометкуУдаления(Истина);

            // Для документа реализации дополнительно удаляется счет на оплату
            Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг")
                И ЗначениеЗаполнено(ДокументОбъект.СчетНаОплатуПокупателю) Тогда

                СчетПокупателюОбъект = ДокументОбъект.СчетНаОплатуПокупателю.ПолучитьОбъект();
                СчетПокупателюОбъект.УстановитьПометкуУдаления(Истина);
            КонецЕсли;

            РезультатФункции.ПомеченныеНаУдалениеДокументы.Добавить(ДокументОбъект.Ссылка);
        КонецЦикла;
    ...
    Исключение
        ...
        РезультатФункции.ПомеченныеНаУдалениеДокументы.Очистить();
        РезультатФункции.ТекстСообщения = СтрШаблон(
                "Ошибка при очистке документов отражения.
                |Информация: %1", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
    КонецПопытки;

    // Выполняем очистку строк документа отражения
    Если РезультатФункции.Успех = Истина Тогда
        Для Каждого ДокументУдаления Из РезультатФункции.ПомеченныеНаУдалениеДокументы Цикл
            ОбщегоНазначения.СообщитьПользователю("Документ <" + Строка(ДокументУдаления) + "> помечен на удаление.");

            НайденныеСтроки = ТаблицаКонтрагентов.НайтиСтроки(Новый Структура("Документ", ДокументУдаления));
            Для Каждого СтрокаДанных Из НайденныеСтроки Цикл
                СтрокаДанных.Документ = Неопределено;
                СтрокаДанных.СчетФактура = Документы.СчетФактураВыданный.ПустаяСсылка();
            КонецЦикла;
        КонецЦикла;
    КонецЕсли;
...
Функция ПолучитьТекстЗапросаДанныхПоказанийСчетчиков()
    ...
    РезультатФункции.ТекстЗапроса =
        ...
        |ПОМЕСТИТЬ ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной
        |ИЗ
        ...
        |	И УПЖКХ_РасчетПриборовУчета.ВидПоказания = ЗНАЧЕНИЕ(Перечисление.КВП_ВидыТарифов.Ночной)
        ...
        |	УПЖКХ_РасчетПриборовУчета.ВидПоказания КАК ВидПоказания,
        |	УПЖКХ_РасчетПриборовУчета.Показание КАК Показание
        |ПОМЕСТИТЬ ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной
        |ИЗ
        |	РегистрНакопления.УПЖКХ_РасчетПриборовУчета КАК УПЖКХ_РасчетПриборовУчета
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДанныеДокументаОтражения КАК ВТ_ДанныеДокументаОтражения
        |		ПО (ИСТИНА)
        ...
        |			ИЗ
        |				ВТ_РасшифровкаНачислений КАК Т)
        |	И НЕ (УПЖКХ_РасчетПриборовУчета.Услуга, УПЖКХ_РасчетПриборовУчета.Счетчик) В
        |				(ВЫБРАТЬ
        |					Т.Услуга,
        |					Т.Счетчик
        |				ИЗ
        |					ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной КАК Т)
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.Период КАК Период,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.Регистратор КАК Регистратор,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.НомерСтроки КАК НомерСтроки,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.ВидДвижения КАК ВидДвижения,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.ЛицевойСчет КАК ЛицевойСчет,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.Счетчик КАК Счетчик,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.Услуга КАК Услуга,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.Номенклатура КАК Номенклатура,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.ВидПоказания КАК ВидПоказания,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной.Показание КАК Показание
        |ПОМЕСТИТЬ ВТ_РасчетыПриборовУчетаРасходВсеЗаписи
        |ИЗ
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной КАК ВТ_РасчетыПриборовУчетаРасходВсеЗаписиНочной
        |
        |ОБЪЕДИНИТЬ ВСЕ
        |
        |ВЫБРАТЬ
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.Период,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.Регистратор,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.НомерСтроки,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.ВидДвижения,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.ЛицевойСчет,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.Счетчик,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.Услуга,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.Номенклатура,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.ВидПоказания,
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной.Показание
        |ИЗ
        |	ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной КАК ВТ_РасчетыПриборовУчетаРасходВсеЗаписиДневной
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |ПОМЕСТИТЬ ВТ_ТекущиеПоказанияСчетчиков
        |ИЗ
        ...
        |	И КВП_ПоказанияСчетчиков.Период <= ВТ_ДанныеДокументаОтражения.КонецПериода
        ...
        |ПОМЕСТИТЬ ВТ_ПркдыдущиеПоказанияСчетчиковПодготовка
        |ИЗ
        |	РегистрСведений.КВП_ПоказанияСчетчиков КАК КВП_ПоказанияСчетчиков
        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТекущиеПоказанияСчетчиков КАК ВТ_ТекущиеПоказанияСчетчиков
        |		ПО КВП_ПоказанияСчетчиков.Период < ВТ_ТекущиеПоказанияСчетчиков.Период
        |			И КВП_ПоказанияСчетчиков.Счетчик = ВТ_ТекущиеПоказанияСчетчиков.Счетчик
        |			И (КВП_ПоказанияСчетчиков.Активность = ИСТИНА)
        |ГДЕ
        |	НЕ ВТ_ТекущиеПоказанияСчетчиков.Период ЕСТЬ NULL
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |	КВП_ПоказанияСчетчиков.КоэффициентТрансформации КАК КоэффициентТрансформации,
        |	КВП_ПоказанияСчетчиков.ТемпературныйКоэффициент КАК ТемпературныйКоэффициент
        |ПОМЕСТИТЬ ВТ_ПркдыдущиеПоказанияСчетчиков
        |ИЗ
        |	ВТ_ПркдыдущиеПоказанияСчетчиковПодготовка КАК КВП_ПоказанияСчетчиков
        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПркдыдущиеПоказанияСчетчиковПодготовка КАК Т
        |		ПО КВП_ПоказанияСчетчиков.Счетчик = Т.Счетчик
        |			И КВП_ПоказанияСчетчиков.АвтоНомер <> Т.АвтоНомер
        |			И КВП_ПоказанияСчетчиков.Период < Т.Период
        |ГДЕ
        |	Т.Период ЕСТЬ NULL
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_ТекущиеПоказанияСчетчиков.Период КАК Период,
        |	ВТ_ПркдыдущиеПоказанияСчетчиков.Период КАК ПериодПредыдущегоПоказания,
        |	ВТ_ТекущиеПоказанияСчетчиков.Регистратор КАК Регистратор,
        |	ВТ_ТекущиеПоказанияСчетчиков.НомерСтроки КАК НомерСтроки,
        |	ВТ_ТекущиеПоказанияСчетчиков.Счетчик КАК Счетчик,
        |	ВТ_ТекущиеПоказанияСчетчиков.ДневноеПоказание КАК ДневноеПоказание,
        |	ВТ_ПркдыдущиеПоказанияСчетчиков.ДневноеПоказание КАК ПредыдущееДневноеПоказание,
        |	ВТ_ТекущиеПоказанияСчетчиков.НочноеПоказание КАК НочноеПоказание,
        |	ВТ_ПркдыдущиеПоказанияСчетчиков.НочноеПоказание КАК ПредыдущееНочноеПоказание,
        |	ВТ_ТекущиеПоказанияСчетчиков.ПиковоеПоказание КАК ПиковоеПоказание,
        |	ВТ_ПркдыдущиеПоказанияСчетчиков.ПиковоеПоказание КАК ПредыдущееПиковоеПоказание,
        |	ВТ_ТекущиеПоказанияСчетчиков.НачальноеПоказание КАК ЭтоНачальноеПоказание,
        |	ВТ_ТекущиеПоказанияСчетчиков.КоэффициентТрансформации КАК КоэффициентТрансформации,
        |	ВТ_ТекущиеПоказанияСчетчиков.ТемпературныйКоэффициент КАК ТемпературныйКоэффициент,
        |	ВТ_ТекущиеПоказанияСчетчиков.НачальноеПоказание КАК ЭтоПредыдущееНачальноеПоказание
        |ПОМЕСТИТЬ ВТ_ПоказанияСчетчиковВсеЗаписи
        |ИЗ
        |	ВТ_ТекущиеПоказанияСчетчиков КАК ВТ_ТекущиеПоказанияСчетчиков
        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПркдыдущиеПоказанияСчетчиков КАК ВТ_ПркдыдущиеПоказанияСчетчиков
        |		ПО ВТ_ТекущиеПоказанияСчетчиков.Счетчик = ВТ_ПркдыдущиеПоказанияСчетчиков.Счетчик
        |			И ВТ_ТекущиеПоказанияСчетчиков.Период > ВТ_ПркдыдущиеПоказанияСчетчиков.Период
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_ПоказанияСчетчиковВсеЗаписи.Период КАК ПериодТекущегоПоказания,
        |	ВТ_ПоказанияСчетчиковВсеЗаписи.ПериодПредыдущегоПоказания КАК ПериодПредыдущегоПоказания,
        ...
        |	ВТ_ПоказанияСчетчиковВсеЗаписи.ПредыдущееДневноеПоказание КАК ПредыдущееДневноеПоказание,
        |	ВТ_ПоказанияСчетчиковВсеЗаписи.НочноеПоказание КАК НочноеПоказание,
        |	ВТ_ПоказанияСчетчиковВсеЗаписи.ПредыдущееНочноеПоказание КАК ПредыдущееНочноеПоказание,
        |	ВТ_ПоказанияСчетчиковВсеЗаписи.ПиковоеПоказание КАК ПиковоеПоказание,
        |	ВТ_ПоказанияСчетчиковВсеЗаписи.ПредыдущееПиковоеПоказание КАК ПредыдущееПиковоеПоказание,
        |	ВТ_ПоказанияСчетчиковВсеЗаписи.ЭтоНачальноеПоказание КАК ЭтоНачальноеПоказание,
        ...
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_ПоказанияСчетчиковПодготовка.ПериодТекущегоПоказания КАК ПериодТекущегоПоказания,
        |	ВТ_ПоказанияСчетчиковПодготовка.ПериодПредыдущегоПоказания КАК ПериодПредыдущегоПоказания,
        |	ВТ_ПоказанияСчетчиковПодготовка.Регистратор КАК Регистратор,
        |	ВТ_ПоказанияСчетчиковПодготовка.Счетчик КАК Счетчик,
        |	ВЫБОР
        |		КОГДА ВТ_ПоказанияСчетчиковПодготовка.ВидПоказания = ЗНАЧЕНИЕ(Перечисление.КВП_ВидыТарифов.Ночной)
        |			ТОГДА ВТ_ПоказанияСчетчиковПодготовка.НочноеПоказание
        |		ИНАЧЕ ВТ_ПоказанияСчетчиковПодготовка.ДневноеПоказание
        |	КОНЕЦ КАК ТекущееПоказание,
        ...
        |		КОГДА ВТ_ПоказанияСчетчиковПодготовка.ВидПоказания = ЗНАЧЕНИЕ(Перечисление.КВП_ВидыТарифов.Ночной)
        |           ТОГДА ВЫБОР
        |               КОГДА ВТ_ПоказанияСчетчиковПодготовка.ПредыдущееНочноеПоказание = 0
        |                       И ВТ_ПоказанияСчетчиковПодготовка.Расход = ВТ_ПоказанияСчетчиковПодготовка.НочноеПоказание
        |                   ТОГДА ВТ_ПоказанияСчетчиковПодготовка.ПредыдущееДневноеПоказание
        |               ИНАЧЕ
        |                   ВТ_ПоказанияСчетчиковПодготовка.ПредыдущееНочноеПоказание
        |           КОНЕЦ
        |		ИНАЧЕ ВТ_ПоказанияСчетчиковПодготовка.ПредыдущееДневноеПоказание
        |	КОНЕЦ КАК ПредыдущееПоказание
        ...
        |			И ВТ_ПоказанияСчетчиковПодготовка.АвтоНомер <> Т.АвтоНомер
        |			И ВТ_ПоказанияСчетчиковПодготовка.ПериодТекущегоПоказания < Т.ПериодТекущегоПоказания
        |ГДЕ
        |	Т.ПериодТекущегоПоказания ЕСТЬ NULL
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_РезультатПодготовка.ПериодТекущегоПоказания КАК ПериодТекущегоПоказания,
        |	ВТ_РезультатПодготовка.ПериодПредыдущегоПоказания КАК ПериодПредыдущегоПоказания,
