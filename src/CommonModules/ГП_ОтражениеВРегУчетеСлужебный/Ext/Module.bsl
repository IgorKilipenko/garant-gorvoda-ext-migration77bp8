...
Функция СформироватьСодержаниеУслугиСУчетомЛС(Знач СтруктураПараметровСтроки, Знач ДанныеПоказанийСчетчиков) Экспорт
    ...
    Если ЗначениеЗаполнено(СтруктураПараметровСтроки.СтрокаРасшифровки.lc_ЛицевойСчет) Тогда
        СтруктураОтбора = Новый Структура;
        СтруктураОтбора.Вставить("ЛицевойСчет", СтруктураПараметровСтроки.СтрокаРасшифровки.lc_ЛицевойСчет);
...
Функция СформироватьПечатнуюФормуСчетаНаОплату(Знач МассивОбъектов) Экспорт
    ПечатьТорговыхДокументов.ПечатьСчетаНаОплату(ТаблицаСведенийСчетНаОплату, ОбъектыПечатиВрем, ПараметрыПечати)
...
Функция ПолучитьТекстЗапросаДанныхПоказанийСчетчиков()
    ...
    РезультатФункции.ТекстЗапроса =
        ...
        |	И УПЖКХ_РасчетПриборовУчета.Период >= ВТ_ДанныеДокументаОтражения.НачалоПериода
