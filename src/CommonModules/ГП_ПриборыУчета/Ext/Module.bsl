...
Функция СоздатьСчетчикОИ(Знач ДанныеЗаполнения, Знач ДопЧислоДляГенерацииИдентификаторов = Неопределено) Экспорт
    НовыйСчетчикОИ = Справочники.КВП_Счетчики.СоздатьЭлемент();
    ...
    НовыйСчетчикОИ.Идентификатор =
        УПЖКХ_ОбщегоНазначенияСервер.ПолучитьИдентификаторСчетчика( , НовыйСчетчикОИ.Код, ДопЧислоДляГенерацииИдентификаторов, Истина);

    НовыйСчетчикОИ.ВидУслуги = ДанныеЗаполнения.Услуга;

    НовыйСчетчикОИ.Разрядность = 6; // По ТЗ
    НовыйСчетчикОИ.Коэффициент = 1; // По ТЗ
    НовыйСчетчикОИ.ИспользоватьТемпературныйКоэффициент = Ложь;

    // Тарифность (по умолчанию 1-тарифный)
    НовыйСчетчикОИ.Тарифность = Перечисления.КВП_ТарифностьСчетчиков.Однотарифный; // По ТЗ
    НовыйСчетчикОИ.СпособРегистрацииПоказаний = Перечисления.КВП_СпособыРегистрацииПоказанийСчетчиков.РасходЗаПериодРасчета;

    НовыйСчетчикОИ.Наименование = СтрШаблон("[[Плата за ОИ]] %1", ДанныеЗаполнения.Наименование);
    ...
    КонецЕсли;
    НовыйСчетчикОИ.ГП_Комментарий = СтрокаКомментария;

    НовыйСчетчикОИ.ГП_ЭтоПлатаЗаВодоснабжениеОИ = Истина;
    НовыйСчетчикОИ.ГП_ТолькоДляКанализации = ДанныеЗаполнения.ТолькоДляКанализации;
    НовыйСчетчикОИ.ГП_ЭтоВиртуальныйСчетчик = Истина;
    НовыйСчетчикОИ.ГП_КоличествоПоСреднему = ДанныеЗаполнения.КоличествоПоСреднему;
