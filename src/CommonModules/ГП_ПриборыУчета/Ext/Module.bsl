...
Функция ПолучитьПриборыУчетаДоговораПоЛицевомуСчету(
    ...
    Запрос.Текст =
        ...
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ИсторияСостоянийПриборовУчета.Период КАК ПериодСчетчика,
        |	НазначенныеНачисления.Организация КАК Организация,
        |	ИсторияСостоянийПриборовУчета.Объект КАК Объект,
        |	ИсторияСостоянийПриборовУчета.ПриборУчета КАК ПриборУчета,
        |	ИсторияСостоянийПриборовУчета.Действует КАК СчетчикДействует,
        |	ИсторияСостоянийПриборовУчета.ДатаНачала КАК ДатаНачала,
        |	ИсторияСостоянийПриборовУчета.ДатаПоверки КАК ДатаПоверки,
        |	ИсторияСостоянийПриборовУчета.НомерПломбы КАК НомерПломбы,
        |	ИсторияСостоянийПриборовУчета.ПричинаОтключения КАК ПричинаОтключения,
        ...
        |ИЗ
        |	ВТ_ИсторияСостоянийПриборовУчета КАК ИсторияСостоянийПриборовУчета
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КВП_НазначенныеНачисления.СрезПоследних КАК НазначенныеНачисления
        |		ПО ИсторияСостоянийПриборовУчета.Объект = НазначенныеНачисления.Объект
        |			И ИсторияСостоянийПриборовУчета.ПриборУчета.ВидУслуги = НазначенныеНачисления.Услуга
        |           И (НазначенныеНачисления.Действует = ИСТИНА)
        |			И &ДополнительноеУсловиеДействияСчетчика
        |";

    Запрос.УстановитьПараметр("ЛицевойСчет", ЛицевойСчет);

    ДополнительноеУсловиеДействияСчетчика = "ИСТИНА";
    Если ТолькоДействующиеСчетчики = Истина Тогда
        ДополнительноеУсловиеДействияСчетчика = СтрШаблон(
                "%1 И (%2)", ДополнительноеУсловиеДействияСчетчика,
                "
                |   ВЫБОР
                |       КОГДА ИсторияСостоянийПриборовУчета.Действует = ИСТИНА ТОГДА ИСТИНА
                |       ИНАЧЕ
                |           ИсторияСостоянийПриборовУчета.ПричинаОтключения = ЗНАЧЕНИЕ(Перечисление.УПЖКХ_ПричиныОтключенияПриборовУчета.Поверка)
                |   КОНЕЦ
                |");
    КонецЕсли;

    Запрос.Текст = СтрЗаменитьПоРегулярномуВыражению(Запрос.Текст,
            "^((?:\s*И\s+)?)&ДополнительноеУсловиеДействияСчетчика(?:\s+|$)",
            СтрШаблон("$1 %1", ДополнительноеУсловиеДействияСчетчика), Истина, Истина);

    Если ТолькоДействующиеУслуги = Ложь Тогда
        Запрос.Текст = СтрЗаменитьПоРегулярномуВыражению(Запрос.Текст, "И\s+\(?НазначенныеНачисления.Действует\s*=\s*ИСТИНА\)?", "");
    КонецЕсли;
...
//      * ТолькоБезПометкиУдаления - Булево - По умолчанию Истина
//      * ТолькоЗаполненныеНомера - Булево - По умолчанию Ложь
// Возвращаемое значение:
//  * ТаблицаЗначений
//      * ЗаводскойНомер - Строка
//      * Счетчик - СправочникСсылка.Счетчик
//      * ГП_ИдентификаторБП77 - Строка
Функция ПолучитьТаблицуЗаводскихНомеровСчетчиков(Знач Параметры = Неопределено) Экспорт
    ТолькоБезПометкиУдаления = ?(Параметры = Неопределено ИЛИ Параметры.Свойство("ТолькоБезПометкиУдаления") = Ложь,
            Истина, Параметры.ТолькоБезПометкиУдаления);
    ТолькоЗаполненныеНомера = ?(Параметры = Неопределено ИЛИ Параметры.Свойство("ТолькоЗаполненныеНомера") = Ложь,
            Ложь, Параметры.ТолькоЗаполненныеНомера);
    ...
    Если ТолькоБезПометкиУдаления = Истина Тогда
        ДополнительныеУсловия = СтрШаблон(
                "%1
                |%2", ДополнительныеУсловия, "И Счетчики.ПометкаУдаления = ЛОЖЬ");
    КонецЕсли;
    Если ТолькоЗаполненныеНомера = Истина Тогда
        ДополнительныеУсловия = СтрШаблон(
                "%1
                |%2", ДополнительныеУсловия, "И Счетчики.ЗаводскойНомер <> """"");
    КонецЕсли;
