// Гарант+ Килипенко 01.11.2024 [F00230427] Создание регистрации оплаты на основании поступления ++
#Область ПрограммныйИнтерфейс

// Получает л/с контрагента из РС.УПЖКХ_СведенияДляВзаиморасчетовПоЛС
// Параметры:
//  КонтрагентСсылка - СправочникСсылка.Контрагенты
//  ОрганизацияСсылка - СправочникСсылка.Организации, Неопределено - Если Неопределено, устанавливается в
//      значение по умолчанию функцией `УПЖКХ_ТиповыеМетодыВызовСервера.ПолучитьОсновнуюОрганизацию`
//  ДатаАктуальности - Дата, Неопределено
//  ТолькоДействующие - Булево
//  СписокДоговоров - Массив, СписокЗначений, Неопределено из СправочникСсылка.ДоговорыКонтрагентов
// Возвращаемое значение:
//  - Массив из СправочникСсылка.КВП_ЛицевыеСчета
Функция ПолучитьЛицевыеСчетаКонтрагента(
        Знач КонтрагентСсылка, Знач ОрганизацияСсылка = Неопределено,
        Знач ДатаАктуальности = Неопределено, Знач ТолькоДействующие = Истина, Знач СписокДоговоров = Неопределено) Экспорт

    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
        |   СведенияДляВзаиморасчетовПоЛС.ЛицевойСчет КАК ЛицевойСчет
        |ПОМЕСТИТЬ ВТ_СписокЛицевыхСчетов
        |ИЗ
        |   РегистрСведений.УПЖКХ_СведенияДляВзаиморасчетовПоЛС.СрезПоследних(&ДатаАктуальности, Контрагент = &Контрагент И Организация = &Организация) КАК СведенияДляВзаиморасчетовПоЛС
        |";

    Если ТолькоДействующие = Истина Тогда
        Запрос.Текст = Запрос.Текст +
            ";
            |////////////////////////////////////////////////////////////////////////////////
            |ВЫБРАТЬ РАЗРЕШЕННЫЕ
            |	ЛицевыеСчета.ЛицевойСчет КАК ЛицевойСчет
            |ИЗ
            |	РегистрСведений.КВП_ЛицевыеСчета.СрезПоследних(&ДатаАктуальности,
            |       ЛицевойСчет В (ВЫБРАТЬ Т.ЛицевойСчет ИЗ ВТ_СписокЛицевыхСчетов КАК Т)) КАК ЛицевыеСчета
            |ГДЕ
            |	ЛицевыеСчета.Действует
            |";
    Иначе
        Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОМЕСТИТЬ ВТ_СписокЛицевыхСчетов", "");
    КонецЕсли;

    Если ДатаАктуальности <> Неопределено Тогда
        Запрос.УстановитьПараметр("ДатаАктуальности", ДатаАктуальности);
    Иначе
        Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДатаАктуальности", "");
    КонецЕсли;

    ОрганизацияСсылка = ?(ОрганизацияСсылка = Неопределено,
            УПЖКХ_ТиповыеМетодыВызовСервера.ПолучитьОсновнуюОрганизацию(), ОрганизацияСсылка);
    Запрос.УстановитьПараметр("Организация", ОрганизацияСсылка);

    Запрос.УстановитьПараметр("Контрагент", КонтрагентСсылка);

    РезультатЗапроса = Запрос.Выполнить();
    РезультатФункции = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("ЛицевойСчет");

    // Отбор лицевых счетов по списку договоров
    Если СписокДоговоров <> Неопределено И РезультатФункции.Количество() > 0 Тогда
        РезультатФункции = ПолучитьОтфильтрованныйСписокЛицевыхСчетовПоДоговорам(
                РезультатФункции, СписокДоговоров, ДатаАктуальности, ОрганизацияСсылка);
    КонецЕсли;

    Возврат РезультатФункции;
КонецФункции

// Гарант+ Килипенко 21.03.2025 [F00236141] Приоритетное погашение долга закрытых л/с ++
//
// Параметры:
//  СписокЛицевыхСчетов - СписокЗначений, Массив из СправочникСсылка.КВП_ЛицевыеСчета
//  ДатаАктуальности - Дата, Неопределено
// Возвращаемое значение:
//  - Массив из СправочникСсылка.КВП_ЛицевыеСчета
Функция ПолучитьЗакрытыеЛицевыеСчетаИзСписка(Знач СписокЛицевыхСчетов, Знач ДатаАктуальности = Неопределено) Экспорт
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   ЛицевыеСчета.ЛицевойСчет КАК ЛицевойСчет
        |ИЗ
        |	РегистрСведений.КВП_ЛицевыеСчета.СрезПоследних(&ДатаАктуальности,
        |       ЛицевойСчет В (&СписокЛицевыхСчетов)) КАК ЛицевыеСчета
        |ГДЕ
        |   ЛицевыеСчета.Действует = ЛОЖЬ
        |";

    Если ДатаАктуальности = Неопределено Тогда
        Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДатаАктуальности", "");
    Иначе
        Запрос.УстановитьПараметр("ДатаАктуальности", ДатаАктуальности);
    КонецЕсли;

    Запрос.УстановитьПараметр("СписокЛицевыхСчетов", СписокЛицевыхСчетов);

    РезультатЗапроса = Запрос.Выполнить();

    Возврат РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("ЛицевойСчет");
КонецФункции // Гарант+ Килипенко 21.03.2025 [F00236141] Приоритетное погашение долга закрытых л/с ++

// Параметры:
//  ЛицевойСчет - СправочникСсылка.КВП_ЛицевыеСчета, СправочникОбъект.КВП_ЛицевыеСчета
// Возвращаемое значение:
//  - СправочникСсылка.КВП_Здания
Функция ПолучитьЗданиеЛицевогоСчета(Знач ЛицевойСчет) Экспорт
    ПомещениеЛС = Неопределено;

    Если ТипЗнч(ЛицевойСчет) = Тип("СправочникСсылка.КВП_ЛицевыеСчета") Тогда
        ПомещениеЛС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЛицевойСчет, "Адрес");
    Иначе
        ПомещениеЛС = ЛицевойСчет.Адрес;
    КонецЕсли;

    Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПомещениеЛС, "Владелец");
КонецФункции

// Получает наименование здания
// Параметры:
//  ЛицевойСчет - СправочникСсылка.КВП_ЛицевыеСчета, СправочникОбъект.КВП_ЛицевыеСчета
// Возвращаемое значение:
//  - Строка
Функция ПолучитьНаименованиеЗданияЛицевогоСчета(Знач ЛицевойСчет) Экспорт
    РезультатФункции = "";
    ЗданиеЛС = ПолучитьЗданиеЛицевогоСчета(ЛицевойСчет);
    РезультатФункции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗданиеЛС, "Наименование");

    Возврат РезультатФункции;
КонецФункции

// Гарант+ Килипенко 02.04.2025 [F00236140] Адреса л/с для печати ++ {
//
// Получает представление адреса здания для счетов (строка содержания)
// Параметры:
//  ЛицевойСчет - СправочникСсылка.КВП_ЛицевыеСчета, СправочникОбъект.КВП_ЛицевыеСчета
//  Параметры - Неопределено, Структура
//      * СУчетомАдресаДляПечати - Булево - Если истина представление адреса приоритетно будет браться из РС ГП_АдресаЛицевыхСчетовДляПечати
//      * ДатаАктуальности - Дата, Неопределено
// Возвращаемое значение:
//  - Строка
Функция ПолучитьПредставлениеАдресаЛицевогоСчетаДляСодержания(Знач ЛицевойСчет, Знач Параметры = Неопределено) Экспорт
    РезультатФункции = "";

    СУчетомАдресаДляПечати = Ложь;
    ДатаАктуальности = Неопределено;
    Если Параметры = Неопределено Тогда
        Параметры = Новый Структура("СУчетомАдресаДляПечати", СУчетомАдресаДляПечати);
    КонецЕсли;

    СУчетомАдресаДляПечати = ?(Параметры.Свойство("СУчетомАдресаДляПечати"),
            Параметры.СУчетомАдресаДляПечати, СУчетомАдресаДляПечати);
    ДатаАктуальности = ?(Параметры.Свойство("ДатаАктуальности"),
            Параметры.ДатаАктуальности, ДатаАктуальности);

    ПредставлениеАдреса = Неопределено;
    Если СУчетомАдресаДляПечати = Истина Тогда
        // Попытка получить представление адрес л/с из РС ГП_АдресаЛицевыхСчетовДляПечати
        ПредставлениеАдреса = ПолучитьПредставлениеАдресаПечатиЛицевогоСчета(ЛицевойСчет, ДатаАктуальности);
    КонецЕсли;

    Если ПредставлениеАдреса = Неопределено Тогда
        // Нет данных представления адрес л/с - получаем из наименования здания л/с
        ЗданиеЛС = ПолучитьЗданиеЛицевогоСчета(ЛицевойСчет);
        ПредставлениеАдреса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗданиеЛС, "Наименование");
    КонецЕсли;

    РезультатФункции = Строка(ПредставлениеАдреса);

    Возврат РезультатФункции;
КонецФункции // Гарант+ Килипенко 02.04.2025 [F00236140] Адреса л/с для печати -- ]

// Гарант+ Килипенко 02.04.2025 [F00236140] Адреса л/с для печати ++ {
//
// Параметры:
//  СписокЛицевыхСчетов - Массив, СписокЗначений из СправочникСсылка.КВП_ЛицевыеСчета
//  ДатаАктуальности - Дата, Неопределено
// Возвращаемое значение:
//  - ТаблицаЗначений
//      * ЛицевойСчет - СправочникСсылка.КВП_ЛицевыеСчета
//      * ПредставлениеАдреса - Строка
Функция ПолучитьПредставленияАдресовПечатиСпискаЛицевыхСчетов(Знач СписокЛицевыхСчетов, Знач ДатаАктуальности = Неопределено) Экспорт
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   АдресаЛицевыхСчетовДляПечати.ЛицевойСчет КАК ЛицевойСчет,
        |   АдресаЛицевыхСчетовДляПечати.ПредставлениеАдреса КАК ПредставлениеАдреса
        |ИЗ
        |   РегистрСведений.ГП_АдресаЛицевыхСчетовДляПечати.СрезПоследних(&ДатаАктуальности, ЛицевойСчет В (&СписокЛицевыхСчетов)) КАК АдресаЛицевыхСчетовДляПечати
        |";

    Запрос.УстановитьПараметр("СписокЛицевыхСчетов", СписокЛицевыхСчетов);

    Если ДатаАктуальности = Неопределено Тогда
        Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДатаАктуальности", "");
    Иначе
        Запрос.УстановитьПараметр("ДатаАктуальности", ДатаАктуальности);
    КонецЕсли;

    РезультатЗапроса = Запрос.Выполнить();
    РезультатФункции = РезультатЗапроса.Выгрузить();
    Возврат РезультатФункции;
КонецФункции // Гарант+ Килипенко 02.04.2025 [F00236140] Адреса л/с для печати -- }

// Гарант+ Килипенко 02.04.2025 [F00236140] Адреса л/с для печати ++ {
//
// Параметры:
//  ЛицевойСчет - СправочникСсылка.КВП_ЛицевыеСчета
//  ДатаАктуальности - Дата, Неопределено
// Возвращаемое значение:
//  - Строка, Неопределено
Функция ПолучитьПредставлениеАдресаПечатиЛицевогоСчета(Знач ЛицевойСчет, Знач ДатаАктуальности = Неопределено) Экспорт
    РезультатФункции = Неопределено;

    ТаблицаДанныхАдресов = ПолучитьПредставленияАдресовПечатиСпискаЛицевыхСчетов(
            ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЛицевойСчет),
            ДатаАктуальности);

    Если ТаблицаДанныхАдресов.Количество() = 0 Тогда
        Возврат РезультатФункции;
    КонецЕсли;

    #Область Диагностика
    ГП_ДиагностикаКлиентСервер.Утверждение(ТаблицаДанныхАдресов.Количество() = 1,
            "ВНИМАНИЕ! Количество строк представления адреса л/с не соответствует ожидаемому значению");
    #КонецОбласти // Диагностика

    РезультатФункции = ТаблицаДанныхАдресов[0].ПредставлениеАдреса;
    Возврат РезультатФункции;
КонецФункции // Гарант+ Килипенко 02.04.2025 [F00236140] Адреса л/с для печати -- )

// Параметры:
//  НаименованиеЛС - Строка
// Возвращаемое значение:
//  - Структура
//      * Успех - Булево
//      * Значение - Строка, Неопределено
Функция УдалитьКонтрагентаИзСтрокиНаименованияЛицевогоСчетаПоШаблону(Знач НаименованиеЛС) Экспорт
    РезультатФункции = Новый Структура("Успех, Значение", Ложь);

    ШаблонПоиска = "^[^/]+\s+/\s+(.*)$";
    РезультатЗамены = СтрЗаменитьПоРегулярномуВыражению(НаименованиеЛС, ШаблонПоиска, "$1");

    РезультатФункции.Успех = НаименованиеЛС <> РезультатЗамены;
    Если РезультатФункции.Успех = Истина Тогда
        РезультатФункции.Значение = РезультатЗамены;
    КонецЕсли;

    Возврат РезультатФункции;
КонецФункции

// Гарант+ Килипенко 18.12.2024 [F00232368] Закрытие неактуальных л/с ++
//
// Параметры:
//  ЛицевойСчет - СправочникСсылка.КВП_ЛицевыеСчета
//  ДатаДокумента - Дата, Неопределено
// Возвращаемое значение:
//  - ДокументОбъект.КВП_ЗакрытиеЛицевогоСчета
Функция НовыйДокументЗакрытияЛицевогоСчета(Знач ЛицевойСчет, Знач ДатаДокумента = Неопределено) Экспорт
    НовыйДокументОбъект = Документы.КВП_ЗакрытиеЛицевогоСчета.СоздатьДокумент();

    НовыйДокументОбъект.ОбнулитьНачисления = Ложь;
    НовыйДокументОбъект.АннулироватьДанныеОСобственности = Ложь;
    НовыйДокументОбъект.Ответственный = Пользователи.ТекущийПользователь();
    Если ДатаДокумента <> Неопределено Тогда
        НовыйДокументОбъект.Дата = ДатаДокумента;
    КонецЕсли;

    НовыйДокументОбъект.ЛицевойСчет = ЛицевойСчет;

    Возврат НовыйДокументОбъект;
КонецФункции // Гарант+ Килипенко 18.12.2024 [F00232368] Закрытие неактуальных л/с --

// Возвращает таблицу кодов МКД л/с
// Возвращаемое значение:
//  - ТаблицаЗначений
//      * ЛицевойСчет - СправочникСсылка.КВП_ЛицевыеСчета
//      * Наименование - Строка
Функция ПолучитьТаблицуВсехКодовМКД() Экспорт
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   ЛицевыеСчета.Ссылка КАК ЛицевойСчет,
        |   ЛицевыеСчета.Наименование КАК Наименование,
        |   ЛицевыеСчета.lc_КодМКД КАК lc_КодМКД,
        |   ЛицевыеСчета.Идентификатор КАК Идентификатор,
        |   ЛицевыеСчета.ГП_ИдентификаторБП77 КАК ГП_ИдентификаторБП77,
        |   ЛицевыеСчета.ОтветственныйСобственникПредставление КАК ОтветственныйСобственникПредставление
        |ИЗ
        |   Справочник.КВП_ЛицевыеСчета КАК ЛицевыеСчета
        |ГДЕ
        |   ЛицевыеСчета.ЭтоГруппа = Ложь
        |";

    РезультатЗапроса = Запрос.Выполнить();
    РезультатФункции = РезультатЗапроса.Выгрузить();
    Возврат РезультатФункции;
КонецФункции

// Параметры:
//  ИсходныйСписокЛицевыхСчетов - Массив, СписокЗначений из СправочникСсылка.КВП_ЛицевыеСчета
//  СписокДоговоров - Массив, СписокЗначений из СправочникСсылка.ДоговорыКонтрагентов
//  ДатаАктуальностиСведений - Дата, Неопределено
//  Организация - СправочникСсылка.Организации, Неопределено
// Возвращаемое значение:
//  - Массив из СправочникСсылка.КВП_ЛицевыеСчета
Функция ПолучитьОтфильтрованныйСписокЛицевыхСчетовПоДоговорам(
        Знач ИсходныйСписокЛицевыхСчетов, Знач СписокДоговоров, Знач ДатаАктуальностиСведений = Неопределено, Знач Организация = Неопределено) Экспорт

    Организация = ?(Организация = Неопределено, УПЖКХ_ТиповыеМетодыВызовСервера.ПолучитьОсновнуюОрганизацию(), Организация);

    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ
        |   МАКСИМУМ(СведенияДляВзаиморасчетовПоЛС.Период) КАК Период,
        |   СведенияДляВзаиморасчетовПоЛС.ЛицевойСчет КАК ЛицевойСчет
        |ПОМЕСТИТЬ ВТ_СведенияДляВзаиморасчетовПоЛС_Подготовка
        |ИЗ
        |   РегистрСведений.УПЖКХ_СведенияДляВзаиморасчетовПоЛС.СрезПоследних(
        |           &ДатаАктуальностиСведений,
        |           Организация = &Организация
        |           И ЛицевойСчет В (&ИсходныйСписокЛицевыхСчетов)
        |       ) КАК СведенияДляВзаиморасчетовПоЛС
        |ГДЕ
        |   СведенияДляВзаиморасчетовПоЛС.Договор В (&СписокДоговоров)
        |СГРУППИРОВАТЬ ПО
        |   СведенияДляВзаиморасчетовПоЛС.ЛицевойСчет
        |;
        |
        |ВЫБРАТЬ
        |   СведенияДляВзаиморасчетовПоЛС.Период КАК Период,
        |   СведенияДляВзаиморасчетовПоЛС.ЛицевойСчет КАК ЛицевойСчет,
        |   СведенияДляВзаиморасчетовПоЛС.Договор КАК Договор
        |ИЗ
        |   РегистрСведений.УПЖКХ_СведенияДляВзаиморасчетовПоЛС КАК СведенияДляВзаиморасчетовПоЛС
        |ГДЕ
        |   ИСТИНА
        |   И СведенияДляВзаиморасчетовПоЛС.Активность = ИСТИНА
        |   И (Период, ЛицевойСчет) В (ВЫБРАТЬ Т.Период, Т.ЛицевойСчет ИЗ ВТ_СведенияДляВзаиморасчетовПоЛС_Подготовка КАК Т)
        |";

    Если ДатаАктуальностиСведений = Неопределено Тогда
        Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДатаАктуальностиСведений", "");
    Иначе
        Запрос.УстановитьПараметр("ДатаАктуальностиСведений", ДатаАктуальностиСведений);
    КонецЕсли;

    Запрос.УстановитьПараметр("Организация", Организация);
    Запрос.УстановитьПараметр("ИсходныйСписокЛицевыхСчетов", ИсходныйСписокЛицевыхСчетов);
    Запрос.УстановитьПараметр("СписокДоговоров", СписокДоговоров);

    РезультатЗапроса = Запрос.Выполнить();

    ТаблицаЛС = РезультатЗапроса.Выгрузить();
    РезультатФункции = ТаблицаЛС.ВыгрузитьКолонку("ЛицевойСчет");
    Возврат РезультатФункции;
КонецФункции

#КонецОбласти // ПрограммныйИнтерфейс
// Гарант+ Килипенко 01.11.2024 [F00230427] Создание регистрации оплаты на основании поступления --
