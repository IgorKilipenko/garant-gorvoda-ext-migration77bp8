...
Функция ПолучитьВсеЗданияБП77(Знач Параметры = Неопределено) Экспорт
    ...
    Если Параметры.Свойство("СписокПолей") = Ложь Тогда
        СписокПолейЗданияБП77 = ПолучитьВсеПоляЗданийБП77();
    Иначе
    ...
    Для Каждого ИмяПоляЗдания Из СписокПолейЗданияБП77 Цикл
        ...
        СтрокаСпискаПолей = СтрШаблон(
                "%1,
                |%2", СтрокаСпискаПолей, ИмяПоляЗдания.Значение);
    КонецЦикла;
    ...
    Если Параметры.Свойство("Отбор") = Истина Тогда
        Если Параметры.Отбор.Свойство("ЭтоНегативноеВоздействиеЦСВ") = Истина Тогда
            ДополнительныеУсловия = СтрШаблон(
                    "%1
                    |   И ЭтоНегативноеВоздействиеЦСВ = %2",
                    ДополнительныеУсловия, ?(Параметры.Отбор.ЭтоНегативноеВоздействиеЦСВ = Истина, "ИСТИНА", "ЛОЖЬ"));
        КонецЕсли;
...
// Возвращаемое значение:
//  - СписокЗначений
Функция ПолучитьВсеПоляЗданийБП77() Экспорт
