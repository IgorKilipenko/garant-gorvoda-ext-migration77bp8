...
Функция ГП_ПечатьСчетаНаОплату(СведенияСчетаНаОплату, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СведенияОДокументе Из СведенияСчетаНаОплату Цикл
		...
		ПечатьQRВозможна =
			СведенияОПокупателе.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо
				И Не СчетВВалюте И Не ЦифровойСчет;

		Если Не ПечатьQRВозможна Тогда
			ДвоичныеДанныеQRКода = Неопределено;
		ИначеЕсли СведенияОДокументе.ВариантПечатиQRКода = Перечисления.ВариантПечатиQRКода.НеПечатать Тогда
			ДвоичныеДанныеQRКода = Неопределено;
		...
		ИначеЕсли СведенияОДокументе.ВариантПечатиQRКода = Перечисления.ВариантПечатиQRКода.СБП Тогда
			...
			Если ТипЗнч(СведенияОДокументе.БанковскийСчетПродавца) = Тип("СправочникСсылка.БанковскиеСчета")
				...
				Если ИнтеграцияСПлатежнымиСистемами.ИнтеграцияДоступна() И ЗначениеЗаполнено(СведенияОДокументе.НастройкаИнтеграцииСБП) Тогда
