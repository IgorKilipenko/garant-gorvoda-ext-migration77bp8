...
Функция ГП_ПечатьАктаОбОказанииУслуг(СведенияАктаОбОказанииУслуг, ОбъектыПечати, СтруктураПараметровПечати)
	...
	Для Каждого СведенияОДокументе ИЗ СведенияАктаОбОказанииУслуг Цикл
        ...
        Если ГП_ЭтоМодифицированныйАктРеализации Тогда
            ...
            ПредставлениеПоставщика = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбПоставщике,
                "НаименованиеДляПечатныхФорм,ИНН,КПП,ЮридическийАдрес,НомерСчета,ИдентификаторЦифровогоСчета,Банк,БИК,КоррСчет");
        КонецЕсли;
        ...
        #Вставка // Гарант+ Килипенко 24.12.2024 [F00232648] Очистка наименования договора от даты ++

        ДанныеПечатиПолучатель.Основание = ГП_РаботаСДоговорами.ОчиститьНаименованиеДоговораОтПрефикснойДаты(
            ДанныеПечатиПолучатель.Основание);

        #КонецВставки // Гарант+ Килипенко 24.12.2024 [F00232648] Очистка наименования договора от даты --
        #Вставка // Гарант+ Килипенко 18.11.2024 [F00229948] Отображение периода в ПФ документов реализации ++

        Если ГП_ЭтоМодифицированныйАктРеализации = Истина Тогда
            СтруктураПериодаРабот = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
                    СведенияОДокументе.Документ, Новый Структура("ГП_ПериодНачисленияНачало, ГП_ПериодНачисленияОкончание"));

            Если ЗначениеЗаполнено(СтруктураПериодаРабот.ГП_ПериодНачисленияОкончание) = Ложь Тогда
                СтруктураПериодаРабот.ГП_ПериодНачисленияОкончание = СведенияОДокументе.ДатаДокумента;
            КонецЕсли;
            Если ЗначениеЗаполнено(СтруктураПериодаРабот.ГП_ПериодНачисленияНачало) = Ложь Тогда
                СтруктураПериодаРабот.ГП_ПериодНачисленияНачало = СтруктураПериодаРабот.ГП_ПериодНачисленияОкончание;
            КонецЕсли;

            ПредставлениеПериодаНачисленияУслуг = ГП_РаботаСДатами.ПолучитьПредставлениеПериодаНачисления(
                    СтруктураПериодаРабот.ГП_ПериодНачисленияНачало, СтруктураПериодаРабот.ГП_ПериодНачисленияОкончание);
            Если ПустаяСтрока(ПредставлениеПериодаНачисленияУслуг) = Ложь Тогда
                ДанныеПечатиПолучатель.Основание = СтрШаблон("%1 %2",
                        ДанныеПечатиПолучатель.Основание,
                        ПредставлениеПериодаНачисленияУслуг);
            КонецЕсли;
        КонецЕсли;

        #КонецВставки // Гарант+ Килипенко 18.11.2024 [F00229948] Отображение периода в ПФ документов реализации --
		...
		Для каждого СтрокаТабличнойЧасти Из ТаблицаУслуги Цикл
            ...
            #Вставка // Гарант+ Килипенко 26.02.2025 Псевдоним Негативного воздействия ++

            // Замена наименований номенклатуры псевдонимами
            НоменклатураНаименованиеЗамены = ГП_РаботаСНоменклатурами.ЗаменитьНаименованиеНоменклатурыПсевдонимом(
                СтрокаТабличнойЧасти.НоменклатураНаименование, СтрокаТабличнойЧасти.Номенклатура);

            СтруктураПараметры.НоменклатураНаименование = НоменклатураНаименованиеЗамены;
            СтруктураПараметры.Товар = НоменклатураНаименованиеЗамены;

            #КонецВставки // Гарант+ Килипенко 26.02.2025 Псевдоним Негативного воздействия --
	...
	Для Каждого СведенияОДокументе Из СведенияСчетаНаОплату Цикл
		...
		Если ТипЗнч(СведенияОДокументе.Получатель) = Тип("СправочникСсылка.Организации")
			...
			ЕстьФаксимиле = НЕ ЭтоПечатнаяФормаЗаПоставщика
				И ЗначениеЗаполнено(Справочники.Организации.ДвоичныеДанныеКартинкиОрганизации(СведенияОДокументе.Получатель, "ФайлПечать"));
		КонецЕсли;
		...
		ИначеЕсли СведенияОДокументе.ВариантПечатиQRКода = Перечисления.ВариантПечатиQRКода.СБП Тогда
			...
			КонецЕсли
		КонецЕсли;
        #Вставка // Гарант+ Килипенко 20.10.2025 Печать Qr кода ++
        // Принудительно устанавливаем вариант печати как - ПлатежныеРеквизиты
        Если ДвоичныеДанныеQRКода = Неопределено И СчетВВалюте = Ложь И ЦифровойСчет = Ложь
                И СведенияОДокументе.ВариантПечатиQRКода <> Перечисления.ВариантПечатиQRКода.НеПечатать Тогда
            ДвоичныеДанныеQRКода = QRКодРеквизитыПлатежа(СведенияОДокументе, ДанныеПечатиЗаголовокСчета, ДанныеПечатиИтоговНДС);
        КонецЕсли;
        #КонецВставки // Гарант+ Килипенко 20.10.2025 Печать Qr кода --
        ...
        Основание = ГП_РаботаСДоговорами.ОчиститьНаименованиеДоговораОтПрефикснойДаты(Основание);
        ...
        ДокументОснование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СведенияОДокументе.Документ, "ДокументОснование");
        Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") И ДокументОснование.Пустая() = Ложь Тогда
            СтруктураПериодаРабот = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
                    ДокументОснование, Новый Структура("ГП_ПериодНачисленияНачало, ГП_ПериодНачисленияОкончание"));
            ...
            ПредставлениеПериодаНачисленияУслуг = ГП_РаботаСДатами.ПолучитьПредставлениеПериодаНачисления(
                    СтруктураПериодаРабот.ГП_ПериодНачисленияНачало, СтруктураПериодаРабот.ГП_ПериодНачисленияОкончание);
            Если ПустаяСтрока(ПредставлениеПериодаНачисленияУслуг) = Ложь Тогда
                ДанныеПечатиПокупатель.Основание = СтрШаблон("%1 %2",
                        ДанныеПечатиПокупатель.Основание,
                        ПредставлениеПериодаНачисленияУслуг);
            КонецЕсли;
        КонецЕсли;
		...
		Для каждого Строка Из СведенияОДокументе.ТаблицаДокумента Цикл
            ...
            // Замена наименований номенклатуры псевдонимами
            НоменклатураНаименованиеЗамены = ГП_РаботаСНоменклатурами.ЗаменитьНаименованиеНоменклатурыПсевдонимом(
                Строка.НоменклатураНаименование, Строка.Номенклатура);

            ДанныеПечатиСтрокаОбластьДанных.Товар = НоменклатураНаименованиеЗамены;
