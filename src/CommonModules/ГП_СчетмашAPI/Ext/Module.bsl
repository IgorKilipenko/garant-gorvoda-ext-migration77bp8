...
Функция ВыполнитьРегистрациюЧека(Знач ДанныеЧека, Знач ТипОперации, Знач ИдентификаторМагазина, Знач ЛогинПользователяСчетмаш) Экспорт
    ...
    Иначе
        РезультатФункции.Успех = Ложь;
        РезультатФункции.ТекстСообщения = "Неизвестная ошибка при запросе данных с сервера счетмаш";
    КонецЕсли;
...
Функция ПолучитьСтатусЧекаСчетмаш(Знач СтруктураИдентификатора, Знач ИдентификаторМагазина, Знач ЛогинПользователяСчетмаш) Экспорт
    ...
    ЭтоВалиднаяСтруктураОтветаОшибкиСтатуса = ТипЗнч(СтруктураТелаОтвета) = Тип("Структура")
        И (СтруктураТелаОтвета.Свойство("id") И СтруктураТелаОтвета.Свойство("status") И СтруктураТелаОтвета.Свойство("timestamp")
            И СтруктураТелаОтвета.Свойство("payload") И СтруктураТелаОтвета.Свойство("error"));

    ЭтоВалиднаяСтруктураОтветаОшибкиСтатуса = ЭтоВалиднаяСтруктураОтветаОшибкиСтатуса И ТипЗнч(СтруктураТелаОтвета.error) = Тип("Структура")
        И (СтруктураТелаОтвета.error.Свойство("code") И СтруктураТелаОтвета.error.Свойство("message"));

    Если ЭтоВалиднаяСтруктураОтветаСтатуса Тогда
        РезультатФункции.ДанныеОтвета = Новый Структура("Идентификатор", СтруктураТелаОтвета.id);
        РезультатФункции.ДанныеОтвета.Вставить("Статус", СтруктураТелаОтвета.status);
        РезультатФункции.ДанныеОтвета.Вставить("МоментВремени", СтруктураТелаОтвета.timestamp);
        РезультатФункции.ДанныеОтвета.Вставить("Данные", СтруктураТелаОтвета.payload);

    ИначеЕсли ЭтоВалиднаяСтруктураОтветаОшибкиСтатуса Тогда
        РезультатФункции.Успех = Ложь;
        РезультатФункции.ТекстСообщения = Строка(СтруктураТелаОтвета.error.message);
        РезультатФункции.ДанныеОтвета = Новый Структура("КодОшибки", СтруктураТелаОтвета.error.code);
        РезультатФункции.ДанныеОтвета.Вставить("Сообщение", СтруктураТелаОтвета.error.message);
        РезультатФункции.ДанныеОтвета.Вставить("МоментВремени", СтруктураТелаОтвета.timestamp);
        РезультатФункции.ДанныеОтвета.Вставить("Данные", СтруктураТелаОтвета.payload);
