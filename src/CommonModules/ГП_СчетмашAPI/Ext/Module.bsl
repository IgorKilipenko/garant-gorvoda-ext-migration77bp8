...
// Возвращаемое значение:
//  - Структура
Функция ПроверитьСоединениеСчетмаш(Знач ЛогинПользователяСчетмаш) Экспорт
    ...
    РезультатПолучения = ПолучитьТокен(Новый Структура("Логин, Пароль",
                ...
                Константы.ГП_КлючАвторизацииСчетмаш.ПолучитьКлючДоступа(ЭтоДемоЛогинСчетмаш(ЛогинПользователяСчетмаш))), Новый Структура);
...
Функция ВыполнитьРегистрациюЧека(Знач ДанныеЧека, Знач ТипОперации, Знач ИдентификаторМагазина, Знач ЛогинПользователяСчетмаш) Экспорт
    ...
    Попытка
        ЗапросPOST = НовыйHTTPЗапросРегистрацииЧекаСчетмаш(
                ЗаписатьЗначениеJSON(ДанныеЧека), ТипОперации, ИдентификаторМагазина, РезультатПолученияТокена.Токен);
        HTTPОтвет = HTTPСоединение.ОтправитьДляОбработки(ЗапросPOST);
    Исключение
        РезультатФункции.Успех = Ложь;
        РезультатФункции.ТекстСообщения = СтрШаблон(
...
Функция СтрокаВДату(Знач СтрокаДаты)
    ...
    Попытка
        РезультатФункции = ОбщегоНазначенияКлиентСервер.СтрокаВДату(Строка(СтрокаДаты), ЧастиДаты.ДатаВремя);
    Исключение
        РезультатФункции = Неопределено;
        ОбщегоНазначения.СообщениеПользователю(СтрШаблон(
