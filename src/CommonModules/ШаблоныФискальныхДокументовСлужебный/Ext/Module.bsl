...
Процедура ГП_ШаблонКассовыйЧекШапка(Шаблон, Данные, Параметры)
    ...
    // Принудительная установка адреса в печатном чеке по требованию Клиента
    Модуль.ДобавитьСтроку(Шаблон, СтрШаблон("Место расчетов: %1, КАССА",
        ВРег("305000, Курская область, г.Курск, ул.Кирова, д.9")), ФорматЦентр);
