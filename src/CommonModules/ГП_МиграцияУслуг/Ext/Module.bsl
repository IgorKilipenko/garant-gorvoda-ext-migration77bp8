...
Функция СоздатьДокументыУправлениеУслугамиПоДаннымРегистраСчетчиковБП77(
        Знач МассивВидовУслугБП77, Знач ОрганизацияСсылка = Неопределено, Знач ДатаСоздания = Неопределено) Экспорт
...
// Выполняет замену услуги-основания ХВС в регистре КВП_ЗависимостиУслуг на указанную услугу для зависимой услуги
//  ~~~ Используется
// Параметры:
//  ЗависимаяУслуга - СправочникСсылка.КВП_Услуги - Зависимая услуга отбора
//  НоваяОсновнаяУслуга - СправочникСсылка.КВП_Услуги - Заменяемая услуга-основание
...
//      * ТекстСообщения - Строка
Функция ВыполнитьЗаменуУслугиОснованияХВСЗависимойУслуги(
        Знач ЗависимаяУслуга, Знач НоваяОсновнаяУслуга, Знач ДатаАктуальности = Неопределено) Экспорт

    РезультатФункции = Новый Структура("Успех, КоличествоИзмененныхСтрок, ТекстСообщения", Истина, 0);
    ...
    Запрос.Текст =
        ...
        |	ЗависимостиУслуг.Организация КАК Организация,
        |	ВЫРАЗИТЬ(ЗависимостиУслуг.Объект КАК Справочник.КВП_Здания) КАК Здание,
        |	ЗависимостиУслуг.Действует КАК Действует,
        |	ЗависимостиУслуг.ЗначениеОснование КАК ЗначениеОснование,
        |	ЗависимостиУслуг.Значение КАК Значение,
        |	ЗависимостиУслуг.НачислятьПриРаспределенииПоказаний КАК НачислятьПриРаспределенииПоказаний,
        |	ЗависимостиУслуг.РазрешитьНачислениеПриРаспределенииОтрицательногоРасходаОДН КАК РазрешитьНачислениеПриРаспределенииОтрицательногоРасходаОДН
        |ПОМЕСТИТЬ ВТ_ЗависимостиУслугХВСНВЗданий
        |ИЗ
        |	РегистрСведений.КВП_ЗависимостиУслуг.СрезПоследних(&ДатаАктуальности, ) КАК ЗависимостиУслуг
        |ГДЕ
        |	ЗависимостиУслуг.УслугаОснование = &УслугаХВС
        |	И ЗависимостиУслуг.Услуга = &УслугаЗависимая
        |	И ЗависимостиУслуг.Объект ССЫЛКА Справочник.КВП_Здания
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_ЗависимостиУслугХВСНВЗданий.Период КАК Период,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.УслугаОснование КАК ИсходнаяУслугаОснование,
        |	&УслугаОснованиеЗамены КАК УслугаОснованиеЗамены,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.Услуга КАК Услуга,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.Организация КАК Организация,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.Здание КАК Объект,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.Действует КАК Действует,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.ЗначениеОснование КАК ЗначениеОснование,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.Значение КАК Значение,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.НачислятьПриРаспределенииПоказаний КАК НачислятьПриРаспределенииПоказаний,
        |	ВТ_ЗависимостиУслугХВСНВЗданий.РазрешитьНачислениеПриРаспределенииОтрицательногоРасходаОДН КАК РазрешитьНачислениеПриРаспределенииОтрицательногоРасходаОДН
        |ИЗ
        |	ВТ_ЗависимостиУслугХВСНВЗданий КАК ВТ_ЗависимостиУслугХВСНВЗданий
        |";
    ...
    Запрос.УстановитьПараметр("УслугаОснованиеЗамены", НоваяОсновнаяУслуга);
    Запрос.УстановитьПараметр("УслугаЗависимая", ЗависимаяУслуга);
    ...
    Попытка
        Для Каждого СтрокаДанных Из ДанныеЗаполнения Цикл
            НаборЗаписейДляИзменения = СоздатьНаборЗаписейЗависимостиУслуг(
                    СтрокаДанных.Период,
                    СтрокаДанных.Объект,
                    СтрокаДанных.ИсходнаяУслугаОснование,
                    СтрокаДанных.Услуга,
                    СтрокаДанных.Организация);

            // Чтение строки регистра для внесения изменений
            НаборЗаписейДляИзменения.Прочитать();
            Если НаборЗаписейДляИзменения.Количество() <> 1 Тогда
                ВызватьИсключение("Ошибка сопоставления строк регистра. Неоднозначность записи регистра");
            КонецЕсли;

            // Замена услуги-основания
            ИзменяемаяЗаписьРегистра = НаборЗаписейДляИзменения[0];
            Если ИзменяемаяЗаписьРегистра.УслугаОснование = СтрокаДанных.УслугаОснованиеЗамены Тогда
                Продолжить; // Без изменений
            Иначе
                ИзменяемаяЗаписьРегистра.УслугаОснование = СтрокаДанных.УслугаОснованиеЗамены;
            КонецЕсли;

            // Контроль наличия записей по текущей аналитике
            НовыйНаборЗаписей = СоздатьНаборЗаписейЗависимостиУслуг(
                    СтрокаДанных.Период,
                    СтрокаДанных.Объект,
                    СтрокаДанных.УслугаОснованиеЗамены,
                    СтрокаДанных.Услуга,
                    СтрокаДанных.Организация);
                    НовыйНаборЗаписей.Прочитать();
            Если НовыйНаборЗаписей.Количество() <> 0 Тогда
                ВызватьИсключение("Ошибка замены зависимости услуг, запись уже существует в регистре.");
            КонецЕсли;

            НоваяЗаписьРегистра = НовыйНаборЗаписей.Добавить();
            ЗаполнитьЗначенияСвойств(НоваяЗаписьРегистра, ИзменяемаяЗаписьРегистра);
            НаборЗаписейДляИзменения.Очистить();
    ...
    Исключение
        ...
        РезультатФункции.ТекстСообщения = СтрШаблон(
                "Ошибка при внесении изменений в зависимости услуг.
...
//  ИгнорироватьНегативноеВоздействие - Булево
//  ИгнорироватьОИ - Булево
// Возвращаемое значение:
//  Структура:
//      * Успех - Булево
...
Функция ЗаписатьДанныеУслугВРегистр(
        Знач ИсходныеДанныеУслуг, Знач СохранятьСвязи = Истина, ИгнорироватьНегативноеВоздействие = Истина, ИгнорироватьОИ = Истина) Экспорт
    ...
    Попытка
        ...
        Для Каждого УслугаБП77 Из ИсходныеДанныеУслуг Цикл
            ...
            Если ИгнорироватьНегативноеВоздействие И ЭтоНегативноеВоздействиеЦСВ Тогда
                Продолжить; // Не записываем в регистр строки негативного воздействия
            КонецЕсли;
            ...
            Если ИгнорироватьОИ И ЭтоПлатаЗаХолодноеВодоснабжениеОИ Тогда
                Продолжить; // Не записываем в регистр строки ВодоснабженияОИ
            КонецЕсли;
...
//  - Структура
//      * Успех - Булево
//      * ХолоднаяВода - ТаблицаЗначений
//      * ГорячаяВода - ТаблицаЗначений
//      * Канализация - ТаблицаЗначений
//      * ХолоднаяВодаВодоотвод - ТаблицаЗначений
//      * НегативноеВоздействие - ТаблицаЗначений
Функция ПолучитьДанныеЗаполненияНазначенийУслуг(
        Знач ДатаАктуальности = Неопределено, Знач ОрганизацияСсылка = Неопределено) Экспорт
    ...
    Запрос.Текст =
        ...
        |	ЕСТЬNULL(ГП_УслугиБП77.НаПодогрев, ЛОЖЬ) КАК НаПодогрев,
        ...
        |	ГП_ЗданияБП77.ОбъектАбонентаКод + ""_"" + ГП_ЗданияБП77.КонтрагентКод КАК СоставнойКодЛС
        |ПОМЕСТИТЬ ВТ_УслугиБП77
        |ИЗ
        |	РегистрСведений.ГП_ЗданияБП77 КАК ГП_ЗданияБП77
        |	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГП_УслугиБП77 КАК ГП_УслугиБП77
        |	ПО ГП_ЗданияБП77.КонтрагентКод = ГП_УслугиБП77.КонтрагентКод
        |	  И ГП_ЗданияБП77.ОбъектАбонентаКод = ГП_УслугиБП77.ОбъектАбонентаКод
        |ГДЕ
...
Функция СоздатьНаборЗаписейЗависимостиУслуг(
        ...
        Знач ЗависимаяУслугаСсылка = Неопределено, Знач ОрганизацияСсылка = Неопределено) Экспорт
