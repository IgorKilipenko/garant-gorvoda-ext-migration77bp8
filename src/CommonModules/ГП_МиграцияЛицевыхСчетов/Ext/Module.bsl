...
Функция ПолучитьДанныеДляСозданияВзаиморасчетовПоЛСДляНесозданных() Экспорт
    ...
    Запрос.Текст =
        ...
        |	СведенияДляВзаиморасчетовПоЛС.Активность КАК Активность,
        ...
        |	ВТ_ЛицевыеСчета.Ссылка.ГП_ИдентификаторБП77 КАК ГП_ИдентификаторБП77
        ...
        |	Контрагенты.РасширенноеПредставлениеИНН КАК РасширенноеПредставлениеИНН,
        |	Контрагенты.РасширенноеПредставлениеКПП КАК РасширенноеПредставлениеКПП,
        ...
        |	//ВТ_ЗданияБП77.ПолноеНаименованиеКонтрагентаБП77 КАК ПолноеНаименованиеКонтрагентаБП77,
        |	ВТ_ЗданияБП77.ОсновнойДоговорКод КАК ОсновнойДоговорКод,
        |	ВТ_ЗданияБП77.ОсновнойДоговорНаименование КАК ОсновнойДоговорНаименование
        |ПОМЕСТИТЬ ВТ_Контрагенты
        |ИЗ
        |	Справочник.Контрагенты КАК Контрагенты
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ЗданияБП77 КАК ВТ_ЗданияБП77
        |		ПО Контрагенты.Наименование = ВТ_ЗданияБП77.НаименованиеКонтрагентаБП77
        |			И (ВЫБОР
        |				КОГДА Контрагенты.ИНН = """"
        |					ТОГДА ИСТИНА
        |				ИНАЧЕ Контрагенты.ИНН = ВТ_ЗданияБП77.ИНН
        |			КОНЕЦ)
        |ГДЕ
        |	Контрагенты.ЭтоГруппа = ЛОЖЬ
        |	И Контрагенты.ПометкаУдаления = ЛОЖЬ
        |	И Контрагенты.ГП_СозданАвтоматически = ЛОЖЬ
        |	И Контрагенты.Ссылка В
        |			(ВЫБРАТЬ
        |				Т.Ссылка.Владелец
        |			ИЗ
        |				ВТ_СоответствияНомеровСуществующихДоговоров КАК Т)
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ЛицевыеСчетаБезВзаиморасчетов КАК ВТ_ЛицевыеСчета
        ...
        |		ПО (ВТ_Контрагенты.ИдентификаторЛС = ВТ_ЗданияБП77.ИдентификаторЛС)
        |			И (ВТ_СоответствияНомеровСуществующихДоговоров.Ссылка.Владелец = ВТ_Контрагенты.Ссылка)
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
