...
//      * Успех - Булево
//      * ТекстСообщения - Строка, Неопределено
//      * Здания - Массив из ДокументСсылка.КВП_Здания
//      * ЛицевыеСчета - Массив из ДокументСсылка.КВП_ЛицевыеСчета
Функция ВыполнитьАктуализациюНаименованийЗданийЛицевыхСчетов(ДатаАктуальности = Неопределено) Экспорт
    РезультатФункции = Новый Структура("Успех, Здания, ЛицевыеСчета, ТекстСообщения", Истина, Новый Массив, Новый Массив);

    ДанныеАктуализации = ПолучитьДанныеДляАктуализацииНаименованийЗданийЛицевыхСчетов(ДатаАктуальности);

    НачатьТранзакцию();
    Попытка
        Для Каждого СтрокаДанных Из ДанныеАктуализации Цикл
            Если СтрокаДанных.ИсходноеНаименованиеЗдания = СтрокаДанных.НовоеНаименованиеЗдания Тогда
                Продолжить;
            КонецЕсли;

            Если СтрокаДанных.ИсходноеНаименованиеЛС = СтрокаДанных.НовоеНаименованиеЛС Тогда
                ОбщегоНазначения.СообщитьПользователю(СтрШаблон("Не выполнено актуализация наименования здания л/с ""%1"""
                        + ", т.к. наименования л/с совпадают", СтрокаДанных.ИсходноеНаименованиеЛС));
                Продолжить;
            КонецЕсли;

            // Актуализация здания
            ЗданиеОбъект = СтрокаДанных.Здание.ПолучитьОбъект();
            ЗданиеОбъект.Наименование = СтрокаДанных.НовоеНаименованиеЗдания;
            ЗданиеОбъект.Записать();
            РезультатФункции.Здания.Добавить(ЗданиеОбъект.Ссылка);

            // Актуализация л/с
            ЛицевойСчетОбъект = СтрокаДанных.ЛицевойСчет.ПолучитьОбъект();
            ЛицевойСчетОбъект.Наименование = СтрокаДанных.НовоеНаименованиеЗдания;
            ЛицевойСчетОбъект.Записать();
            РезультатФункции.ЛицевыеСчета.Добавить(ЛицевойСчетОбъект.Ссылка);
        КонецЦикла;
    ...
    Исключение
        ...
        РезультатФункции.Успех = Ложь;
        РезультатФункции.Здания.Очистить();
        РезультатФункции.ЛицевыеСчета.Очистить();
        РезультатФункции.ТекстСообщения = СтрШаблон(
                "Ошибка при актуализации наименований зданий л/с.
                |Информация: %1", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
    КонецПопытки;
...
// Гарант+ Килипенко 18.12.2024 [F00232368] Закрытие неактуальных л/с ++
//
// Получает данные для актуализации наименований зданий и л/с перенесенных ранее ЛайнКор
// Параметры:
//  ДатаАктуальности - Дата, Неопределено
// Возвращаемое значение:
//  - ТаблицаЗначений
//      * ЛицевойСчет - СправочникСсылка.КВП_ЛицевыеСчета
//      * ИсходноеНаименованиеЛС - Строка
//      * НовоеНаименованиеЛС - Строка
//      * ИсходноеНаименованиеЗдания - Строка
//      * НовоеНаименованиеЗдания - Строка
//      * Помещение - СправочникСсылка.УПЖКХ_Помещения
//      * Здание - СправочникСсылка.КВП_Здания
//      * ИдентификаторЛС - Строка
Функция ПолучитьДанныеДляАктуализацииНаименованийЗданийЛицевыхСчетов(Знач ДатаАктуальности = Неопределено) Экспорт
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |ПОМЕСТИТЬ ВТ_ЗакрытыеЛицевыеСчета
        |ИЗ
        |	РегистрСведений.КВП_ЛицевыеСчета.СрезПоследних(&ДатаАктуальности, ) КАК ЛицевыеСчета
        |ГДЕ
        ...
        |	ЛицевыеСчета.Наименование КАК Наименование,
        |	ЛицевыеСчета.Адрес КАК Помещение,
        |	ВЫРАЗИТЬ(ЛицевыеСчета.Адрес.Владелец КАК Справочник.КВП_Здания) КАК Здание,
        |	ЛицевыеСчета.Идентификатор КАК Идентификатор,
        |	ЛицевыеСчета.ДокументОснованиеЛС КАК ДокументОснованиеЛС,
        ...
        |	ЛицевыеСчета.ГП_Комментарий КАК ГП_Комментарий,
        |	ЛицевыеСчета.ГП_ИдентификаторБП77 КАК ГП_ИдентификаторБП77,
        |	ЛицевыеСчета.ГП_СозданАвтоматически КАК ГП_СозданАвтоматически
        |ПОМЕСТИТЬ ВТ_ЛицевыеСчета
        |ИЗ
        |	Справочник.КВП_ЛицевыеСчета КАК ЛицевыеСчета
        |ГДЕ
        ...
        |	И ЛицевыеСчета.ЭтоГруппа = ЛОЖЬ
        |	И ЛицевыеСчета.ПометкаУдаления = ЛОЖЬ
        |	И (ВЫРАЗИТЬ(ЛицевыеСчета.Адрес.Владелец КАК Справочник.КВП_Здания)) <> ЗНАЧЕНИЕ(Справочник.КВП_Здания.ПустаяСсылка)
        |	И ЛицевыеСчета.Идентификатор <> """"
        |	И ЛицевыеСчета.Идентификатор ПОДОБНО ""%[0-9]~_[0-9]%"" СПЕЦСИМВОЛ ""~""
        |	И НЕ ЛицевыеСчета.Ссылка В
        |				(ВЫБРАТЬ
        ...
        |			И (КонтрагентыБП77.Код <> """")
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_ЛицевыеСчета.ЛицевойСчет КАК ЛицевойСчет,
        |	ВТ_ЛицевыеСчета.Наименование КАК ИсходноеНаименованиеЛС,
        |	ВЫРАЗИТЬ("""" КАК СТРОКА(200)) КАК НовоеНаименованиеЛС,
        |	ВТ_ЛицевыеСчета.Здание.Наименование КАК ИсходноеНаименованиеЗдания,
        |	ВТ_ЗданияБП77.ОбъектАбонентаНаименование КАК НовоеНаименованиеЗдания,
        |	ВТ_ЛицевыеСчета.Помещение КАК Помещение,
        |	ВТ_ЛицевыеСчета.Здание КАК Здание,
        |	ВТ_ЗданияБП77.НаименованиеКонтрагентаБП77 КАК НаименованиеКонтрагентаБП77,
        |	ВТ_ЗданияБП77.ПолнНаименованиеКонтрагентаБП77 КАК ПолнНаименованиеКонтрагентаБП77,
        |	ВТ_ЛицевыеСчета.Идентификатор КАК ИдентификаторЛС,
        |	ВТ_ЛицевыеСчета.ОтветственныйСобственникПредставление КАК ОтветственныйСобственникПредставление,
        |	ВТ_ЛицевыеСчета.ГП_ИдентификаторБП77 КАК ГП_ИдентификаторБП77,
        |	ВТ_ЗданияБП77.КонтрагентКод КАК КонтрагентКод,
        |	ВТ_ЗданияБП77.ОбъектАбонентаКод КАК ОбъектАбонентаКод,
        ...
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ЗданияБП77 КАК ВТ_ЗданияБП77
        |		ПО ВТ_ЛицевыеСчета.Идентификатор = ВТ_ЗданияБП77.ИдентификаторЛС
        |			И ВТ_ЛицевыеСчета.Здание.Наименование <> ВТ_ЗданияБП77.ОбъектАбонентаНаименование
        |			И ВТ_ЛицевыеСчета.Помещение.Наименование = ВТ_ЗданияБП77.ДоговорНомер
        |";
    ...
    // Формирование новых наименования л/с
    Для Каждого СтрокаДанных Из РезультатФункции Цикл
        НовоеНаименованиеЛС = СформироватьНаименованиеЛицевогоСчета(
                СтрокаДанных.НаименованиеКонтрагентаБП77, СтрокаДанных.НовоеНаименованиеЗдания);

        СтрокаДанных.НовоеНаименованиеЛС = НовоеНаименованиеЛС;
    КонецЦикла;
...
Функция ВыполнитьКорректировкуОткрытыхЛСПоДублямДоговоров() Экспорт
    ...
    НачатьТранзакцию();
    Попытка
        ДанныеДляКорректировкиСвязейКонтрагентов = Новый ТаблицаЗначений;
        ДанныеДляКорректировкиСвязейКонтрагентов.Колонки.Добавить("Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
        ДанныеДляКорректировкиСвязейКонтрагентов.Колонки.Добавить("КонтрагентЗамены", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));

        ДанныеДляКорректировкиСвязейДоговоров = Новый ТаблицаЗначений;
        ДанныеДляКорректировкиСвязейДоговоров.Колонки.Добавить("Договор", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
        ДанныеДляКорректировкиСвязейДоговоров.Колонки.Добавить("ДоговорЗамены",
        ...
        Для Каждого ДокументыОткрытияЛС Из СтруктураДанныхКорректировки.ДокументыОткрытияЛС Цикл
            ...
            // Заполнение данных для последующей корректировки связей
            НоваяСтрокаКонтрагентов = ДанныеДляКорректировкиСвязейКонтрагентов.Добавить();
            НоваяСтрокаКонтрагентов.Контрагент = ДокументыОткрытияЛСОбъект.Контрагент;
            НоваяСтрокаКонтрагентов.КонтрагентЗамены = ДокументыОткрытияЛС.КонтрагентЗамены;
        ...
        КонецЦикла;
        ДанныеДляКорректировкиСвязейКонтрагентов.Свернуть("Контрагент, КонтрагентЗамены");
        ...
        Для Каждого СведенияДляВзаиморасчетов Из СтруктураДанныхКорректировки.СведенияДляВзаиморасчетов.Строки Цикл
            ...
            Для Каждого СтрокаКорректировки Из СведенияДляВзаиморасчетов.Строки Цикл
                ...
                Для Каждого СтрокаИзменения Из НайденныеСтроки Цикл
                    // Заполнение данных для последующей корректировки связей
                    НоваяСтрокаКонтрагентов = ДанныеДляКорректировкиСвязейКонтрагентов.Добавить();
                    НоваяСтрокаКонтрагентов.Контрагент = СтрокаИзменения.Контрагент;
                    НоваяСтрокаКонтрагентов.КонтрагентЗамены = СтрокаКорректировки.КонтрагентЗамены;
                    НоваяСтрокаДоговоров = ДанныеДляКорректировкиСвязейДоговоров.Добавить();
                    НоваяСтрокаДоговоров.Договор = СтрокаИзменения.ДоговорКонтрагента;
                    НоваяСтрокаДоговоров.ДоговорЗамены = СтрокаКорректировки.ДоговорЗамены;
        ...
        КонецЦикла;
        ДанныеДляКорректировкиСвязейКонтрагентов.Свернуть("Контрагент, КонтрагентЗамены");
        ДанныеДляКорректировкиСвязейДоговоров.Свернуть("Договор, ДоговорЗамены");

        // Корректировка связей Договоров (БП77)
        НаборЗаписейБП77 = РегистрыСведений.ГП_ДоговорыБП77.СоздатьНаборЗаписей();
        НаборЗаписейБП77.Прочитать();
        ТребуетсяЗаписьРегистра = Ложь;

        Для Каждого ТекущаяЗапись Из НаборЗаписейБП77 Цикл
            НайденныеСтроки = ДанныеДляКорректировкиСвязейДоговоров.НайтиСтроки(Новый Структура("Договор", ТекущаяЗапись.Договор));
            Если НайденныеСтроки.Количество() = 0 Тогда
                Продолжить;
            ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
                ОбщегоНазначения.СообщитьПользователю(СтрШаблон(
                        "ВНИМАНИЕ! Неоднозначная замена для договора: %1", Строка(ТекущаяЗапись.Договор)));
            КонецЕсли;

            ТекущаяЗапись.Договор = НайденныеСтроки[0].ДоговорЗамены;
            ТребуетсяЗаписьРегистра = Истина;
        КонецЦикла;

        Если ТребуетсяЗаписьРегистра = Истина Тогда
            НаборЗаписейБП77.Записать(Истина);
        КонецЕсли;

        // Корректировка связей Контрагентов (БП77)
        НаборЗаписейБП77 = РегистрыСведений.ГП_КонтрагентыБП77.СоздатьНаборЗаписей();
        НаборЗаписейБП77.Прочитать();
        ТребуетсяЗаписьРегистра = Ложь;

        Для Каждого ТекущаяЗапись Из НаборЗаписейБП77 Цикл
            НайденныеСтроки = ДанныеДляКорректировкиСвязейКонтрагентов.НайтиСтроки(Новый Структура("Контрагент", ТекущаяЗапись.Контрагент));
            Если НайденныеСтроки.Количество() = 0 Тогда
                Продолжить;
            ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
                ОбщегоНазначения.СообщитьПользователю(СтрШаблон(
                        "ВНИМАНИЕ! Неоднозначная замена для контрагента: %1", Строка(ТекущаяЗапись.Контрагент)));
            КонецЕсли;

            ТекущаяЗапись.Контрагент = НайденныеСтроки[0].КонтрагентЗамены;
            ТребуетсяЗаписьРегистра = Истина;
        КонецЦикла;
        ...
        Для Каждого ДоговорДляУдаления Из СтруктураДанныхКорректировки.ДоговорыДляУдаления Цикл
            ...
            ДоговорОбъект.Наименование = СтрШаблон("[удалить] %1", ДоговорОбъект.Наименование);
            ДоговорОбъект.Записать();
            ДоговорОбъект.УстановитьПометкуУдаления(Истина, Ложь);
        КонецЦикла;
        ...
        Для Каждого КонтрагентДляУдаления Из СтруктураДанныхКорректировки.КонтрагентыДляУдаления Цикл
            ...
            КонтрагентОбъект.Наименование = СтрШаблон("[удалить] %1", КонтрагентОбъект.Наименование);
            КонтрагентОбъект.Записать();
            КонтрагентОбъект.УстановитьПометкуУдаления(Истина, Ложь);
        КонецЦикла;
...
Функция ПолучитьДанныеКорректировкиЛСПоДублямДокументов() Экспорт
    ...
    Запрос.Текст =
        ...
        |	ВТ_ДокументыОткрытияЛСДляКорректировки.ИдентификаторБП77 КАК ИдентификаторБП77,
