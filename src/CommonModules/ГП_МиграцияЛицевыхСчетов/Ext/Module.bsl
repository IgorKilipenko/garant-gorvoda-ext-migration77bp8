...
Функция ВыполнитьКорректировкуОткрытыхЛСПоДублямДоговоров() Экспорт
    ...
    Попытка
        ...
        Для Каждого СведенияДляВзаиморасчетов Из СтруктураДанныхКорректировки.СведенияДляВзаиморасчетов.Строки Цикл
            ...
            //Для Счетчик = 0 По ДанныеДляИзмененияВзаиморасчетов.Количество() - 1 Цикл
            //    Если ДанныеДляИзмененияВзаиморасчетов[Счетчик] = Неопределено Тогда
            //        Продолжить;
            //    КонецЕсли;

            //    УправлениеСведениямиДляВзаиморасчетовОбъект.СведенияДляВзаиморасчетов[Счетчик].ДоговорКонтрагента =
            //        ДанныеДляИзмененияВзаиморасчетов[Счетчик].ДоговорКонтрагента;
            //    УправлениеСведениямиДляВзаиморасчетовОбъект.СведенияДляВзаиморасчетов[Счетчик].Контрагент =
            //        ДанныеДляИзмененияВзаиморасчетов[Счетчик].Контрагент;
            //КонецЦикла;
        ...
        Для Каждого КонтрагентДляУдаления Из СтруктураДанныхКорректировки.ДоговорыДляУдаления Цикл
            КонтрагентОбъект = КонтрагентДляУдаления.Договор.ПолучитьОбъект();
