    ...
    Попытка
        Для Каждого АбонентБП77 Из ТаблицаСоответствияКонтрагентов Цикл
            ...
            Если ЗначениеЗаполнено(АбонентБП77.КонтрагентСопоставление) = Ложь Тогда
                Если РазрыватьСуществующиеСвязи = Истина Тогда
...
Функция ПолучитьТаблицуАбонентовБП77ДляСоздания() Экспорт
    ...
    Запрос.Текст =
        ...
        |	И КВП_ЛицевыеСчета.ГП_ИдентификаторБП77 ПОДОБНО ""%[0-9]~_[0-9]%"" СПЕЦСИМВОЛ ""~""
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |		ПО (ВТ_ПеремещенныеЛицевыеСчета.ГП_ИдентификаторБП77 ПОДОБНО (ВТ_Абоненты.Код + ""~_[0-9]%"") СПЕЦСИМВОЛ ""~"")
        |
        |СГРУППИРОВАТЬ ПО
        |	ВТ_Абоненты.Код
        |;
...
Функция ПолучитьТаблицуДляСозданияСвязейКонтрагентовБП77(Знач ДатаАктуальности = Неопределено) Экспорт
    Запрос = Новый Запрос;
    // 15.22.2024 Изменен текст запроса для установления соответствий контрагентов
...
// Получает массив созданных ГП контрагентов
// Возвращаемое значение:
//  - Массив из СправочникСсылка.Контрагент
Функция ПолучитьВсехКонтрагентовСПризнакомСозданАвтоматически(Знач БезПометкиУдаления = Ложь) Экспорт
    ...
    Если БезПометкиУдаления <> Истина Тогда
        Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Контрагенты.ПометкаУдаления = ЛОЖЬ", "");
    КонецЕсли;
    ...
    Пока Выборка.Следующий() Цикл
        РезультатФункции.Добавить(Выборка.Контрагент);
    КонецЦикла;
...
Функция УдалитьВсехКонтрагентовСПризнакомСозданАвтоматически(
        Знач УстанавливатьПометкуУдаления = Истина, Знач ОчиститьСвязи = Истина) Экспорт

    РезультатФункции = Новый Структура("Успех, КоличествоОчищенныхСвязей, КоличествоУдаленных, ТекстСообщения", Истина, 0, 0);

    КонтрагентыДляУдаления = ПолучитьВсехКонтрагентовСПризнакомСозданАвтоматически(?(УстанавливатьПометкуУдаления = Истина, Истина, Ложь));

    НачатьТранзакцию();
    Попытка
        Если ОчиститьСвязи = Истина Тогда
            Запрос = Новый Запрос;
            Запрос.Текст =
                "ВЫБРАТЬ РАЗРЕШЕННЫЕ
                |   *
                |ИЗ
                |   РегистрСведений.ГП_КонтрагентыБП77 КАК Контрагенты
                |ГДЕ
                |   Контрагенты.Контрагент В (&КонтрагентыДляУдаления)
                |";

            Запрос.УстановитьПараметр("КонтрагентыДляУдаления", КонтрагентыДляУдаления);
            РезультатЗапроса = Запрос.Выполнить();
            ДанныеДляОчистки = РезультатЗапроса.Выгрузить();

            Для Каждого СтрокаДанных Из ДанныеДляОчистки Цикл
                НаборЗаписейКонтрагенты = РегистрыСведений.ГП_КонтрагентыБП77.СоздатьНаборЗаписей();
                НаборЗаписейКонтрагенты.Отбор.Код.Установить(СтрокаДанных.Код);
                НаборЗаписейКонтрагенты.Прочитать();

                ТребуетсяЗаписьНабора = Ложь;
                Для Каждого ТекущаяЗапись Из НаборЗаписейКонтрагенты Цикл
                    Если ТекущаяЗапись.Контрагент <> СтрокаДанных.Контрагент Тогда
                        Продолжить;
                    КонецЕсли;
                    ТекущаяЗапись.Контрагент = Справочники.Контрагенты.ПустаяСсылка();
                    ТребуетсяЗаписьНабора = Истина;
                КонецЦикла;

                Если ТребуетсяЗаписьНабора = Истина Тогда
                    НаборЗаписейКонтрагенты.Записать(Истина);
                    РезультатФункции.КоличествоОчищенныхСвязей = РезультатФункции.КоличествоОчищенныхСвязей + 1;
                КонецЕсли;
            КонецЦикла;
        КонецЕсли;

        Для Каждого КонтрагентСсылка Из КонтрагентыДляУдаления Цикл
            КонтрагентОбъект = КонтрагентСсылка.ПолучитьОбъект();
            Если УстанавливатьПометкуУдаления = Истина Тогда
                КонтрагентОбъект.УстановитьПометкуУдаления(Истина, Ложь);
            Иначе
                КонтрагентОбъект.Удалить();
            КонецЕсли;

            РезультатФункции.КоличествоУдаленных = РезультатФункции.КоличествоУдаленных + 1;
        КонецЦикла;
    ...
    Исключение
        ...
        РезультатФункции.Успех = Ложь;
        РезультатФункции.КоличествоОчищенныхСвязей = 0;
        РезультатФункции.КоличествоУдаленных = 0;
        РезультатФункции.ТекстСообщения = СтрШаблон(
                "Ошибка при удалении контрагентов.
...
Функция СформироватьТекстЗапросаДляСозданияСвязейКонтрагентовБП77()
    // 15.12.2024 Новый вариант текста запроса (не тестировался при переносе данных)
    ТекстЗапроса =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |	ЗданияБП77.ВидДоговораКод КАК ВидДоговораКод,
        |	ЗданияБП77.ВидДоговораНаименование КАК ВидДоговораНаименование,
        ...
        |ГДЕ
        |	ИСТИНА
        |	И ЗданияБП77.ВидДоговораКод = ""00018""
        ...
        |	ОтветственныйСобственникНанимательЛицевогоСчета.ЛицевойСчет.Идентификатор КАК ИдентификаторЛС,
        |	ВЫРАЗИТЬ(ОтветственныйСобственникНанимательЛицевогоСчета.ОтветственныйВладелец КАК Справочник.Контрагенты) КАК Контрагент,
        |	ВТ_ЗданияБП77.КонтрагентКод КАК КонтрагентКодБП77
        |ПОМЕСТИТЬ ВТ_ОтветственныйСобственникЛицевогоСчета
        |ИЗ
        |	РегистрСведений.УПЖКХ_ОтветственныйСобственникНанимательЛицевогоСчета.СрезПоследних(&ДатаАктуальности, ОтветственныйВладелец ССЫЛКА Справочник.Контрагенты) КАК ОтветственныйСобственникНанимательЛицевогоСчета
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ЗданияБП77 КАК ВТ_ЗданияБП77
        |		ПО ОтветственныйСобственникНанимательЛицевогоСчета.ЛицевойСчет.Идентификатор = ВТ_ЗданияБП77.ИдентификаторЛС
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
        |	ВТ_ОтветственныйСобственникЛицевогоСчета.Контрагент КАК Контрагент,
        |	ВТ_ОтветственныйСобственникЛицевогоСчета.КонтрагентКодБП77 КАК КонтрагентКодБП77
        |ПОМЕСТИТЬ КодыСуществующихКонтрагентовЛС
        |ИЗ
        |	ВТ_ОтветственныйСобственникЛицевогоСчета КАК ВТ_ОтветственныйСобственникЛицевогоСчета
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |	Контрагенты.ГП_ИдентификаторБП77 КАК ГП_ИдентификаторБП77,
        |	КодыСуществующихКонтрагентовЛС.КонтрагентКодБП77 КАК КонтрагентКодБП77
        |ПОМЕСТИТЬ ВТ_РанееСозданныеКонтрагенты
        |ИЗ
        |	Справочник.Контрагенты КАК Контрагенты
        |		ЛЕВОЕ СОЕДИНЕНИЕ КодыСуществующихКонтрагентовЛС КАК КодыСуществующихКонтрагентовЛС
        |		ПО Контрагенты.Ссылка = КодыСуществующихКонтрагентовЛС.Контрагент
        |			И (КодыСуществующихКонтрагентовЛС.КонтрагентКодБП77 <> """")
        |ГДЕ
        ...
        |	ВТ_РанееСозданныеКонтрагенты.КонтрагентКодБП77 КАК КонтрагентКодБП77
        |ПОМЕСТИТЬ ВТ_КонтрагентыСИдентификаторомБП77
        |ИЗ
        |	ВТ_РанееСозданныеКонтрагенты КАК ВТ_РанееСозданныеКонтрагенты
        |ГДЕ
        |	ИСТИНА
        |	И НЕ ВТ_РанееСозданныеКонтрагенты.КонтрагентКодБП77 ЕСТЬ NULL
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |ГДЕ
        |	ИСТИНА
        |	И ВТ_РанееСозданныеКонтрагенты.КонтрагентКодБП77 ЕСТЬ NULL
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |// Коды контрагентов для зданий БП77 (из регистра ГП_ЗданияБП77)
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |ГДЕ
        |	ВТ_ЗданияБП77.ЭтоНегативноеВоздействиеЦСВ = ЛОЖЬ
        |	И ВТ_ЗданияБП77.ЭтоПлатаЗаХолодноеВодоснабжениеОИ = ЛОЖЬ
        |
        |СГРУППИРОВАТЬ ПО
        |	ВТ_ЗданияБП77.КонтрагентКод
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |		ПО ВТ_КонтрагентыСИдентификаторомБП77.КонтрагентКодБП77 = ВТ_АбонентыБП77.Код
        |
        |ОБЪЕДИНИТЬ ВСЕ
        |
        |ВЫБРАТЬ
        ...
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_АбонентыБП77 КАК ВТ_АбонентыБП77
        |		ПО (ВЫБОР
        |				КОГДА ВТ_КонтрагентыБезИдентификаторомБП77.ИНН = ВТ_АбонентыБП77.ИНН
        ...
        |					ТОГДА ИСТИНА
        |				КОГДА ВТ_КонтрагентыБезИдентификаторомБП77.ИНН <> """"
        |					ТОГДА ВТ_КонтрагентыБезИдентификаторомБП77.ИНН = ВТ_АбонентыБП77.ИНН
        |							И ВТ_КонтрагентыБезИдентификаторомБП77.КПП = ВТ_АбонентыБП77.КПП
        |				КОГДА ВТ_КонтрагентыБезИдентификаторомБП77.Наименование <> """"
        |					ТОГДА ВТ_КонтрагентыБезИдентификаторомБП77.Наименование = ВТ_АбонентыБП77.Наименование
        |				ИНАЧЕ ЛОЖЬ
        |			КОНЕЦ)
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |// Исключение дублирования соответствий контрагентов
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_Результат.КодБП77 КАК Код,
        |	ВТ_Результат.Контрагент КАК Контрагент,
        |	МИНИМУМ(ВТ_Результат.КонтрагентСопоставление) КАК КонтрагентСопоставление
        |ИЗ
