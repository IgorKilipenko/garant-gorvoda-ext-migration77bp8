...
Функция СоздатьАбонентовДляПеренесенныхЛицевыхСчетов() Экспорт
    ...
    Попытка
        Для каждого СтрокаДанныхКонтрагента Из ТаблицаКонтрагентовБП77 Цикл
            ...
            Если ЗначениеЗаполнено(СтрокаДанныхКонтрагента.ЮридическийАдрес) Тогда
                // ЮрАдресКонтрагента
                ...
                СтрокаКонтактнойИнформации.Значение = СтрокаДанныхКонтрагента.ЮридическийАдрес;
            КонецЕсли;
            Если ЗначениеЗаполнено(СтрокаДанныхКонтрагента.ПочтовыйАдрес) Тогда
                // ПочтовыйАдресКонтрагента
                СтрокаКонтактнойИнформации = ТаблицаКонтактнойИнформации.Добавить();
                СтрокаКонтактнойИнформации.Вид = Справочники.ВидыКонтактнойИнформации.ПочтовыйАдресКонтрагента;
                СтрокаКонтактнойИнформации.Значение = СтрокаДанныхКонтрагента.ПочтовыйАдрес;
            КонецЕсли;
            Если ЗначениеЗаполнено(СтрокаДанныхКонтрагента.Телефоны) Тогда
                // ТелефонКонтрагента
                СтрокаКонтактнойИнформации = ТаблицаКонтактнойИнформации.Добавить();
                СтрокаКонтактнойИнформации.Вид = Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента;
                СтрокаКонтактнойИнформации.Значение = СтрокаДанныхКонтрагента.Телефоны;
            КонецЕсли;

            Если ТаблицаКонтактнойИнформации.Количество() > 0 Тогда
                // УправлениеКонтактнойИнформацией.УстановитьКонтактнуюИнформациюОбъекта(
                //     НовыйКонтрагентОбъект, ТаблицаКонтактнойИнформации, Истина); // Установка с замещением

                Для каждого СтрокаКонтактнойИнформации Из ТаблицаКонтактнойИнформации Цикл
                    НоваяКИ = НовыйКонтрагентОбъект.КонтактнаяИнформация.Добавить();
                    НоваяКИ.Вид = СтрокаКонтактнойИнформации.Вид;
                    НоваяКИ.Значение = СтрокаКонтактнойИнформации.Значение;
                КонецЦикла;
            КонецЕсли;
    ...
    Исключение
        ...
        ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(
            "Ошибка при создании контрагентов.",
            УровеньЖурналаРегистрации.Ошибка, , ,
            ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));

        РезультатФункции.СозданныеКонтрагенты.Очистить();
        //РезультатФункции.ТекстСообщения = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
