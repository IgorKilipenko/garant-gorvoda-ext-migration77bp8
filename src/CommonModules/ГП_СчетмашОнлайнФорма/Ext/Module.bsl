...
Процедура ПробитьЧекСчетмаш(Форма, Знач ТипОперации, Знач ТекущаяСтрокаОплаты, Знач Логин) Экспорт
    // // Запись JSON
    // Структура = Новый Структура;
    // Структура.Вставить("timestamp", ТекущаяСтрока.timestamp); // дата оплаты
    // Если ТипОперации = "sell_refund" Тогда // Коррекция
    //     // Структура.Вставить("external_id", "RF_" + ТекущаяСтрока.external_id);
    //     Структура.Вставить("external_id", ТекущаяСтрока.external_id);
    // Иначе
    //     Структура.Вставить("external_id", ТекущаяСтрока.external_id);
    // КонецЕсли;
    ...
    Для каждого ТекущаяСтрокаРасшифровки Из НайденныеСтрокиРасшифровки Цикл
        ...
        СтруктураЭлементаЧека.type = ГП_СчетмашAPIКлиентСервер.ПолучитьПризнакПредметаРасчетаСчетмашПоЗначению(ТекущаяСтрокаРасшифровки.ПризнакПредметаРасчета);
        СтруктураЭлементаЧека.mode = ГП_СчетмашAPIКлиентСервер.ПолучитьПризнакСпособаРасчетаСчетмашПоЗначению(ТекущаяСтрокаРасшифровки.ПризнакСпособаРасчета);
        ...
        СтруктураЭлементаЧека.tax_sum = УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекущаяСтрокаРасшифровки.Сумма,
            Истина, ТекущаяСтрокаРасшифровки.СтавкаНДСЗначение); // Сумма НДС
