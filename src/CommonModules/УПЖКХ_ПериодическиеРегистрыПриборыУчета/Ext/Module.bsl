...
Функция ГП_ДополнитьТекстЗапросаУсловиемПричиныОтключения(Знач ТекстОтбораПоДействию, Знач ПараметрыПостроения)
    ...
    // ВНИМАНИЕ !!!
    // !!! Глобальное отключение контроля просроченной поверки счетчиков - Установлено по требованию Заказчика
    ПараметрыПостроения.Вставить("ГП_ИгнорироватьОтключениеНаПоверку", Истина);
    ...
    ИгнорироватьОтключениеПриПоверке =
        ЭтоОбрабатываемыйПризнакДействия
            И ПараметрыПостроения.Свойство("ГП_ИгнорироватьОтключениеНаПоверку")
            И ПараметрыПостроения.ГП_ИгнорироватьОтключениеНаПоверку = Истина;

    Если ИгнорироватьОтключениеПриПоверке = Ложь Тогда
    ...
    РезультатФункции = СтрШаблон(
        ...
        ?(ПараметрыПостроения.ПризнакДействия = Истина, "ИЛИ", "И"));
