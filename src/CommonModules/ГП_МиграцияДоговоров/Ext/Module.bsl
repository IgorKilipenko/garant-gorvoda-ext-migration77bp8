...
Функция ЗаписатьДанныеДоговоровВРегистр(Знач ИсходныеДанныеДоговоров, Знач СохранятьСвязи = Истина) Экспорт
    ...
    КлючиЧиселДоговора = "СтавкаШтрафныхСанкций,ОплатаДоговора,ОтражатьСуммовуюРазницуПриЗачетеАванса,ПроцентКорректировкиКурсаУЕ"
        + ",ВодаПоДог,КанПоДог,СуммаПоДог";
    КлючиДатДоговора = "ДатаВозникновенияОбязательства,ДатаПогашенияОбязательства,ДатаНачалаНачисленияШтрафныхСанкций"
    ...
    Попытка
        ...
        Для Каждого ДоговорБП77 Из ИсходныеДанныеДоговоров Цикл
            ...
            СвязанныйДоговор = Неопределено;
            Если СохранятьСвязи Тогда
                НаборЗаписейДоговоров.Прочитать();
                Если НаборЗаписейДоговоров.Количество() > 0 Тогда
                    СвязанныйДоговор = НаборЗаписейДоговоров[0].Договор;
                    СвязанныйДоговор = ?(ЗначениеЗаполнено(СвязанныйДоговор), СвязанныйДоговор, Неопределено);
            ...
            ЗаполнитьЗначенияСвойств(НоваяЗапись, ДоговорБП77, ,
                СтрШаблон("%1,%2,%3", КлючиУсловийДоговора, КлючиЧиселДоговора, КлючиДатДоговора));

            // Заполнение полей с преобразованием типов
            Для Каждого Ключ Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КлючиДатДоговора, ",", Ложь, Истина) Цикл
            ...
            КонецЦикла;
            Для Каждого Ключ Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КлючиЧиселДоговора, ",", Ложь, Истина) Цикл
                НоваяЗапись[Ключ] = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ДоговорБП77[Ключ]);
            КонецЦикла;

            // Заполнение поля Номер (договора)
            СтруктураНомера = ГП_МиграцияОбщегоНазначения.ПолучитьНомерДоговораПоНаименованию(ДоговорБП77.Наименование);
            Если СтруктураНомера.Успех = Истина Тогда
                НоваяЗапись.Номер = СтруктураНомера.Номер + ?(ПустаяСтрока(СтруктураНомера.Суффикс),
