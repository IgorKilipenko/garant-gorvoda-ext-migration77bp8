...
Функция ПолучитьТаблицуДанныхОплатПрочихУслугСчетмаш(
        Знач ДатаНачала, Знач ДатаОкончания, Знач СтруктураОтбора, МВТ = Неопределено) Экспорт
...
Функция ПолучитьТекстЗапросаДанныхОплатПрочихУслугСчетмаш(Знач СОтборомПоКонтрагенту = Ложь, Знач ПоместитьВоВременнуюТаблицу = Ложь)
    ТекстЗапроса =
        ...
        |	ПоступлениеНаРасчетныйСчет.СубконтоКт3 КАК СубконтоКт3,
        ...
        |			ТОГДА ВЫБОР
        |                   КОГДА СчетНаОплатуПокупателюТовары.Номенклатура.Услуга ТОГДА ЗНАЧЕНИЕ(Перечисление.ПризнакиПредметаРасчета.Услуга)
        ...
        |	СчетНаОплатуПокупателюТовары.Сумма + СчетНаОплатуПокупателюТовары.СуммаНДС КАК СуммаСчетаНаОплату,
...
Функция ПолучитьТекстЗапросаРасшифровкиОплатПрочихУслугДляЧековСчетмаш(
    ...
    ТекстЗапроса = ТекстЗапроса
        ...
        |           ТОГДА ВТ_ТаблицаОплатПрочихУслугСчетмаш.СуммаПлатежа
        |       ИНАЧЕ
        |           ВТ_ТаблицаОплатПрочихУслугСчетмаш.СуммаПлатежа * ВТ_СуммыОплатПоСчету.Сумма / ВТ_ТаблицаОплатПрочихУслугСчетмаш.СуммаДокумента
        |   КОНЕЦ КАК Сумма,
