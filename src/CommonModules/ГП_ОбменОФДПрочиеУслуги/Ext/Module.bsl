...
Функция ПолучитьРасшифровкуОплатПрочихУслугДляЧековСчетмаш(Знач ДатаНачала, Знач ДатаОкончания, Знач СтруктураОтбора) Экспорт
    ...
    // Пересчет сумм НДС
    РасшифровкаОплатДляЧеков.Колонки.Добавить("СуммаНДС", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
...
Функция ПолучитьТекстЗапросаРасшифровкиОплатПрочихУслугДляЧековСчетмаш(
    ...
    ТекстЗапроса = ТекстЗапроса
        ...
        |	СУММА(ВТ_ТаблицаОплатПрочихУслугСчетмаш.СуммаПлатежа) КАК СуммаДокументаОплаты,
        ...
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	МАКСИМУМ(ОплатаСчетов.Период) КАК Период,
        |	ВТ_СуммыОплатПоСчету.Документ КАК Документ,
        |	ВТ_СуммыОплатПоСчету.Контрагент КАК Контрагент,
        |	СУММА(ОплатаСчетов.Сумма) КАК СуммаОплат
        |ПОМЕСТИТЬ ВТ_ОплаченныеСуммыПоСчетам
        |ИЗ
        |	РегистрНакопления.ОплатаСчетов КАК ОплатаСчетов
        |	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СуммыОплатПоСчету_Общая КАК ВТ_СуммыОплатПоСчету
        |	ПО ОплатаСчетов.СчетНаОплату = ВТ_СуммыОплатПоСчету.СчетНаОплату
        |		И ОплатаСчетов.Период < ВТ_СуммыОплатПоСчету.Документ.Дата
        |		И ОплатаСчетов.Активность = ИСТИНА
        |
        |СГРУППИРОВАТЬ ПО
        ...
        |	ВТ_СуммыОплатПоСчету.СуммаДокументаОплаты КАК СуммаДокументаОплаты,
        |	ВТ_СуммыОплатПоСчету.СуммаСчетаНаОплату КАК СуммаСчетаНаОплату,
        |	ЕСТЬNULL(ВТ_ОплаченныеСуммыПоСчетам.СуммаОплат, 0) КАК ОплаченнаяСумма,
        |	ВТ_СуммыОплатПоСчету.СуммаСчетаНаОплату - ЕСТЬNULL(ВТ_ОплаченныеСуммыПоСчетам.СуммаОплат, 0) КАК Долг
        |ПОМЕСТИТЬ ВТ_СуммыОплатПоСчетуИДолг
        |ИЗ
        |	ВТ_СуммыОплатПоСчету_Общая КАК ВТ_СуммыОплатПоСчету
        |	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОплаченныеСуммыПоСчетам КАК ВТ_ОплаченныеСуммыПоСчетам
        |	ПО ВТ_СуммыОплатПоСчету.Документ = ВТ_ОплаченныеСуммыПоСчетам.Документ
        |		И ВТ_СуммыОплатПоСчету.Контрагент = ВТ_ОплаченныеСуммыПоСчетам.Контрагент
        |		И ВТ_СуммыОплатПоСчету.Документ.Дата > ВТ_ОплаченныеСуммыПоСчетам.Период
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |	ВТ_СуммыОплатПоСчету.Контрагент КАК Контрагент,
        |	ВТ_СуммыОплатПоСчету.СчетНаОплату КАК СчетНаОплату,
        |	ВТ_СуммыОплатПоСчету.Долг КАК Сумма,
        |	ВТ_СуммыОплатПоСчету.Долг КАК Долг,
        |	ЛОЖЬ КАК ЭтоАванс,
        ...
        |ИЗ
        |	ВТ_СуммыОплатПоСчетуИДолг КАК ВТ_СуммыОплатПоСчету
        |ГДЕ
        |	ВТ_СуммыОплатПоСчету.Долг <= ВТ_СуммыОплатПоСчету.СуммаДокументаОплаты
        |
        |ОБЪЕДИНИТЬ ВСЕ
        |
        |ВЫБРАТЬ
        ...
        |	ВТ_СуммыОплатПоСчету.СуммаДокументаОплаты - ВТ_СуммыОплатПоСчету.Долг,
        |	ВТ_СуммыОплатПоСчету.Долг,
        |	ИСТИНА,
        ...
        |// РЕЗУЛЬТАТ
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ВТ_Результат_Подготовка.Организация КАК Организация,
        |	ВТ_Результат_Подготовка.Документ КАК Документ,
        |	ВТ_Результат_Подготовка.Контрагент КАК Контрагент,
        |	ВТ_Результат_Подготовка.ДоговорКонтрагента КАК ДоговорКонтрагента,
        |	ВТ_Результат_Подготовка.СчетНаОплату КАК СчетНаОплату,
        |	ВТ_Результат_Подготовка.СтавкаНДС КАК СтавкаНДС,
        |	ВТ_Результат_Подготовка.ВидСтавкиНДС КАК ВидСтавкиНДС,
        |	ВТ_Результат_Подготовка.Номенклатура КАК Номенклатура,
        |	ВТ_Результат_Подготовка.ПризнакПредметаРасчета КАК ПризнакПредметаРасчета,
        |	ВТ_Результат_Подготовка.ПризнакСпособаРасчета КАК ПризнакСпособаРасчета,
        |	ВТ_Результат_Подготовка.ПредставлениеТовараУслуги КАК ПредставлениеТовараУслуги,
        |	ВТ_Результат_Подготовка.ЦенаСчетаНаОплату КАК Цена,
        |	ВТ_Результат_Подготовка.КоличествоСчетаНаОплату КАК Количество,
        |   ВТ_Результат_Подготовка.Сумма КАК Сумма,
        |   ВТ_Результат_Подготовка.СуммаПлатежа КАК ОбщаяСуммаПлатежа,
        |   ВТ_Результат_Подготовка.СуммаСчетаНаОплату КАК ОбшаяСуммаСчетаНаОплату,
        |   ВТ_Результат_Подготовка.СуммаНДС КАК ОбшаяСуммаНДСПлатежа,
        |   ВТ_Результат_Подготовка.Долг КАК Долг,
        |	ВТ_Результат_Подготовка.ЭтоАванс КАК ЭтоАванс
        |ИЗ
