...
Функция СобратьДанныеЧековСчетмаш(Знач ДатаНачала, Знач ДатаОкончания, Знач СтруктураОтбора) Экспорт
    ...
    КолнкиТаблицыОплат = "Организация, Документ, Контрагент, ЛицевойСчет, ВариантРаспределенияОплатКапРемонт";
    РезультатФункции.ТаблицаОплат = ТаблицаОплатСДопДанными.Скопировать(, КолнкиТаблицыОплат);
    РезультатФункции.ТаблицаОплат.Свернуть(КолнкиТаблицыОплат);
    РасшифровкаОплатДляЧеков = ПолучитьРасшифровкуОплатДляЧековСчетмаш(РезультатФункции.ТаблицаОплат, СтруктураПараметров);

    КолнкиТаблицыОплат = "Документ, Контрагент, ВариантРаспределенияОплатКапРемонт";
    ТаблицаОплатСДопДанными = ТаблицаОплатСДопДанными.Скопировать(, КолнкиТаблицыОплат + ", Сумма");
    РезультатФункции.ТаблицаОплат = ТаблицаОплатСДопДанными;
    РезультатФункции.ТаблицаОплат.Свернуть(КолнкиТаблицыОплат, "Сумма");
...
Функция ПолучитьТаблицуОплатСчетмаш(Знач ДатаНачала, Знач ДатаОкончания, Знач СтруктураОтбора) Экспорт
    ...
    ТаблицаРасшифровкиОплат = ПолучитьОбщуюТаблицуРасшифровкиОплатДляОФД(
            ДатаНачала, ДатаОкончания, Неопределено, КонтрагентОтбора);
...
Функция ПолучитьРасшифровкуОплатДляЧековСчетмаш(ТаблицаОплатСДопДанными, СтруктураПараметров) Экспорт
    ...
    //ТаблицаОплатСДопДаннымиИЛицевымиСчетами = ТаблицаОплатСДопДанными.СкопироватьКолонки();
...
// Возвращаемое значение:
//  - Строка
Функция СформироватьВнешнийИдентификаторЧекаСчетмаш(Знач ДанныеДокумента, Знач ДанныеКонтрагента) Экспорт
    ПредставлениеДатыДокумента = Формат(ДанныеДокумента.Дата, "ДФ=гг");
        // + Формат(ДанныеДокумента.Дата, "ДФ=ММ")
        // + Формат(ДанныеДокумента.Дата, "ДФ=дд");

    Возврат СтрШаблон("%2%1%3%1%4", "_", ПредставлениеДатыДокумента, ДанныеДокумента.Номер, ДанныеКонтрагента.Код);
КонецФункции
