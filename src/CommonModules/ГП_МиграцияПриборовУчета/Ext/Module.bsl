...
Функция ПолучитьЛицевыеСчетаОбъектовАбонентовДляСопоставления() Экспорт
    ...
    Запрос.Текст =
        ...
        |	ВТ_ГП_УслугиБП77.МетодРасчетаХВ КАК МетодРасчетаХВ,
        |	ВТ_ГП_УслугиБП77.МетодРасчетаГВ КАК МетодРасчетаГВ,
        |	ВТ_ГП_УслугиБП77.МетодРасчетаКан КАК МетодРасчетаКан,
        |	ВТ_ГП_УслугиБП77.ТолькоДляКанализации КАК ТолькоДляКанализации,
        |	ВТ_ГП_УслугиБП77.ПроцентХВ КАК ПроцентХВ,
        |	ВТ_ГП_УслугиБП77.ПроцентГВ КАК ПроцентГВ,
        |	ВТ_ГП_УслугиБП77.ОбъектАбонентаНаименование КАК ОбъектАбонентаНаименование
        |ИЗ
...
//      * ДатаПоверки - Дата
//      * ДатаУстановки - Дата
Функция ПолучитьТаблицуСчетовСчетчиковДляУстановки() Экспорт
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |	// ВТ_Счетчики.Счетчик.ДатаПервойПоверки КАК СчетчикДатаПервойПоверки,
        |	// ВТ_Счетчики.Счетчик.ГП_ДатаУстановки КАК СчетчикДатаУстановки,
...
Функция ПолучитьДанныеЗаполненияНазначенийУслугПоДаннымРегистраСчетчиковБП77(
    ...
    Запрос.Текст =
        ...
        |	РегистрСведений.ГП_СчетчикиБП77 КАК ГП_СчетчикиБП77
        |ГДЕ
        |	((ГП_СчетчикиБП77.МетодРасчетаХВ <> ""Нет"" И ГП_СчетчикиБП77.МетодРасчетаХВ <> """")
        |			ИЛИ (ГП_СчетчикиБП77.МетодРасчетаГВ <> ""Нет"" И ГП_СчетчикиБП77.МетодРасчетаГВ <> """")
        |			ИЛИ (ГП_СчетчикиБП77.МетодРасчетаКан <> ""Нет"" И ГП_СчетчикиБП77.МетодРасчетаКан <> """"))
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |	ВЫБОР
        |		КОГДА ВТ_ОбъектыАбонентовПодготовка.МетодРасчетаХВ <> ""Нет"" И ВТ_ОбъектыАбонентовПодготовка.МетодРасчетаХВ <> """"
        ...
        |	ВЫБОР
        |		КОГДА ВТ_ОбъектыАбонентовПодготовка.МетодРасчетаГВ <> ""Нет"" И ВТ_ОбъектыАбонентовПодготовка.МетодРасчетаГВ <> """"
        |			ТОГДА ИСТИНА
        |		ИНАЧЕ ЛОЖЬ
        |	КОНЕЦ КАК ЭтоГорячаяВода,
        |	ВЫБОР
        |		КОГДА ВТ_ОбъектыАбонентовПодготовка.МетодРасчетаКан <> ""Нет"" И ВТ_ОбъектыАбонентовПодготовка.МетодРасчетаГВ <> """"
        |			ТОГДА ИСТИНА
        |		ИНАЧЕ ЛОЖЬ
