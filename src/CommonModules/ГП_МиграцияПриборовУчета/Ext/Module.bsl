...
Функция СоздатьСчетчикиПоСреднемуМетодуРасчета(Знач ДатаАктуальности = Неопределено) Экспорт
    ...
    Попытка
        Для Каждого СтрокаДанных Из ДанныеДляЗаполнения Цикл
            ПараметрыЗаполнения = Новый Структура("Услуга, Наименование, Комментарий");
...
// Гарант+ Килипенко 03.10.2024 [F00229366] создание и установка счетчиков с методом по среднему ++
//
Функция ПолучитьДанныеДляСозданияСчетчиковПоСреднему(Знач ДатаАктуальности = Неопределено) Экспорт
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        ...
        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГП_СчетчикиБП77 КАК ГП_СчетчикиБП77
        |		ПО ВТ_ЗданияБП77.КонтрагентКод = ГП_СчетчикиБП77.КонтрагентКод
        |		И ВТ_ЗданияБП77.ОбъектАбонентаКод = ГП_СчетчикиБП77.ОбъектАбонентаКод
        |ГДЕ
        |	КВП_ЛицевыеСчета.Действует = ИСТИНА
        |	И ГП_СчетчикиБП77.СчетчикКод ЕСТЬ NULL
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
        ...
        |
        |ВЫБРАТЬ
        |	ВТ_ЛицевыеСчетаПоСреднемуГВСБезСчетчика.ЛицевойСчет КАК ЛицевойСчет,
        |	ВТ_ЛицевыеСчетаПоСреднемуГВСБезСчетчика.КонтрагентНаименование КАК КонтрагентНаименование,
        |	ВТ_ЛицевыеСчетаПоСреднемуГВСБезСчетчика.Услуга КАК Услуга,
        |	ВТ_ЛицевыеСчетаПоСреднемуГВСБезСчетчика.ИдентификаторЛС КАК ИдентификаторЛС
        |ИЗ
