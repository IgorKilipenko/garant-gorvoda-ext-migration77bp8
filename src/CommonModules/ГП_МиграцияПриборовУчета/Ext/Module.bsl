...
Функция ЭтоНаименованиеУслугиХолодногоВодоснабженияОИ(Знач НаименованиеОбъектаАбонента) Экспорт
    ШаблонПоиска =
        "(?i).*([Пп]?лата)\s+((за)|(по))\s+([хХ][оаОА]лодн)\s+(водосн.+бж[а-яА-Я]+)[\s,]+.*((ОИ)|(потребленное при содержании))\s*.*";
    РезультатФункции = СтрПодобнаПоРегулярномуВыражению(ШаблонПоиска, ШаблонПоиска);
    Возврат РезультатФункции;
КонецФункции
