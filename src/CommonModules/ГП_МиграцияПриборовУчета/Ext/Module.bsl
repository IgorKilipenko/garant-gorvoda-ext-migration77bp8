...
Функция ПолучитьСоответствиеКодовУслугСчетчиковБП77() Экспорт
    ...
    РезультатФункции.Вставить("ХолоднаяВода_Водоотвод", "00-005");
...
Функция ПолучитьДанныеЗаполненияНазначенийУслугПоДаннымРегистраСчетчиковБП77() Экспорт
    РезультатФункции = Новый Структура("Успех, ХолоднаяВода, ГорячаяВода, Канализация, ХолоднаяВодаВодоотвод", Истина);
    ...
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ    // Сбор данных объектов абонентов из регистра ГП_СчетчикиБП77
        |	ГП_СчетчикиБП77.ОбъектАбонентаКод КАК ОбъектАбонентаКод,
        |	ГП_СчетчикиБП77.КонтрагентКод КАК КонтрагентКод,
        |	ГП_СчетчикиБП77.КонтрагентНаименование КАК КонтрагентНаименование,
        |	ГП_СчетчикиБП77.ОбъектАбонентаКод + ""_"" + ГП_СчетчикиБП77.КонтрагентКод КАК СинтИдентификатор,
        |	ГП_СчетчикиБП77.ВидСчетчика КАК ВидСчетчика,
        |	ГП_СчетчикиБП77.ТолькоДляКанализации КАК ТолькоДляКанализации,
        |	ВЫБОР
        |		КОГДА ГП_СчетчикиБП77.ВидСчетчика = ""ХолоднаяВода""
        |			ТОГДА ГП_СчетчикиБП77.МетодРасчетаХВ
        |		КОГДА ГП_СчетчикиБП77.ВидСчетчика ПОДОБНО ""ГорячаяВода%""
        |			ТОГДА ГП_СчетчикиБП77.МетодРасчетаГВ
        |		КОГДА ГП_СчетчикиБП77.ВидСчетчика = ""Канализация""
        |			ТОГДА ГП_СчетчикиБП77.МетодРасчетаКан
        |		КОГДА ГП_СчетчикиБП77.ВидСчетчика = ""Счетчик ХВ (Подогрев)""
        |			ТОГДА ГП_СчетчикиБП77.МетодРасчетаХВ
        |		ИНАЧЕ """"
        |	КОНЕЦ КАК МетодРасчета
        |ПОМЕСТИТЬ ВТ_ГП_СчетчикиБП77Подготовка
        |ИЗ
        |	РегистрСведений.ГП_СчетчикиБП77 КАК ГП_СчетчикиБП77
        |ГДЕ
        |	ГП_СчетчикиБП77.ВидСчетчика <> """"
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |// Фильтрация данных полученных из регистра ГП_СчетчикиБП77
        |//
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	ГП_СчетчикиБП77.ОбъектАбонентаКод КАК ОбъектАбонентаКод,
        |	ГП_СчетчикиБП77.КонтрагентКод КАК КонтрагентКод,
        ...
        |	ГП_СчетчикиБП77.ОбъектАбонентаКод + ""_"" + ГП_СчетчикиБП77.КонтрагентКод КАК СинтИдентификатор,
        |	ГП_СчетчикиБП77.ВидСчетчика КАК ВидСчетчика,
        |	ГП_СчетчикиБП77.МетодРасчета КАК МетодРасчета,
        |	ГП_СчетчикиБП77.ТолькоДляКанализации КАК ТолькоДляКанализации
        |ПОМЕСТИТЬ ВТ_ГП_СчетчикиБП77
        |ИЗ
        |	ВТ_ГП_СчетчикиБП77Подготовка КАК ГП_СчетчикиБП77
        |ГДЕ
        |	ГП_СчетчикиБП77.МетодРасчета <> ""Нет""
        |	И ГП_СчетчикиБП77.МетодРасчета <> """"
        |
        |СГРУППИРОВАТЬ ПО
        |	ГП_СчетчикиБП77.КонтрагентКод,
        |	ГП_СчетчикиБП77.ОбъектАбонентаКод,
        |	ГП_СчетчикиБП77.КонтрагентНаименование,
        |	ГП_СчетчикиБП77.ВидСчетчика,
        |	ГП_СчетчикиБП77.МетодРасчета,
        |	ГП_СчетчикиБП77.ТолькоДляКанализации
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |	И КВП_ЛицевыеСчета.Идентификатор ПОДОБНО ""%_%_____%""
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |// Соединение данных объектов абонентов и лицевых счетов
        |ВЫБРАТЬ
        |	ВТ_ГП_СчетчикиБП77.ОбъектАбонентаКод КАК ОбъектАбонентаКод,
        |	ВТ_ГП_СчетчикиБП77.КонтрагентКод КАК КонтрагентКод,
        |	ВТ_ГП_СчетчикиБП77.КонтрагентНаименование КАК КонтрагентНаименование,
        |	ВТ_ГП_СчетчикиБП77.СинтИдентификатор КАК СинтИдентификатор,
        |	ВТ_ГП_СчетчикиБП77.ВидСчетчика КАК ВидСчетчика,
        |	ВТ_ГП_СчетчикиБП77.МетодРасчета КАК МетодРасчета,
        |	ВТ_ГП_СчетчикиБП77.ТолькоДляКанализации КАК ТолькоДляКанализации,
        |	ВТ_КВП_ЛицевыеСчета.Ссылка КАК Ссылка,
        |	ВТ_КВП_ЛицевыеСчета.ОтветственныйСобственникПредставление КАК ОтветственныйСобственникПредставление,
        |	ВТ_КВП_ЛицевыеСчета.Наименование КАК Наименование,
        |	ВТ_КВП_ЛицевыеСчета.Идентификатор КАК Идентификатор
        |ПОМЕСТИТЬ ВТ_ВидыСчетчиковЛицевыхСчетов
        |ИЗ
        |	ВТ_ГП_СчетчикиБП77 КАК ВТ_ГП_СчетчикиБП77
        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_КВП_ЛицевыеСчета КАК ВТ_КВП_ЛицевыеСчета
        |		ПО ВТ_ГП_СчетчикиБП77.СинтИдентификатор = ВТ_КВП_ЛицевыеСчета.Идентификатор
        |;";
    ...
    Запрос.Текст =
        ...
        |ВЫБРАТЬ
        |	ВТ_ВидыСчетчиковЛицевыхСчетов.Ссылка КАК ЛицевойСчет,
        |	ВТ_ВидыСчетчиковЛицевыхСчетов.ОбъектАбонентаКод КАК ОбъектАбонентаКод,
        |	ВТ_ВидыСчетчиковЛицевыхСчетов.КонтрагентКод КАК КонтрагентКод,
        |	ВТ_ВидыСчетчиковЛицевыхСчетов.МетодРасчета КАК МетодРасчета
        |ИЗ
        |	ВТ_ВидыСчетчиковЛицевыхСчетов КАК ВТ_ВидыСчетчиковЛицевыхСчетов
        |ГДЕ
        |	ВТ_ВидыСчетчиковЛицевыхСчетов.ВидСчетчика = ""ХолоднаяВода""
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |	ВТ_ВидыСчетчиковЛицевыхСчетов КАК ВТ_ВидыСчетчиковЛицевыхСчетов
        |ГДЕ
        |	ВТ_ВидыСчетчиковЛицевыхСчетов.ВидСчетчика ПОДОБНО ""ГорячаяВода%""
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |	ВТ_ВидыСчетчиковЛицевыхСчетов КАК ВТ_ВидыСчетчиковЛицевыхСчетов
        |ГДЕ
        |	ВТ_ВидыСчетчиковЛицевыхСчетов.ВидСчетчика = ""Канализация""
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        ...
        |ГДЕ
        |	ВТ_ВидыСчетчиковЛицевыхСчетов.ВидСчетчика = ""ХолоднаяВода""
        |	И ВТ_ВидыСчетчиковЛицевыхСчетов.ТолькоДляКанализации = ИСТИНА
        |;";
...
Функция ПолучитьСведенияДляВзаиморасчетовПоЛицевомуСчету(Знач ЛицевойСчетСсылка, Знач ОрганизацияСсылка = Неопределено, Знач ДатаАктуальности = Неопределено)
    Запрос = Новый Запрос();
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
