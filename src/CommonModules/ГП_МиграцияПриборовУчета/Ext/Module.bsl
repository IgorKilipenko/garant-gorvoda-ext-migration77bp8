...
Функция СоздатьВиртуальныеСчетчикиПоСреднемуДляЛСБезСчетчиков(Знач ДатаАктуальности = Неопределено) Экспорт
    ...
    // Дополнение данных
    ДанныеДляСоздания.Колонки.Добавить("ТолькоДляКанализации", Новый ОписаниеТипов("Булево"));
    ДанныеДляСоздания.Колонки.Добавить("КоличествоПоСреднему", Новый ОписаниеТипов("Число"));
    ...
    Попытка
        Для Каждого СтрокаДанных Из ДанныеДляСоздания Цикл
            ...
            Если ГП_МиграцияОбщегоНазначения.ЭтоНаименованиеУслугиХолодногоВодоснабженияОИ(СтрокаДанных.Наименование) Тогда
                Продолжить;
            КонецЕсли;

            // Игнорирование для объектов Негативное воздействие
            Если ГП_МиграцияОбщегоНазначения.ЭтоНаименованиеУслугиНегативногоВоздействия(СтрокаДанных.Наименование) Тогда
                Продолжить;
            КонецЕсли;

            // Игнорирование для объектов Потри воды
            Если ГП_МиграцияОбщегоНазначения.ЭтоНаименованиеПотериВоды(СтрокаДанных.Наименование) Тогда
                Продолжить;
            КонецЕсли;
            ...
            ИдентификаторЛС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДанных.ЛицевойСчет, "Идентификатор");
            Если ПустаяСтрока(ИдентификаторЛС) = Ложь Тогда
                СчетчикОбъект.ГП_Комментарий = СтрШаблон("(ид: (%1))", ИдентификаторЛС);
            КонецЕсли;
