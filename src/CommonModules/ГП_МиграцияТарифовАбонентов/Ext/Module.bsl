...
// Параметры:
//  ДанныеОбъектов - ТаблицаЗначений
...
// Возвращаемое значение:
//  - ДокументСсылка.КВП_УстановкаТарифов
Функция СоздатьДокументУстановкиТарифов(
        Знач ДанныеОбъектов, Знач Услуга, Знач ВидТарифа, Знач Тариф, Знач ДатаДокумента = Неопределено, Знач Организация = Неопределено) Экспорт

    #Область Отладка
    ГП_ДиагностикаКлиентСервер.Утверждение(ТипЗнч(Услуга) = Тип("СправочникСсылка.КВП_Услуги"), "Ошибка типа аргумента Услуга");
    ГП_ДиагностикаКлиентСервер.Утверждение(ТипЗнч(ВидТарифа) = Тип("СправочникСсылка.lc_ВидыТарифов"), "Ошибка типа аргумента ВидТарифа");
    #КонецОбласти // Отладка

    РезультатФункции = Новый Структура(
            "Успех, ТекстСообщения, СозданныйДокумент", Истина);

    Организация = ?(Организация = Неопределено,
            УПЖКХ_ТиповыеМетодыВызовСервера.ПолучитьОсновнуюОрганизацию(), Организация);
    ДатаДокумента = ?(ДатаДокумента = Неопределено, ТекущаяДатаСеанса(), ДатаДокумента); // Константа по условиям ТЗ

    НовыйДокументОбъект = НовыйДокументУстановкиТарифов(ДанныеОбъектов, Услуга, ВидТарифа, Тариф, ДатаДокумента, Организация);

    // Записываем документ
    Попытка
        НовыйДокументОбъект.Записать(РежимЗаписиДокумента.Запись);
        РезультатФункции.СозданныйДокумент = НовыйДокументОбъект.Ссылка;
    Исключение
        ОбщаяЧастьСообщения = "Ошибка при записи документа Установка тарифов.";
        СтруктураОшибки = ГП_МиграцияОбщегоНазначения.ЗаписатьОшибкуВЖурнал(
                ОбщаяЧастьСообщения, ИнформацияОбОшибке(), Ложь, Истина);
...
//  Организация - СправочникСсылка.Организации
// Возвращаемое значение:
//  - ДокументОбъект.КВП_УстановкаТарифов
Функция НовыйДокументУстановкиТарифов(
        Знач ДанныеОбъектов, Знач Услуга, Знач ВидТарифа, Знач Тариф, Знач ДатаДокумента = Неопределено, Знач Организация = Неопределено) Экспорт
    ...
    НовыйДокументОбъект.Комментарий = СтрШаблон(
            "#создан автоматически для вида услуги: ""%1"", для вида тарифа: ""%2"" БП 7.7. (ГарантПлюс)",
            Строка(Услуга), Строка(ВидТарифа));
    НовыйДокументОбъект.Услуга = Услуга;
    НовыйДокументОбъект.ГП_ВидТарифаПотребителя = ВидТарифа;

    // Добавление тарифов (ТЧ СписокУслуг) Дневной / Общий
    Для Каждого ВидТарифаУстановки Из ВидыТарифовДляУстановки Цикл
        НоваяСтрокаТарифа = НовыйДокументОбъект.СписокУслуг.Добавить();
        НоваяСтрокаТарифа.ВидТарифа = ВидТарифаУстановки;
        НоваяСтрокаТарифа.ИспользоватьФормулу = Ложь;
        НоваяСтрокаТарифа.Тариф = Тариф;
    КонецЦикла;

    // Заполнение списка объектов (зданий, помещений ТЧ СписокОбъектов)
    Для Каждого ДанныеДляЗаполненияСпискаОбъектов Из ДанныеОбъектов Цикл
        НоваяСтрокаОбъекты = НовыйДокументОбъект.СписокОбъектов.Добавить();
        НоваяСтрокаОбъекты.Здание = ДанныеДляЗаполненияСпискаОбъектов.Здание;
        НоваяСтрокаОбъекты.Помещение = ДанныеДляЗаполненияСпискаОбъектов.Помещение;
    КонецЦикла;
