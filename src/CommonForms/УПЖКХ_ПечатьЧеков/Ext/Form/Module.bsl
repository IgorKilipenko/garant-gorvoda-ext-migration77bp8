...
Процедура ГП_ПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
    ...
    Если ЭтотОбъект.ГП_ИспользуетсяФормированиеЧековПоКонтрагентуДокумента
            И ГП_ПолучитьТипДокументаИзПараметровНаСервере(ЭтотОбъект.Параметры) <> Тип("ДокументСсылка.КВП_РегистрацияОплаты") Тогда

        УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьОбОшибке(
            "В текущей реализации механизм распределения по контрагенту доступен только для документов: ""Регистрация оплаты""", Отказ);
		Возврат;
    КонецЕсли;
КонецПроцедуры
...
Функция ГП_СоздатьЭлементТаблицаОплатКонтрагентНаСервере()
    ...
    ИмяСоздаваемогоЭлемента = СтрШаблон("%1%2", НаименованиеСоздаваемогоРеквизита, ИмяЭлементаТаблицыОплат);
...
Функция ГП_ИспользуетсяФормированиеЧековПоКонтрагентуДокументаНаСервере()
    Возврат Истина // ЭтотОбъект.ТипДокументаОплаты = Тип("ДокументСсылка.КВП_РегистрацияОплаты")
        И Документы.КВП_РегистрацияОплаты.ГП_ИспользуетсяФормированиеЧековПоКонтрагентуДокумента(Неопределено);
КонецФункции
