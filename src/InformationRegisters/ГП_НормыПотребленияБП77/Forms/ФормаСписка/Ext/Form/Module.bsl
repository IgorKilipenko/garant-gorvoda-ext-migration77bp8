// Гарант+ Килипенко 14.08.2024 [F00227047] перенос норм потребления из БП77 ++
#Область ОбработчикиКомандФормы

// Выполняет создание норм потребления холодной воды по виду тарифа "для населения"
&НаКлиенте
Асинх Процедура СоздатьНормыПотребленияХолоднойВодыДляНаселения(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет выполнена процедура создания норм потребления холодной воды (для населения).");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    // Создание объектов
    РезультатСоздания = СоздатьНормыПотребленияХолоднойВодыДляНаселенияНаСервере();

    // Вывод сообщения о результате
    ТекстСообщенияРезультатОперации = "";
    Если РезультатСоздания.Успех = Ложь Тогда
        ТекстСообщенияРезультатОперации = СтрШаблон("Создание норм потребления не выполнено.
                |Сообщение о причине: %1", РезультатСоздания.ТекстСообщения);
    Иначе
        ТекстСообщенияРезультатОперации = СтрШаблон("Создание норм потребления выполнено успешно.
                |Создано ""%1"" норм потребления услуг.", РезультатСоздания.СозданныеНормыПотребления.Количество());
    КонецЕсли;
    ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияРезультатОперации);
КонецПроцедуры

// Выполняет создание норм потребления горячей воды по виду тарифа "для населения"
&НаКлиенте
Асинх Процедура СоздатьНормыПотребленияГорячейВодыДляНаселения(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет выполнена процедура создания норм потребления горячей воды (для населения).");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    // Создание объектов
    РезультатСоздания = СоздатьНормыПотребленияГорячейВодыДляНаселенияНаСервере();

    // Вывод сообщения о результате
    ТекстСообщенияРезультатОперации = "";
    Если РезультатСоздания.Успех = Ложь Тогда
        ТекстСообщенияРезультатОперации = СтрШаблон("Создание норм потребления не выполнено.
                |Сообщение о причине: %1", РезультатСоздания.ТекстСообщения);
    Иначе
        ТекстСообщенияРезультатОперации = СтрШаблон("Создание норм потребления выполнено успешно.
                |Создано ""%1"" норм потребления услуг.", РезультатСоздания.СозданныеНормыПотребления.Количество());
    КонецЕсли;
    ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияРезультатОперации);
КонецПроцедуры

// Гарант+ Килипенко 15.08.2024 [F00227106] установка норм потребления услуг на л/с ++
//
// Выполняет запись норм потребления услуг для л/с в регистр КВП_НормыПотребленияУслугЛС
&НаКлиенте
Асинх Процедура ВыполнитьНазначениеНормПотребленияНаЛицевыеСчета(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет выполнена процедура назначения норм потребления услуг на л/с (записи в регистр ""КВП_НормыПотребленияУслугЛС"").");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    ПериодНазначенияУслуг = Дата(2024, 01, 01); // !!! Задано по ТЗ

    // Назначение норм потреблений (запись в регистр КВП_НормыПотребленияУслугЛС)
    РезультатСоздания = ВыполнитьНазначениеНормПотребленияНаЛицевыеСчетаНаСервере(ПериодНазначенияУслуг);

    // Вывод сообщения о результате
    ТекстСообщенияРезультатОперации = "";
    Если РезультатСоздания.Успех = Ложь Тогда
        ТекстСообщенияРезультатОперации = СтрШаблон("Назначение норм потребления услуг на л/с не выполнено.
                |Сообщение о причине: %1", РезультатСоздания.ТекстСообщения);
    Иначе
        ТекстСообщенияРезультатОперации = СтрШаблон("Назначение норм потребления услуг на л/с выполнено успешно.
                |Назначено ""%1"" норм потребления услуг на л/с.", РезультатСоздания.КоличествоЗаписей);
    КонецЕсли;
    ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияРезультатОперации);
КонецПроцедуры // Гарант+ Килипенко 15.08.2024 [F00227106] установка норм потребления услуг на л/с --

#КонецОбласти // ОбработчикиКомандФормы
// Гарант+ Килипенко 14.08.2024 [F00227047] перенос норм потребления из БП77 --

// Гарант+ Килипенко 14.08.2024 [F00227047] перенос норм потребления из БП77 ++
#Область СлужебныеПроцедурыИФункции

// Создает нормы потребления холодной воды по виду тарифа "для населения"
// Возвращаемое значение:
//  - Структура
//      * Успех - Булево
//      * СозданныеНормыПотребления - Массив из СправочникСсылка.БанковскиеСчета
//      * ТекстСообщения - Строка, Неопределено
&НаСервереБезКонтекста
Функция СоздатьНормыПотребленияХолоднойВодыДляНаселенияНаСервере()
    Возврат ГП_МиграцияНормПотребления.СоздатьНормыПотребленияХолоднойВодыДляНаселения();
КонецФункции

// Создает нормы потребления горячей воды по виду тарифа "для населения"
// Возвращаемое значение:
//  - Структура
//      * Успех - Булево
//      * СозданныеНормыПотребления - Массив из СправочникСсылка.БанковскиеСчета
//      * ТекстСообщения - Строка, Неопределено
&НаСервереБезКонтекста
Функция СоздатьНормыПотребленияГорячейВодыДляНаселенияНаСервере()
    Возврат ГП_МиграцияНормПотребления.СоздатьНормыПотребленияГорячейВодыДляНаселения();
КонецФункции

// Гарант+ Килипенко 15.08.2024 [F00227106] установка норм потребления услуг на л/с ++
//
// Выполняет запись норм потребления услуг для л/с в регистр КВП_НормыПотребленияУслугЛС
// Параметры:
//  ПериодНазначения - Дата - Период назначения (начало месяца)
// Возвращаемое значение:
//  - Структура
//      * Успех - Булево
//      * КоличествоЗаписей - Число
//      * ТекстСообщения - Строка, Неопределено
&НаСервереБезКонтекста
Функция ВыполнитьНазначениеНормПотребленияНаЛицевыеСчетаНаСервере(Знач ПериодНазначения)
    Возврат ГП_МиграцияНормПотребления.ВыполнитьНазначениеНормПотребленияНаЛицевыеСчета(ПериодНазначения);
КонецФункции // // Гарант+ Килипенко 15.08.2024 [F00227106] установка норм потребления услуг на л/с ++

#КонецОбласти // СлужебныеПроцедурыИФункции
// Гарант+ Килипенко 14.08.2024 [F00227047] перенос норм потребления из БП77 --
