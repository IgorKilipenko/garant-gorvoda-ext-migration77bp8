...
&НаКлиенте
Асинх Процедура СоздатьОтсутствующиеЛицевыеСчета(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет выполнена операция создания лицевых счетов"
            + " для зданий (объектов абонентов) для которы л/с отсутствуют в ИБ.");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    // Создание лицевых счетов
    РезультатОбновления = СоздатьОтсутствующиеЛицевыеСчетаНаСервере();

    // Вывод сообщения о результате
    ТекстСообщенияРезультатОперации = "";
    Если РезультатОбновления.Успех = Ложь Тогда
        ТекстСообщенияРезультатОперации = СтрШаблон("Создание лицевых счетов не выполнено.
                |Сообщение о причине: %1", РезультатОбновления.ТекстСообщения);
    Иначе
        ТекстСообщенияРезультатОперации = СтрШаблон("Создание лицевых счетов выполнено успешно.
                |Создано ""%1"" лицевых счетов.", РезультатОбновления.СозданныеЛицевыеСчета.Количество());
    КонецЕсли;
    ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияРезультатОперации);
...
// Гарант+ Килипенко 05.08.2024 [F00226996] перенос лицевых счетов из БП77 ++
//
// Выполняет создание документов открытия лицевого счета для неоткрытых л/с (только для л/с созданных автоматически)
&НаКлиенте
Асинх Процедура ОткрытьСозданныеЛицевыеСчета(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет выполнена операция создания документов открытия лицевых счетов"
            + " для лицевых счетов созданных автоматически по данным БП 7.7 (только для неоткрытых л/с).");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    // Создание документов открытия лицевых счетов
    ДатаОткрытияПоУмолчанию = Дата(2024, 01, 01);
    РезультатОбновления = ОткрытьСозданныеЛицевыеСчетаНаСервере(ДатаОткрытияПоУмолчанию, Истина);

    // Вывод сообщения о результате
    ТекстСообщенияРезультатОперации = "";
    Если РезультатОбновления.Успех = Ложь Тогда
        ТекстСообщенияРезультатОперации = СтрШаблон("Создание документов открытия лицевых счетов не выполнено.
                |Сообщение о причине: %1", РезультатОбновления.ТекстСообщения);
    Иначе
        ТекстСообщенияРезультатОперации = СтрШаблон("Создание документов открытия лицевых счетов выполнено успешно.
                |Создано ""%1"" лицевых счетов.", РезультатОбновления.СозданныеДокументыОткрытияЛицевогоСчета.Количество());
    КонецЕсли;
    ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияРезультатОперации);
КонецПроцедуры // Гарант+ Килипенко 05.08.2024 [F00226996] перенос лицевых счетов из БП77 --
...
//
// Устарела. Функция используется только в процессе разработки
//
// Выполняет исправление идентификаторов лицевых счетов
&НаКлиенте
Асинх Процедура ИсправитьИдентификаторыПеренесенныхЛицевыхСчетов(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет выполнена операция исправления идентификаторов лицевых счетов.");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    // Исправление идентификаторов л/с
    РезультатИсправления = ИсправитьИдентификаторыПеренесенныхЛицевыхСчетовНаСервере();

    // Вывод сообщения о результате
    ТекстСообщенияРезультатОперации = "";
    Если РезультатИсправления.Успех = Ложь Тогда
        ТекстСообщенияРезультатОперации = СтрШаблон("Исправление идентификаторов л/с не выполнено.
                |Сообщение о причине: %1", РезультатИсправления.ТекстСообщения);
    Иначе
        ТекстСообщенияРезультатОперации = СтрШаблон("Исправление идентификаторов л/с выполнено успешно.
                |Исправлено ""%1"" идентификаторов лицевых счетов.",
                РезультатИсправления.КоличествоОбновленныхИдентификаторов);
    КонецЕсли;
    ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияРезультатОперации);
...
//
// Выполняет исправление (записывает) поле Контрагент в созданных автоматически (ГарантПлюс) документах открытия лицевых счетов
&НаКлиенте
Асинх Процедура ИсправитьКонтрагентовВСозданныхАвтоматическиДокументахОткрытияЛС(Команда)
    // Предупреждение пользователю о необратимости операции
    РезультатДиалога = Ждать ГП_ОбщегоНазначенияКлиент.СпроситьПользователяОГотовностиВыполнятьОперациюАсинх(
            "Будет выполнена операция исправления значений контрагентов для документов открытия лицевых счетов.");
    Если РезультатДиалога = Ложь Тогда
        Возврат; // Пользователь отказался от выполнения операции
    КонецЕсли;

    // Исправление
    РезультатИсправления = ИсправитьКонтрагентовВСозданныхАвтоматическиДокументахОткрытияЛСНаСервере();

    // Вывод сообщения о результате
    ТекстСообщенияРезультатОперации = "";
    Если РезультатИсправления.Успех = Ложь Тогда
        ТекстСообщенияРезультатОперации = СтрШаблон("Исправление документов открытия л/с не выполнено.
                |Сообщение о причине: %1", РезультатИсправления.ТекстСообщения);
    Иначе
        ТекстСообщенияРезультатОперации = СтрШаблон("Исправление документов открытия л/с выполнено успешно.
                |Исправлено ""%1"" документов открытия л/с.",
                РезультатИсправления.КоличествоОбновленных);
    КонецЕсли;
    ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияРезультатОперации);
КонецПроцедуры // Гарант+ Килипенко 08.08.2024 [F00227208] перенос контрагентов из БП77 --
...
//      * ТекстСообщения - Строка, Неопределено
&НаСервереБезКонтекста
Функция СоздатьОтсутствующиеЛицевыеСчетаНаСервере()
    Возврат ГП_МиграцияЛицевыхСчетов.СоздатьЛицевыеСчетаДляПеренесенныхЗданийБП77();
КонецФункции
...
//      * ТекстСообщения - Строка, Неопределено
&НаСервереБезКонтекста
Функция ОткрытьСозданныеЛицевыеСчетаНаСервере(Знач ДатаОткрытия, Знач ТолькоСозданныеАвтоматически)
    Возврат ГП_МиграцияЛицевыхСчетов.СоздатьДокументыОткрытияЛицевыхСчетов(ДатаОткрытия, ТолькоСозданныеАвтоматически);
...
//      * ТекстСообщения - Строка, Неопределено
&НаСервереБезКонтекста
Функция ИсправитьИдентификаторыПеренесенныхЛицевыхСчетовНаСервере()
    Возврат ГП_МиграцияЛицевыхСчетов.ИсправитьИдентификаторыПеренесенныхЛицевыхСчетов();
КонецФункции
...
//      * ТекстСообщения - Строка, Неопределено
&НаСервереБезКонтекста
Функция ИсправитьКонтрагентовВСозданныхАвтоматическиДокументахОткрытияЛСНаСервере()
    Возврат ГП_МиграцияЛицевыхСчетов.ИсправитьКонтрагентовВСозданныхАвтоматическиДокументахОткрытияЛС(РежимЗаписиДокумента.Запись);
КонецФункции
