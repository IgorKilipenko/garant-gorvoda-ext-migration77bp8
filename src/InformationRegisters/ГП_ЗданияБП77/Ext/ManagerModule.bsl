...
Функция ЗаполнитьНомераДоговоров() Экспорт
    ...
    Исключение
        СтруктураОшибки =
            ГП_МиграцияОбщегоНазначения.УдалитьЗаписатьОшибкуВЖурнал(
...
//      * Успех - Булево
//      * КоличествоИзмененныхСтрок - Число
Функция ЗаполнитьАдресаЗданий() Экспорт
    РезультатФункции = Новый Структура("Успех, КоличествоИзмененныхСтрок, СообщениеОбОшибке", Истина, 0);
    ...
    Для Каждого ТекущаяСтрока Из НаборВсехЗаписейРегистра Цикл
        ТекущееЗначениеДома = ТекущаяСтрока.Дом;
        ТекущееЗначениеУлицы = ТекущаяСтрока.Улица2;
        СтруктураАдреса = ГП_МиграцияОбщегоНазначения.ПолучитьАдресПоНаименованиюОбъектаАбонента(
                ТекущаяСтрока.ОбъектАбонентаНаименование);

        Если СтруктураАдреса.Успех = Ложь Тогда
            ТекущаяСтрока.Дом = "";
            ТекущаяСтрока.Улица2 = "";
        Иначе
            ТекущаяСтрока.Дом = СтруктураАдреса.Дом;
            ТекущаяСтрока.Улица2 = СтруктураАдреса.Улица;
        КонецЕсли;

        Если ТекущееЗначениеДома <> ТекущаяСтрока.Дом ИЛИ ТекущееЗначениеУлицы <> ТекущаяСтрока.Улица2 Тогда
            РезультатФункции.КоличествоИзмененныхСтрок = РезультатФункции.КоличествоИзмененныхСтрок + 1;
        КонецЕсли;
    КонецЦикла;
    ...
    Исключение
        СтруктураОшибки =
            ГП_МиграцияОбщегоНазначения.УдалитьЗаписатьОшибкуВЖурнал(
                "Ошибка при записи в регистр сведений ""Услуги БП 7.7""
                |при обновлении значений полей Адрес здания.", Ложь, Истина);
