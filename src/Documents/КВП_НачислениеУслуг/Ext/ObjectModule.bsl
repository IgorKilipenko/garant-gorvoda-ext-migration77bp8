...
Процедура ГП_ОбработкаПроведения1(Отказ, РежимПроведения)
    Если Отказ Тогда
        Возврат;
    КонецЕсли;
    
    КоличестоЭлементов = Мин(ЭтотОбъект.Движения.КВП_НачислениеДляОтраженияВРеглУчете.Количество(), 
        ЭтотОбъект.Движения.УПЖКХ_Начисления.Количество());
    Для Сч = 0 По КоличестоЭлементов - 1 Цикл
        СтрокаНачислений = ЭтотОбъект.Движения.УПЖКХ_Начисления[Сч];
        СтрокаОтражений = ЭтотОбъект.Движения.КВП_НачислениеДляОтраженияВРеглУчете[Сч];
        
        ЭтоСовпадающаяАналитика = СтрокаНачислений.Договор = СтрокаОтражений.ДоговорКонтрагента;
        ЭтоСовпадающаяАналитика = ЭтоСовпадающаяАналитика И СтрокаНачислений.Услуга.Услуга = СтрокаОтражений.Номенклатура;
        ЭтоСовпадающаяАналитика = ЭтоСовпадающаяАналитика И СтрокаНачислений.Тариф = СтрокаОтражений.Тариф;
        ЭтоСовпадающаяАналитика = ЭтоСовпадающаяАналитика И СтрокаНачислений.Количество = СтрокаОтражений.Количество;
        ЭтоСовпадающаяАналитика = ЭтоСовпадающаяАналитика И СтрокаНачислений.СуммаНачисления = СтрокаОтражений.Сумма;
        
        Если ЭтоСовпадающаяАналитика = Истина Тогда
            СтрокаОтражений.ГП_ЛицевойСчет = СтрокаНачислений.ЛицевойСчет;
        Иначе
            ЕстьОшибкиИдентификацииСток = Истина;
            Прервать;
        КонецЕсли;
    КонецЦикла;
    
    Если ЕстьОшибкиИдентификацииСток = Истина Тогда
        ОбщегоНазначения.СообщитьПользователю("Ошибка при заполнении лицевых счетов для отражения в рег. учете");
        Отказ = Истина;
    КонецЕсли;
