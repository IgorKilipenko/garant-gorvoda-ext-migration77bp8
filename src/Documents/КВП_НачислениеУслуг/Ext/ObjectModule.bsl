...
Процедура ГП_ДополнитьТаблицуУслугПУЗависимымиУслугами(ТаблицаПоУслугам, ПоПрочимОрганизациям)
	...
	Если НЕ мЗависимыеУслугиПоСобственномуНормативу Тогда
        ...
        КэшУслугаКанализация = ГП_РаботаСУслугами.ПолучитьУслугуКанализации().Ссылка;

        // Поиск строк без показаний для исключения из расчетов
        ТаблицыРедактирования = Новый Массив;
        ТаблицыРедактирования.Добавить(ТаблицаПоУслугам);
        ТаблицыРедактирования.Добавить(ТаблицаПоказаний);
        Для каждого ТекущаяТаблицаРедактирования Из ТаблицыРедактирования Цикл
            СтрокиПоказанийДляИсключения = ТекущаяТаблицаРедактирования.НайтиСтроки(Новый Структура(
                "Услуга, СостояниеПУ, ПоказаниеСчЛС, ТипРасчета, Зависимая, ЗависимаяПоНормативу",
                КэшУслугаКанализация,
                Перечисления.УПЖКХ_СостоянияПриборовУчета.НетИПУ, 0, 4, Ложь, Ложь));

            Для Каждого СтрокаДляУдаления Из СтрокиПоказанийДляИсключения Цикл
                ТекущаяТаблицаРедактирования.Удалить(СтрокаДляУдаления);
            КонецЦикла;
        КонецЦикла;
		...
		Для Каждого СтрокаЗависимойУслуги ИЗ ТаблицаЗависимыхУслуг Цикл
            ...
            Если СтрокаЗависимойУслуги.УслугаОснование = КэшУслугаКанализация
                ...
                // СтрокиТаблицыПоказаний = Новый Массив;
...
Процедура ГП_ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    ГП_РаботаСНачислениемУслуг.ПересчитатьСуммыДвиженийНачисленийИВзаиморасчетовСУчетомНДС(
        ЭтотОбъект.Движения.УПЖКХ_Начисления, Ложь, Перечисления.СтавкиНДС.НДС20_120);

    // Перерасчет сумм движений начислений с учетом НДС
    ГП_РаботаСНачислениемУслуг.ПересчитатьСуммыДвиженийНачисленийИВзаиморасчетовСУчетомНДС(
        ЭтотОбъект.Движения.КВП_ВзаиморасчетыПоЛицевымСчетам, Ложь, Перечисления.СтавкиНДС.НДС20_120);
