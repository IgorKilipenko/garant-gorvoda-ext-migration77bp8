...
&НаСервере
Функция ГП_ПолучитьДанныеЗаполненияДокументаОтраженияНаСервере(Знач ЗаполнятьКомпенсациюЛьгот = Ложь)
    ...
    РезультатФункции.Вставить("Дата", ?(ЗначениеЗаполнено(ЭтотОбъект.Объект.Дата),
            КонецМесяца(ЭтотОбъект.Объект.Дата), КонецМесяца(ТекущаяДатаСеанса())));
