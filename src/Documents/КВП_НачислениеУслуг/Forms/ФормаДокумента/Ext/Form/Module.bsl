...
Функция ГП_ЗаполнитьНаОснованииВводаПоказанийНаПУНаСервере()
    ...
    Если ЭтотОбъект.Параметры.Свойство("ГП_СписокУслуг") Тогда
        ЭтотОбъект.Объект.ГП_УслугиВводаПоказаний.Очистить();

        Для каждого СтрокаУслугПоказаний Из ЭтотОбъект.Параметры.ГП_СписокУслуг Цикл
            НоваяСтрокаУслугПоказаний = ЭтотОбъект.Объект.ГП_УслугиВводаПоказаний.Добавить();
            НоваяСтрокаУслугПоказаний.Услуга = СтрокаУслугПоказаний.Значение;
        КонецЦикла;
    КонецЕсли;
