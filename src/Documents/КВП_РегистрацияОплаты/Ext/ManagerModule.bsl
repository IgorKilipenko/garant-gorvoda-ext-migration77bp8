...
Функция ГП_ПолучитьРасшифровкуОплатДляЧеков(ТаблицаОплатСДопДанными, СтруктураПараметров)
	...
	Для Каждого ТекущаяСтрока Из РасшифровкаОплатДляЧеков Цикл
        ...
        Если Не ТекущаяСтрока.СтавкаНДС = Неопределено Тогда
			СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекущаяСтрока.Сумма, Истина, ТекущаяСтрока.СтавкаНДС);
            #Вставка // Гарант+ Килипенко 10.04.2025 [F00235125] Печать чеков по контрагенту документа ++ {

            // !!! Исправление расчета ставки НДС (в штатной логике расчет: 120% вместо 20%)
            Если ПрименяютсяРасчетныеСтавкиНДС Тогда
                СтавкаНДСДляРасчета = Неопределено;
                Если СтавкаНДССУчетомУСН <> Неопределено Тогда
            		СтавкаНДСДляРасчета = СтавкаНДССУчетомУСН;
            	Иначе
            		СтавкаНДСДляРасчета = Перечисления.СтавкиНДС.СтавкаНДС(ТекущаяСтрока.ВидСтавкиНДС, ТекущаяДата);
            	КонецЕсли;
                СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекущаяСтрока.Сумма, Истина,
                    УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтавкаНДСДляРасчета));
            КонецЕсли;

            #КонецВставки // Гарант+ Килипенко 10.04.2025 [F00235125] Печать чеков по контрагенту документа -- }
			ТекущаяСтрока.СуммаНДС = Окр(СуммаНДС, 2);
		КонецЕсли;
	...
	Если Не СтруктураПараметров.ПечататьЧекиСРасшифровкойПоУслугам Тогда
		КолонкиГруппировки = "Документ, ЛицевойСчет, ЭтоПени,"
								+ " ПризнакПредметаРасчета, ПризнакСпособаРасчета, СтавкаНДС, СуммаНДС";
        #Вставка // Гарант+ Килипенко 24.02.2025 [F00235125] Печать чеков по контрагенту документа ++
