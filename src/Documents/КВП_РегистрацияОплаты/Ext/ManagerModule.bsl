...
Функция ГП_ПолучитьДопДанныеПоОплатам(ТаблицаОплат)
    ...
    //|		КОГДА ТаблицаОбъектов.Документ ССЫЛКА Документ.УПЖКХ_ЗакрытиеПериодаПоНачислениямЖКХ
    //|				И втОтветственныеСобственникиНаниматели.ОтветственныйВладелец
    //|				И втОтветственныеСобственникиНаниматели.ОтветственныйВладелец ССЫЛКА Справочник.Контрагенты
    //|			ТОГДА ВЫРАЗИТЬ(втОтветственныеСобственникиНаниматели.ОтветственныйВладелец КАК Справочник.Контрагенты)
...
Функция ГП_СобратьДанныеЧеков(ТаблицаОплат, ПараметрыПодключения)
    ...
    Для Каждого ТекущаяОплата Из ТаблицаОплатСДопДанными Цикл
        ...
        //Если СтруктураПараметров.ГП_ФормироватьЧекиЖКХПоКонтрагентуДокумента = Истина
        //        И ЗначениеЗаполнено(ТекущаяОплата.ГП_КонтрагентДокументаОплаты) Тогда
        //    Отбор = Новый Структура("Документ, ГП_КонтрагентДокументаОплаты");
        //Иначе
        //    Отбор = Новый Структура("Документ, ЛицевойСчет");
        //КонецЕсли;
