...
Функция ГП_ПолучитьДопДанныеПоОплатам(ТаблицаОплат)
    ...
    #Удаление // Гарант+ Килипенко 07.04.2025 [F00235125] Печать чеков по контрагенту документа
    Возврат Запрос.Выполнить().Выгрузить();
    #КонецУдаления
    #Вставка // Гарант+ Килипенко 07.04.2025 [F00235125] Печать чеков по контрагенту документа ++ {

    РезультатФункции = Запрос.Выполнить().Выгрузить();

    // Механизм заполнения чека по контрагенту
    Если ГП_ИспользуетсяФормированиеЧековПоКонтрагентуДокумента(Неопределено) Тогда
        // Заполнение данных контрагента
        Для Каждого СтрокаРезультата Из РезультатФункции Цикл
            СтруктуруПредставленияПлательщикаЧека =
                ГП_РегистрацияОплатыСлужебный.ПолучитьСтруктуруПредставленияПлательщикаЧека(СтрокаРезультата.Документ, Истина, Ложь);
            ...
            СтрокаРезультата.ОтветственныйВладелецПредставление = СокрЛП(СтруктуруПредставленияПлательщикаЧека.ПредставлениеПлательщика);
            СтрокаРезультата.ИННОтветственного = СокрЛП(СтруктуруПредставленияПлательщикаЧека.СведенияОПокупателе.ИНН);
            СтрокаРезультата.ЭтоФизЛицо = СокрЛП(СтруктуруПредставленияПлательщикаЧека.ЭтоФизЛицо);
        КонецЦикла;
    КонецЕсли;

    Возврат РезультатФункции;

    #КонецВставки // Гарант+ Килипенко 07.04.2025 [F00235125] Печать чеков по контрагенту документа -- }
...
Функция ГП_ПолучитьРасшифровкуОплатДляЧеков(ТаблицаОплатСДопДанными, СтруктураПараметров)
	...
	Если СтруктураПараметров.ПечататьЧекиСРасшифровкойПоУслугам Тогда
		...
		Если СтруктураПараметров.РассчитыватьКоличествоИЦенуВПлатежах Тогда
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	втТариф.Документ КАК Документ,
			|	втТариф.ЛицевойСчет КАК ЛицевойСчет,
			|	втТариф.Услуга КАК Услуга,
			|	МАКСИМУМ(втТариф.МесяцНачисления) КАК МесяцНачисления
			|ПОМЕСТИТЬ ВтПоследняяДатаИзмененияТарифа
			|ИЗ
			|	втТариф КАК втТариф
			|
			|СГРУППИРОВАТЬ ПО
			...
			|	втТариф.ЛицевойСчет КАК ЛицевойСчет,
			|	втТариф.Услуга КАК Услуга,
			|	втТариф.Тариф КАК Тариф
			|ПОМЕСТИТЬ втПоследниеТарифы
			|ИЗ
			|	втПоследняяДатаИзмененияТарифа КАК втПоследняяДатаИзмененияТарифа
			|		ЛЕВОЕ СОЕДИНЕНИЕ втТариф КАК втТариф
			|		ПО (втТариф.Документ = втПоследняяДатаИзмененияТарифа.Документ)
			|			И (втТариф.ЛицевойСчет = втПоследняяДатаИзмененияТарифа.ЛицевойСчет)
			|			И (втТариф.Услуга = втПоследняяДатаИзмененияТарифа.Услуга)
			|			И втПоследняяДатаИзмененияТарифа.МесяцНачисления = втТариф.МесяцНачисления
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВтПоследняяДатаИзмененияТарифа
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|	ЕСТЬNULL(втПоследниеТарифы.Тариф, 0) КАК Тариф,
			|	втДанныеПоВидамОплатБезТарифа.ЭтоПени КАК ЭтоПени,
			...
			|ИЗ
			|	втДанныеПоВидамОплатБезТарифа КАК втДанныеПоВидамОплатБезТарифа
			|		ЛЕВОЕ СОЕДИНЕНИЕ втПоследниеТарифы КАК втПоследниеТарифы
			|		ПО (втДанныеПоВидамОплатБезТарифа.Документ = втПоследниеТарифы.Документ
			|			И втДанныеПоВидамОплатБезТарифа.ЛицевойСчет = втПоследниеТарифы.ЛицевойСчет
			|			И втДанныеПоВидамОплатБезТарифа.Услуга = втПоследниеТарифы.Услуга)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ втПоследниеТарифы
			|;";
		Иначе
			Запрос.Текст = Запрос.Текст + "
			|ВЫБРАТЬ
	...
	РасшифровкаОплатДляЧеков.Колонки.Добавить("СуммаНДС", Новый ОписаниеТипов("Число"));
	...
	Для Каждого ТекущаяСтрока Из РасшифровкаОплатДляЧеков Цикл
		...
		ТекущаяСтрока.СтавкаНДС = УПЖКХ_ОбщегоНазначенияСервер.ОпределитьСтавкуНДСВСтрокеОплатЧека(ТекущаяСтрока.ВидСтавкиНДС,
																									ТекущаяДата,
																									ТекущаяСтрока.ПризнакСпособаРасчета,
		...
		Если Не ТекущаяСтрока.СтавкаНДС = Неопределено Тогда
			ЗначениеНеРасчетнойСтавки = УПЖКХ_ОбщегоНазначенияСервер.ОпределитьСтавкуНДСВСтрокеОплатЧека(ТекущаяСтрока.ВидСтавкиНДС,
																											ТекущаяДата,
																											ТекущаяСтрока.ПризнакСпособаРасчета,
																											Ложь,
																											СтавкаНДССУчетомУСН);
			СтавкаНДСДляРасчетаСуммы = УПЖКХ_ТиповыеМетодыВызовСервера.ПолучитьСтавкуНДС(ЗначениеНеРасчетнойСтавки);
			ТекущаяСтрока.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекущаяСтрока.Сумма, Истина, СтавкаНДСДляРасчетаСуммы);
		КонецЕсли;
	...
	Иначе
		КолонкиГруппировки = "Документ, ЛицевойСчет, ВидУслугиПредставление, ЭтоПени, ЭтоАванс, ПризнакПредметаРасчета,"
		+ " ПризнакСпособаРасчета, Тариф, ЭтоДобровольноеСтрахование, УказыватьВЧекеПризнакАгентаВПлатежах, СтавкаНДС";

		Если СтруктураПараметров.УказыватьВЧекеПризнакАгентаВПлатежах Тогда
			КолонкиГруппировки = КолонкиГруппировки + ", ИННПоставщикаУслуги, ТелефонПоставщикаУслуги, ПоставщикУслугиПредставление"
		КонецЕсли;
        #Вставка // Гарант+ Килипенко 24.02.2025 [F00235125] Печать чеков по контрагенту документа ++
    ...
    #Удаление // Гарант+ Килипенко 17.04.2025 Обновление ++
	РасшифровкаОплатДляЧеков.Свернуть(КолонкиГруппировки, "Сумма, СуммаНДС");
    #КонецУдаления
    #Вставка // Гарант+ Килипенко 17.04.2025 Обновление ++

    Если НЕ СтруктураПараметров.ГП_ФормироватьЧекиЖКХПоКонтрагентуДокумента Тогда
        РасшифровкаОплатДляЧеков.Свернуть(КолонкиГруппировки, "Сумма, СуммаНДС");
    КонецЕсли;

    #КонецВставки // Гарант+ Килипенко 17.04.2025 Обновление --
...
Процедура ГП_ДобавитьСведенияОбОплатеВЧек(МассивЧеков, ТекущаяОплата, РасшифровкаТекущейОплаты, СтруктураПараметров)
	...
	Если СтруктураПараметров.ПечататьЧекиСРасшифровкойПоУслугам Тогда
        ...
        Если НЕ СтруктураПараметров.ГП_ФормироватьЧекиЖКХПоКонтрагентуДокумента Тогда
            УПЖКХ_ОбщегоНазначенияСервер.ДополнитьПозицииЧекаОснованиемИСуммой(
                ПозицииЧека,
                СтрокаНаименованиеПлательщикаИОснования,
                ШиринаСтрокиЧека,
                СуммаЧека);
	...
	Иначе
		...
		Для Каждого ТекущаяСтрокаОплаты Из РасшифровкаТекущейОплаты Цикл
			...
			ФискальнаяСтрокаПозицииЧека.СуммаНДС               = ТекущаяСтрокаОплаты.СуммаНДС;
