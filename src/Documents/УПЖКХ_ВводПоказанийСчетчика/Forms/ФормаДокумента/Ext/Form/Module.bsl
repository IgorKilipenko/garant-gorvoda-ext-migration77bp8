...
Функция ГП_ПолучитьСписокЛицевыхСчетовДокументаНаСервере(Знач ДокументСсылка)
    ...
    // Дополнение зависимыми услугами (Только Канализация)
    УслугаКанализация = ГП_РаботаСУслугами.ПолучитьУслугуКанализации().Ссылка;
    ДатаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Дата");
    НачалоПериодаДокумента = НачалоМесяца(ДатаДокумента); // !!! Уточнить начало периода для поиска зависимых услуг
    КонецПериодаДокумента = ДатаДокумента;
    КэшДополнительныхУслуг = Новый Соответствие; // Кэш дополнительных услуг для объединения
    Для Каждого ЭлементКЗ Из РезультатФункции Цикл
        Для Каждого ЭлементЛС Из ЭлементКЗ.Значение Цикл
            МассивЗависимыхУслуг = ГП_РаботаСУслугами.ПолучитьСписокЗависимыхУслугЗданияЛицевогоСчетаЗаПериод(
                    ЭлементЛС.Значение, ЭлементКЗ.Ключ, НачалоПериодаДокумента, КонецПериодаДокумента);
            Если МассивЗависимыхУслуг.Количество() = 0 Тогда
                Продолжить; // Нет зависимых услуг
            КонецЕсли;

            Для Каждого ЭлементУслуга Из МассивЗависимыхУслуг Цикл
                Если ЭлементУслуга <> УслугаКанализация Тогда
                    Продолжить; // Игнорируем если НЕ Канализация
                КонецЕсли;

                ДобавитьУслугуВКонтейнер(ЭлементУслуга, ЭлементЛС.Значение, КэшДополнительныхУслуг);
            КонецЦикла;
        КонецЦикла;
    КонецЦикла;

    Для Каждого ЭлементКЗ Из КэшДополнительныхУслуг Цикл
        Для Каждого ЭлементЛС Из ЭлементКЗ.Значение Цикл
            ДобавитьУслугуВКонтейнер(ЭлементКЗ.Ключ, ЭлементЛС.Значение, РезультатФункции);
        КонецЦикла;
    КонецЦикла;

    // // Очистка дублей л/с
    // Для каждого ЭлементКЗ Из РезультатФункции Цикл
    //     ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(ЭлементКЗ.Значение);
    // КонецЦикла;
