...
Асинх Процедура ГП_НачислитьУслугиПоПоказаниямПриНажатии(Команда)
    ...
    Если ДанныеЗаполненияНачислений.Количество() = 1 Тогда
        УслугаДляНачисления = Неопределено;
        Для Каждого ЛСУслугиКЗ Из ДанныеЗаполненияНачислений Цикл
            УслугаДляНачисления = ЛСУслугиКЗ.Ключ;
            Прервать;
        КонецЦикла;
        ФормаНачисленияУслуг = ОткрытьНовыйДокументНачисленияУслуг(УслугаДляНачисления, ЛСУслугиКЗ.Значение);

    Иначе
        СписокУслугДляНачисления = Новый СписокЗначений;
        Для Каждого ЛСУслугиКЗ Из ДанныеЗаполненияНачислений Цикл
            СписокУслугДляНачисления.Добавить(ЛСУслугиКЗ.Ключ);
        КонецЦикла;

        ГП_РаботаСУслугамиКлиент.ОткрытьФормуВыбораУслуги(
            Новый Структура("ОтображатьСписком, Отбор", Истина, Новый Структура("ГП_Ссылка", СписокУслугДляНачисления)),
            Новый ОписаниеОповещения(ОбработкаВыбораУслугПриСозданииНачислений, ЭтотОбъект,
                Новый Структура("ДанныеЗаполненияНачислений", ДанныеЗаполненияНачислений)),
            ЭтотОбъект);
    КонецЕсли;
КонецПроцедуры
...
// Обработчик выбора услуги при создании документа начислений
&НаКлиенте
Процедура ОбработкаВыбораУслугПриСозданииНачислений(РезультатВыбора, ДополнительныеПараметры) Экспорт
	Если РезультатВыбора = Неопределено Тогда
		Возврат;
	КонецЕсли;

    СписокЛицевыхСчетов = ДополнительныеПараметры[РезультатВыбора];
    ФормаНачисленияУслуг = ОткрытьНовыйДокументНачисленияУслуг(ДополнительныеПараметры[РезультатВыбора], СписокЛицевыхСчетов);
...
//  - Форма
&НаКлиенте
Функция ОткрытьНовыйДокументНачисленияУслуг(Знач Услуга, Знач СписокЛицевыхСчетов)
