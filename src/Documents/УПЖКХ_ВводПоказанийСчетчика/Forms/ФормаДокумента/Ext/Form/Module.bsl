...
Функция ГП_ЗаполнитьДневныеПоказанийВиртуальныхСчетчиковНаСервере()
    ...
    Для Каждого СтрокаДанных Из ЭтотОбъект.Объект.Главная Цикл
        ...
        СтруктураДанныхСчетчика = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаДанных.Счетчик,
                Новый Структура("ГП_ЭтоВиртуальныйСчетчик, СпособРегистрацииПоказаний"));

        ЭтоСчетчикСРасчетомПоСреднему = СтруктураДанныхСчетчика.СпособРегистрацииПоказаний =
            Перечисления.КВП_СпособыРегистрацииПоказанийСчетчиков.РасходЗаПериодРасчета;

        Если ЭтоСчетчикСРасчетомПоСреднему = Ложь Тогда
            Продолжить; // Это не виртуальный счетчик по среднему
        КонецЕсли;

        СтруктураКоличестваПоСреднему = ГП_ПриборыУчета.ПолучитьТекущееКоличествоПоСреднемуСчетчика(СтрокаДанных.Счетчик,
            ?(ЗначениеЗаполнено(ЭтотОбъект.Объект.Дата), ЭтотОбъект.Объект.Дата, Неопределено));

        Если СтруктураКоличестваПоСреднему.Успех = Ложь ИЛИ СтруктураКоличестваПоСреднему.Значение = 0 Тогда
            Продолжить; // Нет показаний
        КонецЕсли;

        СтрокаДанных.ДневноеПоказание = СтруктураКоличестваПоСреднему.Значение;
