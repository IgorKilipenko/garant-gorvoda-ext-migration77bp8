...
Асинх Процедура ГП_НачислитьУслугиПоПоказаниямПриНажатии(Команда)
    ...
    ЗначенияЗаполненияНачислений = Новый Структура("ВидОперации",
            ПредопределенноеЗначение("Перечисление.КВП_ВидыОперацийНачисленияУслуг.НачисленияПоПоказаниямПУ"));
    ПараметрыВводаПоказаний = Новый Структура("ИзменитьВидОперации, ЗначенияЗаполнения", Истина,
            ЗначенияЗаполненияНачислений);

    ПараметрыВводаПоказаний.Вставить("Основание", ЭтотОбъект.Объект.Ссылка);
    ...
    ФормаНачисленияУслуг.Объект.Дата = ЭтотОбъект.Объект.Дата;
КонецПроцедуры
