...
Процедура ГП_ЗаполнитьПоБУПриНажатии(Команда)
    ...
    Если РезультатЗаполнения = Истина Тогда
        ПриходныйКассовыйОрдерФормыКлиентСервер.ОбновитьИтоги(ЭтотОбъект);
...
Процедура ГП_УстановитьВидимостьДополнительныхЭлементов(Форма)
    Если Форма.ВидОперации = ПредопределенноеЗначение(
            "Перечисление.ВидыОперацийПКО.ОплатаПокупателя") Тогда
...
Функция ГП_ЗаполнитьРасшифровкуПоБУНаСервере()
    ...
    // Резервная копия таблицы расшифровки для восстановления
    РезервТаблицыРасшифровки = ЭтотОбъект.РасшифровкаПлатежа.Выгрузить();

    // Сбор общих данных заполнения расшифровки (по первой строке ТЧ)
    ...
    // Сбор общих данных документа оплат
    ОбщиеДанныеДокументаОплаты = Новый Структура("Контрагент, Дата, Организация, СуммаОплаты");
    ОбщиеДанныеДокументаОплаты.Контрагент = ЭтотОбъект.Контрагент;
    ОбщиеДанныеДокументаОплаты.Дата = ЭтотОбъект.ТекущаяДатаДокумента;
    ОбщиеДанныеДокументаОплаты.Организация = ЭтотОбъект.Организация;
    ОбщиеДанныеДокументаОплаты.СуммаОплаты = РезервТаблицыРасшифровки.Итог("СуммаПлатежа");
    Если ЗначениеЗаполнено(ОбщиеДанныеДокументаОплаты.Дата) = Ложь И ЗначениеЗаполнено(ЭтотОбъект.Ссылка) = Истина Тогда
        ОбщиеДанныеДокументаОплаты.Дата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
                ЭтотОбъект.Ссылка, "Дата");
    КонецЕсли;
    ОбщиеДанныеДокументаОплаты.Дата = ?(ЗначениеЗаполнено(ОбщиеДанныеДокументаОплаты.Дата),
            ОбщиеДанныеДокументаОплаты.Дата, ТекущаяДатаСеанса());

    // Распределение долга:
    ДанныеРаспределения = ГП_РаботаСПоступлениемПлатежей.РаспределитьСуммуДокументаОплатыУслуг(ОбщиеДанныеДокументаОплаты);
    ...
    Для Каждого СтрокаДанных Из ДанныеРаспределения.ТаблицаРаспределения Цикл
        ...
        НоваяСтрокаРасшифровки.НомерСтроки = ТекущийНомерСтроки;

        ПриходныйКассовыйОрдерФормыКлиентСервер.ПересчитатьСуммуНДС(НоваяСтрокаРасшифровки);
    КонецЦикла;

    РаспределенныйДолг = ЭтотОбъект.РасшифровкаПлатежа.Итог("СуммаПлатежа");
    НераспределенныйОстаток = ОбщиеДанныеДокументаОплаты.СуммаОплаты - РаспределенныйДолг;

    Если НераспределенныйОстаток > 0 Тогда
        // Распределение аванса:
        ОбщиеДанныеДокументаОплаты.СуммаОплаты = НераспределенныйОстаток;
        ДанныеРаспределения = ГП_РаботаСПоступлениемПлатежей.РаспределитьОстатокПлатежаВПропорцииНачислений(ОбщиеДанныеДокументаОплаты);
        ТекущийНомерСтроки = ЭтотОбъект.РасшифровкаПлатежа.Количество();
        Для Каждого СтрокаДанных Из ДанныеРаспределения.ТаблицаРаспределения Цикл
            ТекущийНомерСтроки = ТекущийНомерСтроки + 1;
            НоваяСтрокаРасшифровки = ЭтотОбъект.РасшифровкаПлатежа.Добавить();
            ...
            НоваяСтрокаРасшифровки.ГП_НоменклатурнаяГруппа = СтрокаДанных.НоменклатурнаяГруппа;
            НоваяСтрокаРасшифровки.СуммаПлатежа = СтрокаДанных.Сумма;
            НоваяСтрокаРасшифровки.СуммаВзаиморасчетов = СтрокаДанных.Сумма;
            НоваяСтрокаРасшифровки.НомерСтроки = ТекущийНомерСтроки;
    ...
    Если ЭтотОбъект.РасшифровкаПлатежа.Количество() = 0 Тогда
        Для Каждого СтрокаДанных Из РезервТаблицыРасшифровки Цикл
            НоваяСтрокаРасшифровки = ЭтотОбъект.РасшифровкаПлатежа.Добавить();
            ЗаполнитьЗначенияСвойств(НоваяСтрокаРасшифровки, СтрокаДанных);
        КонецЦикла;

        ОбщегоНазначения.СообщитьПользователю("Внимание! Не удалось выполнить заполнение таблицы расшифровки."
            + " Данные оставлены без изменения");
