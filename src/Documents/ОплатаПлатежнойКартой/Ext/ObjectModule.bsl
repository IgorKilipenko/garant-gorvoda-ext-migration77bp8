...
Процедура ГП_ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    ВидДвиженияОплатаПоУслугам = Неопределено;
    Если ЭтотОбъект.ВидОперации = Перечисления.ВидыОперацийОплатаПлатежнойКартой.ОплатаПокупателя Тогда
        ВидДвиженияОплатаПоУслугам = ВидДвиженияНакопления.Приход;
    ИначеЕсли ЭтотОбъект.ВидОперации = Перечисления.ВидыОперацийОплатаПлатежнойКартой.ВозвратПокупателю Тогда
        ВидДвиженияОплатаПоУслугам = ВидДвиженияНакопления.Расход;
    КонецЕсли;
    
    Если ВидДвиженияОплатаПоУслугам = Неопределено Тогда
        Возврат;
    КонецЕсли;
