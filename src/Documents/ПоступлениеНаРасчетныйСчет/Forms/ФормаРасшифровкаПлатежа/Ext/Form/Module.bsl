...
Функция ГП_ЗаполнитьРасшифровкуПоБУНаСервере()
    ...
    ОбщиеДанныеДокументаОплаты = Новый Структура("Контрагент, Дата, Организация, СуммаДокумента");
    Если ЗначениеЗаполнено(ЭтотОбъект.Ссылка) Тогда
        ОбщиеДанныеДокументаОплаты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
                ЭтотОбъект.Ссылка, ОбщиеДанныеДокументаОплаты);
        ОбщиеДанныеДокументаОплаты.Вставить("СуммаОплаты", ЭтотОбъект.СуммаДокумента);
    Иначе
    ...
    Иначе
        ОбщегоНазначения.СообщитьПользователю("Не удалось выполнить заполнение расшифровки. Отсутствуют остатки взаиморасчетов");
        Возврат Ложь;
    КонецЕсли;
