...
Функция ГП_ПолучитьДанныеДляЗаполненияОтраженияПоЛицевымСчетам(Знач СтруктураНастроекКапРемонта, Знач ВыводитьСообщения = Истина)
    ...
    Если ЗначениеЗаполнено(ЭтотОбъект.Контрагент) Тогда
		Если ЭтотОбъект.Контрагент.ЭтоГруппа Тогда
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.НоменклатурнаяГруппа) Тогда
		Если ЭтотОбъект.НоменклатурнаяГруппа.ЭтоГруппа Тогда
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Взаиморасчеты) Тогда
		Если ЭтотОбъект.Взаиморасчеты = 1 Тогда
			ТекстДопУсловия = ТекстДопУсловия + " И НЕ ЭтоЛьгота И НЕ ЭтоПени";
		ИначеЕсли ЭтотОбъект.Взаиморасчеты = 2 Тогда
			ТекстДопУсловия = ТекстДопУсловия + " И ЭтоЛьгота";
		ИначеЕсли ЭтотОбъект.Взаиморасчеты = 3 Тогда
