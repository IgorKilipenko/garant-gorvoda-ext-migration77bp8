...
Процедура ГП_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
    ...
    // Очистка документов отражения при установки пометки на удаления
    Если НЕ ЭтотОбъект.ИспользуетсяНовыйМеханизмОтраженияНачислений Тогда
        Если ЭтотОбъект.Ссылка.Пустая() = Ложь И ЭтотОбъект.ПометкаУдаления = Истина Тогда
            ИсходноеЗначениеПризнакаПометкаУдаления = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭтотОбъект.Ссылка, "ПометкаУдаления");
            Если ИсходноеЗначениеПризнакаПометкаУдаления <> ЭтотОбъект.ПометкаУдаления Тогда
                РезультатУдаленияДокументов = ГП_ОтражениеВРегУчетеСлужебный.ПометитьНаУдалениеВсеДокументОтраженияНачислений(ЭтотОбъект);
                Если РезультатУдаленияДокументов.Успех = Ложь Тогда
                    ОбщегоНазначения.СообщитьПользователю("Ошибка при очистке документов отражения");
                    ОбщегоНазначения.СообщитьПользователю(РезультатУдаленияДокументов.ТекстСообщения);
                    Отказ = Истина;

                    Возврат; // Ошибка при удалении документов - ОТКАЗ
                КонецЕсли;
            КонецЕсли;
        КонецЕсли;
    КонецЕсли;
...
Процедура ГП_СформироватьДокументОтраженияНачислений(
    ...
    // Гарант+ Килипенко 18.11.2024 [F00229948] Период начислений в документах реализации ++
    //
    ДокументРеализацииОбъект.ГП_ПериодНачисленияНачало = ЭтотОбъект.ГП_ПериодНачисленияНачало;
    ДокументРеализацииОбъект.ГП_ПериодНачисленияОкончание = ЭтотОбъект.ГП_ПериодНачисленияОкончание;
    //
    // Гарант+ Килипенко 18.11.2024 [F00229948] Период начислений в документах реализации --

    СпециальныйБанковскийСчетОрганизации = ГП_РаботаСКонтрагентами.ПолучитьБанковскийСчетКонтрагентаДляРасчета(
            ЭтотОбъект.Контрагент, ?(ЗначениеЗаполнено(ЭтотОбъект.Дата), ЭтотОбъект.Дата, Неопределено));
    Если СпециальныйБанковскийСчетОрганизации <> Неопределено Тогда
        ДокументРеализацииОбъект.БанковскийСчетОрганизации = СпециальныйБанковскийСчетОрганизации;
    КонецЕсли;
