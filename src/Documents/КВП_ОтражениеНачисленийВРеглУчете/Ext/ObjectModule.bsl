...
Функция ГП_ПолучитьТаблицуОтражениеНачислений()
    Если ГП_ЭтоЗаполнениеОтраженияПоЛицевымСчетам() = Ложь Тогда
        РезультатФункции = ПродолжитьВызов();
        Возврат РезультатФункции; // Штатный механизм заполнения
    КонецЕсли;

    // Альтернативный (с учетом л/с) механизм заполнения отражения
...
Процедура ГП_ВыполнитьСортировкуУслугДокументаРеализации(Знач ДокументРеализацииОбъект)
    ...
    ШаблонОчисткиСодержания = "\s*,\s*Показания\s+счетчиков[:]\s+([0-9]*\s+-\s+[0-9]+(;\s+)?)+\s*$";
