...
&НаСервере
&ИзменениеИКонтроль("ПометитьНаУдалениеДокументОтраженияНачислений")
Процедура ГП_ПометитьНаУдалениеДокументОтраженияНачислений(ТекущаяСтрока, МассивСсылокУдаляемыхДокументов)
    ...
    Если ЗначениеЗаполнено(ТекДокумент)
        ...
        Попытка
            ...
            #Вставка // Гарант+ Килипенко 22.01.2025 Удаление счетов на оплату ++
            
            Если ТипЗнч(ДокОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг")
                И ЗначениеЗаполнено(ДокОбъект.СчетНаОплатуПокупателю) Тогда

                СчетПокупателюОбъект = ДокОбъект.СчетНаОплатуПокупателю.ПолучитьОбъект();
                СчетПокупателюОбъект.УстановитьПометкуУдаления(Истина);
            КонецЕсли;
            
            УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьОбОшибке("Документ <" + Строка(ДокОбъект.СчетНаОплатуПокупателю) + "> помечен на удаление.");
            #КонецВставки // Гарант+ Килипенко 22.01.2025 Удаление счетов на оплату --
...
Функция ГП_СоздатьКомандуЗаполнитьПоЛицевымСчетамНаСервере()
    ...
    РезультатФункции.Кнопка.Отображение = ОтображениеКнопки.КартинкаИТекст;
...
Функция ГП_СоздатьЭлементФормыПериодНачисленияНаСервере()
    ...
    // Создание группы
    НовыйГруппаПериод = ЭтотОбъект.Элементы.Добавить(
            "ГП_ПериодНачисления",
            Тип("ГруппаФормы"),
...
Процедура ГП_УстановитьОграниченияФункциональностиФормыНаСервере()
    ...
    // Настройка отображения штатной кнопки Заполнить
    ЭтотОбъект.Элементы.КонтрагентыУпрЗаполнить1.ЦветФона = ЦветаСтиля.ЦветФонаПодвалаТаблицы;
    ЭтотОбъект.Элементы.КонтрагентыУпрЗаполнить1.Картинка = Новый Картинка;
КонецПроцедуры
